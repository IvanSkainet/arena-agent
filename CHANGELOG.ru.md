## v4.82.1 - Хотфикс конфликта типов RECT в Window Action на Windows

### Цель

В релизе `v4.82.0` при вызове нативного Windows Window Action возникал конфликт типов ctypes. Функция `GetWindowRect` ожидает строгий тип `ctypes.POINTER(wt.RECT)`, тогда как внутри `window_action.py` мы создавали и передавали локальную структуру `RECT`. Это приводило к ошибке `ArgumentError` в ctypes и возвращало 500 ошибку при вызовах `/v1/desktop/window_action` на Windows.

Этот релиз — критический хотфикс, заменяющий локальный класс на стандартный тип `ctypes.wintypes.RECT` (`wt.RECT`), что полностью решает конфликт типов.

### Изменения

* **Исправлен нативный Window Action на Windows (`perform_window_action`):**
  - Локальный класс `RECT` заменен на стандартную структуру `ctypes.wintypes.RECT` (`wt.RECT`) из модуля `ctypes.wintypes`.
  - Успешно проверено перемещение, изменение размеров и восстановление окон на живом Windows-хосте.
