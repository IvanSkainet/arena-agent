...
### Выключение моста оставляло запись работать на телефоне

В `arena/mobile/mirror.py` есть `stop_all()`. `arena/mobile/__init__.py`
реэкспортирует её как `mirror_stop_all`. В самом mirror.py есть фраза
«`stop_all()` при выключении моста страдал тем же» — написанная мной
цикл назад, в предположении вызывающего, которого не существует.

Её не вызывал никто. Нигде в дереве.

Каждая живая сессия зеркала держит процесс `screenrecord` **на
телефоне**. Убийство моста оставляло его писать, заполняя хранилище
устройства, и локального процесса, который знает, как его остановить, не
оставалось — а перезапуск моста сироту не подхватывает, он заводит вторую
сессию. Проверено: сессия с заглушенным долгим screenrecord пережила
`on_cleanup` совершенно нетронутой.

Теперь её останавливают оба пути завершения. `on_cleanup` — это штатное
выключение aiohttp; `signal_handler` заканчивается `os._exit(0)` по
пятисекундному таймеру и на SIGTERM может сработать раньше, чем cleanup
завершится. Процесс браузера убивался в обоих местах именно по этой
причине, и асимметрия «браузер обработан дважды, рекордер — ноль раз» как
раз и держала пробел невидимым.

Ворота проверяют **вызов**, а не наличие функции, и разбирают AST, а не
ищут по тексту: упоминание в комментарии — ровно то, что обмануло меня в
прошлый раз, а в lifecycle.py их теперь несколько. Отдельный тест
по-настоящему вызывает `stop_all()` против заглушенных сессий, потому что
пустая реализация удовлетворила бы любую проверку по AST.

### Сторонний MCP-сервер мог наращивать память моста без предела

`arena/mcp_client/client.py` (73.9% покрытия) запускает внешние
MCP-серверы — `npx some-server`, `uvx another` — и читает их stdout в
фоновом потоке. Ни длина строки, ни глубина очереди не были ограничены:

    self._stdout_q: queue.Queue[str | None] = queue.Queue()
    for line in self.proc.stdout:
        self._stdout_q.put(line)

Замер с заглушкой, которая отвечает на `initialize`, а затем льёт строки
по 100 КБ: RSS вырос с 13 МБ до **1433 МБ за четыре секунды**, 14 933
строки в очереди и рост продолжался. С ограничениями та же заглушка
стоит 96 МБ, а очередь упирается в потолок.

Злой умысел здесь не главная причина. Сервер, который по ошибке пишет
свой отладочный лог в stdout вместо stderr, делает ровно это, а умирает
при этом мост, а не сбойный сервер, — и тот, кто станет разбираться,
увидит нехватку памяти у моста и закономерно обвинит мост.

Два решения стоит проговорить. Слишком большие кадры **отбрасываются, а
не обрезаются**: строка в 4+ МБ, разрезанная пополам, — это невалидный
JSON, который парсер всё равно пропустит, так что хранить огрызок значило
бы только сделать логи менее честными. А при переполнении уходит
**самая старая** строка, не новая: вызывающий блокируется, пока не увидит
свой `id` ответа, поэтому выбрасывание нового вывода превратило бы
болтливый сервер в зависание.

С EOF-маркером так же: в полной очереди для него обязано найтись место,
иначе `request()` выждет весь таймаут вместо того, чтобы сообщить
«сервер закрыл соединение».

### Живые метрики выдумывали гигабайт в секунду

`arena/observability/live_metrics.py` (38.6% покрытия) считает каждую
скорость как разницу счётчиков, делённую на время с прошлого снимка, и
снизу этот интервал ничем не ограничен.

Два вызывающих делят один модульный образец: WebSocket-поток дашборда и
обычный `GET /v1/live-metrics`. Когда они попадают одновременно, делитель
измеряется микросекундами:

    1000 байт за 1 микросекунду -> 999 992 385 байт/с

На работающем мосту это наблюдалось как `cpu=75.0` на простаивающей
машине, два раза подряд, с падением до `0.9`, когда опросы разнесли по
времени.

Чинить это стоит из-за направления ошибки. Панель мониторинга, которая
слегка врёт, — мелкое неудобство; панель, которая иногда выдумывает
всплеск в 1 ГБ/с, — это панель, читатель которой приучается не обращать
на неё внимания, а потом пропускает настоящий всплеск. Такой отказ учит
неверному.

Снимок, запрошенный раньше чем через 250 мс после предыдущего, теперь
переиспользует прошлые скорости и сообщает `stale: true` с причиной,
вместо деления почти на ноль. Абсолютные значения счётчиков остаются
живыми — это показания, а не разницы. Порог заметно ниже частоты опроса
дашборда (1 Гц), и тест это фиксирует: подними его выше секунды — он
задушит сам дашборд, лента будет выглядеть замершей, и починку не
поправят, а откатят.

Заодно проверены два соседних режима отказа, и оба уже обработаны: сброс
счётчика (переконфигурация сетевой карты) и сдвиг стенных часов назад
(коррекция NTP, переход на зимнее время) дают ноль, а не отрицательную
скорость. Теперь это тоже зафиксировано.

Также осмотрен и признан чистым `agentctl_extras/status.py` — второй файл
с нулевым покрытием в верхушке рейтинга. Все его вызовы подпроцессов в
argv-форме с литеральными командами, а печатаемое в рабочей копии
`hwinfo.py not found` — корректное поведение: `ROOT` указывает на каталог
установки, а сам скрипт в релизном архиве присутствует.

### Один туннель был без аутентификации, а ворота, которые должны были это поймать, были слепы

`arena/admin/handlers.py` (25.5% покрытия) выставляет пять транспортов
туннелей, которые публикуют этот мост в интернет. У четырёх стоит
`@authed`. У `bore`, добавленного пятым, — нет: форму скопировали,
декоратор не перенесли.

Проверено против работающего моста:

    POST /v1/bore/tunnel/status   -> 200, без учётных данных
    POST /v1/ngrok/tunnel/status  -> 401

`start` и `stop` тоже отвечали. Бинарника bore здесь нет, поэтому `start`
остановился на «binary not found», но на машине, где он установлен,
неавторизованный локальный вызов мог опубликовать мост наружу.

Интереснее вторая половина — почему никто не заметил.
`tests/test_auth_surface_guard.py` существует ровно затем, чтобы обойти
все маршруты и убедиться, что они отказывают анонимному вызывающему, — и
он проходил. Его построитель путей сохранял `{path}`, но всё прочее
обрезал по первой скобке, так что `/v1/bore/tunnel/{action}` запрашивался
как `/v1/bore/tunnel`. Такого маршрута нет. 404 попадал в
`BENIGN_STATUSES`, маршрут засчитывался проверенным, а его обработчик не
запускался ни разу.

Это слепое пятно покрывало **66 из 274 зарегистрированных маршрутов** —
каждый путь с параметром, отличным от `{path}`: все пять туннелей,
эндпоинты ZeroTier и Central, вся мобильная поверхность по серийнику,
autostart по транспорту, статус запусков кода. У четверти API были
ворота, которые отчитывались о ней, не проверяя её.

Переменные шаблонов теперь подставляются правдоподобными значениями, а не
обрезаются, и второй тест проверяет, что обход действительно доходит:
шаблонный маршрут, ответивший 404, означает, что зонд не достиг
обработчика, и теперь это роняет сборку. Два файловых маршрута
(`/gui/assets/{path}`, `/gui/docs/{path}`) вынесены в явные исключения:
их 404 приходит от обработчика, а не от роутера, — правило пошире вроде
«пропускать /gui» разрослось бы и начало прятать настоящие пробелы.

Доказано саботажем в обе стороны: со старым обрезающим построителем
отсутствие `@authed` проходит чисто, с починенным — падает с
`200 POST /v1/bore/tunnel/{action}`.

### Аудит 200 закрытых алертов code scanning и один, который они прятали

Пользователь спросил, не слишком ли легко предыдущие агенты закрывали
алерты. Двести закрытий, проверенных исполнением, а не чтением
обоснований.

Почти все выдерживают проверку. 109 штук `DS162092` («обращение к
localhost») — это архитектура моста, а не отладочный код. Семь закрытий
`py/path-injection` высокой важности я переатаковал напрямую: десять
traversal-payload'ов против обработчика ассетов дашборда (`../`,
закодированные точки, каталог-сосед с общим префиксом, симлинк, подложенный
внутрь корня ассетов) и тринадцать против `standalone_tools._jail`
(абсолютные пути, `~/..`, симлинк внутри `$HOME`, указывающий на `/etc`,
запрещённые имена файлов) — отбито всё. Каждое `contents: write` выдано
джобе, вся суть которой в пуше, при `permissions: {}` наверху каждого
workflow. Пара `js/disabling-certificate-validation` указывала на
расширение браузера, которого в репозитории действительно больше нет.

Два обоснования оказались небрежнее решений, которые они защищали.
`artipacked` закрыт словами «persist-credentials: false добавлено во все
checkout-шаги»; два из 35 checkout'ов по-прежнему `true`. Это бот бейджа и
релокер Dependabot — оба пушат, у обоих стоит поясняющий комментарий:
решение верное, а фраза, его описывающая, — нет. Так же и
`security-events: write` у DevSkim закрыт как «удалено», хотя право на
месте и по-прежнему нужно для загрузки SARIF. Дыр нет ни там, ни там; но
это напоминание, что комментарий при закрытии — это утверждение, а
утверждения проверяемы.

Единственная настоящая находка — комментарий у пина:
`microsoft/DevSkim-Action` закреплён на SHA тега `v1.0.16`, а в
комментарии стояло `# v1` — на это и указывал `ref-version-mismatch` от
zizmor. Экшен при этом не архивирован: тот алерт просто устарел.
Комментарий исправлен, а не оспорен.

### CDP-эндпоинт мог сам выбрать, куда подключится клиент

`arena/browser/cdp_client/` спрашивает у отладочного HTTP-порта браузера
список вкладок и подключается к найденному `webSocketDebuggerUrl`. Этот
URL приходил из JSON-тела и открывался без проверки. Проверено
исполнением: заглушка на CDP-порту ответила

    {"type": "page", "webSocketDebuggerUrl":
     "ws://attacker.example:9999/devtools/page/STOLEN"}

и `get_websocket_url` вернул его для `AsyncCDPBrowser.connect`. CDP-сокет
читает любую страницу, любые cookie и любые нажатия клавиш в браузере,
так что «куда он указывает» — не деталь.

Запрос всегда идёт на `http://127.0.0.1:<порт>`, поэтому ответу теперь
доверено только *назвать вкладку на этом порту*: `ws`/`wss`,
loopback-хост, совпадающий порт. Всё прочее даёт None, что проявляется как
уже существующее «Cannot connect to browser CDP on port N».

Может ли злоумышленник управлять этим ответом — справедливый вопрос, и
ответ «иногда»: любой локальный процесс может занять свободный порт, а
значения `--remote-debugging-port` угадываемы. Закрыть дёшево — значит
закрыто.

### «Загрузка» в ASR умела читать локальные файлы

`arena/mcp/tool_asr.py` скачивает ffmpeg, whisper.cpp и модель, а первые
два потом запускает. `asr.bootstrap` берёт `model_url` и
`whisper_zip_url` прямо из вызова инструмента, а `_download_atomic`
передавал их в `urllib.request.urlopen` — который говорит на любой схеме,
какую дадут.

Проверено исполнением:

    _download_atomic("file:///etc/hostname", dest, force=True)
    -> {"ok": True, "size_bytes": 9}, и в dest лежало имя хоста

`http://` тоже принимался: незашифрованная загрузка бинарника, который
затем исполняется.

Одной проверки схемы было бы недостаточно, и это стоит записать.
Произвольный HTTPS всё равно позволяет навести bootstrap на любой сервер
и получить результат, запущенный как whisper-cli, — поэтому хост тоже
закреплён. Список разрешённых *выводится* из трёх констант с URL по
умолчанию, а не набирается рядом с ними: второй список, поддерживаемый
вручную, рано или поздно разъедется, и естественной реакцией будет
неаккуратно его расширить. Тест проверяет именно вывод из констант, так
что захардкоженный набор упадёт, даже если значения сегодня совпадают.

Это та же форма, что и баг с digest'ами рантаймов ранее в этом релизе:
код, который что-то скачивает и затем исполняет, доверяя вызывающему
выбор источника.

### Input Helper отдавал управление клавиатурой всем подряд

`arena/input_helper/helper_server.py` работает внутри интерактивной сессии
рабочего стола пользователя и выставляет по HTTP `/click`, `/move`,
`/type`, `/key` и `/launch`. Покрытие — 17%, а аутентификация была
необязательной:
...
Запущенный без аргументов — а именно так и советовала подсказка об ошибке
в самом MCP-инструменте — он обслуживал все эндпоинты для любого local
caller'а. Проверено исполнением: при пустом токене
`POST /launch {"path": "/bin/true"}` дошёл до `subprocess.Popen` и упал
только потому, что `CREATE_NEW_CONSOLE` существует лишь на Windows. На той
платформе, ради которой этот helper и написан, процесс бы запустился.

Привязка к 127.0.0.1 — это не аутентификация. Вкладка браузера с
недружественным JavaScript может слать POST на localhost, и любой другой
процесс или пользователь машины тоже. А этот конкретный loopback-порт —
пульт управления рабочим столом: мышь, клавиатура, запуск произвольных
процессов. Оставлять его открытым — локальное повышение привилегий в
обход всего.

Теперь он отказывается стартовать без токена и печатает, как его
сгенерировать, потому что отказ без средства исправления — это отказ,
который обходят. Проверка на уровне запроса тоже отказывает, а не
доверяет стартовой, и код выхода пробрасывается наружу, чтобы супервизор,
читающий только статус, не принял «я так работать не буду» за штатное
завершение.

Сравнение было через `==`, которое отдаёт общий секрет по байту любому,
кто может измерить задержку ответа. Оно охраняет инъекцию нажатий клавиш в
живой рабочий стол, поэтому теперь это `hmac.compare_digest`. Остальной
мост уже везде использовал сравнение за константное время — этот файл был
единственным исключением.

Подсказка MCP, которая учила небезопасному запуску, исправлена тем же
коммитом, и тест следит, чтобы она не вернулась.

## v4.163.0 — 2026-08-05

### Клик мышью был инъекцией команд — в файле с нулевым покрытием

Четвёртый в рейтинге опасности и самый показательный пункт в нём:
`arena/desktop/cli/input.py` управляет мышью и клавиатурой хост-машины, и
**ни одна его строка не исполнялась тестами**.

`click()` подставлял сырой argv в шелл-строки:

    run(f'xdotool mousemove --sync {args.x} {args.y}', timeout=3)
    run(f'ydotool click {args.button}', timeout=3)

`--button` объявлен как обычная строка без `choices`, поэтому
`--button "1; touch /tmp/PWNED"` создавал файл. То же с координатами.

Поучительно здесь то, в каком файле это жило. `key()` и `type_text()` уже
были аккуратны — `shq()` на одном пути, список argv на другом, — и
`move()` тоже был безопасен, но по другой причине: он прогоняет `int()`
над входными данными до форматирования. Три из четырёх точек входа были в
порядке, каждая своим механизмом, а четвёртая — нараспашку. Не хватало
единообразия, а не знания.

Починка следует инстинкту `move()`, а не добавляет четвёртый механизм:
координаты и кнопки — это числа, значит их надо разобрать и отвергнуть то,
что не разбирается. Кнопка вдобавок обязана быть одной из пяти настоящих —
ydotool делает с 99 что-то непредсказуемое, и отказ понятнее, чем что бы
это ни оказалось. `move()` теперь отвечает вместо `ValueError`, который
раньше превращал опечатку в трейсбек.

Храповик отмечает любое будущее `f'tool {args.something}'`, не обёрнутое в
`shq()`, чтобы следующая точка входа не стала тихо пятым механизмом.

### /v1/exec/script не работал ни у кого, у кого в пути есть пробел

Следующим в рейтинге «опасность x непокрытость» шёл
`arena/exec/handlers.py` с 11.3% покрытия — модуль, который выполняет
команды. Баг там не дыра в безопасности; это эндпоинт, который просто не
работает, молча, у изрядной доли пользователей.

`/v1/exec/script` пишет тело запроса во временный файл и подставляет путь
в шаблон интерпретатора:

    "bash": {"cmd": "bash -euo pipefail {path}", ...}
    full_cmd = template.format(path=tmp_path)

Эти шаблоны уходят в **шелл**, поэтому путь заново разбивается по
пробелам. Один пробел — и команда разваливается:

    bash -euo pipefail /tmp/root with space/.../scr-ab.sh
    -> bash: /tmp/root: No such file or directory

Воспроизведено от начала до конца через `run_shell_command_stream`: без
кавычек скрипт не запускался вовсе, с кавычками — выдал свой вывод.
Проверено для bash, sh и python3.

Это не экзотическая конфигурация. `C:\Users\Ivan Petrov` — то, что
Windows выдаёт учётной записи с именем из двух слов, а `~/My Drive/...`
обычное дело на macOS. Отказ ещё и вводит в заблуждение: он называет
каталог, который очевидно существует, поэтому естественный вывод — «мост
сломан», а не «закавычьте путь».

Квотирование зависит от платформы, и это важно: `shlex.quote` оборачивает
в одинарные кавычки, которые cmd.exe и PowerShell вообще не считают
кавычками, так что символы кавычек попали бы в имя файла. Windows
получает двойные кавычки; POSIX — shlex, который справляется и со
встроенными кавычками, `$`, обратными кавычками и точками с запятой. Семь
таких payload'ов зафиксированы как round-trip тесты: что бы ни было в
пути, `shlex.split` собранной команды обязан вернуть исходный путь одним
словом.

`handlers.py` во время починки перешагнул предел в 600 строк.
Архитектурный храповик это поймал, и таблица интерпретаторов вместе с
тремя функциями вокруг неё переехала в `arena/exec/interpreters.py` — ни
одна из них не касается aiohttp, авторизации или жизненного цикла
запроса, так что шов был именно там.

### Мутационное тестирование, которое пропускает уже доказанное

Пользователь спросил, можно ли прогнать мутационное тестирование один раз,
а потом перепроверять только изменившееся. Это правильная форма, и теперь
она построена: ``scripts/mutation_cache.py``.

Результаты ключуются хешем **исходника, охраняющих его тестов и версии
mutmut**. Важны все три. Если хешировать только исходник, ослабленный тест
будет вечно ехать на старом «пройдено» — а это ровно та регрессия, ради
которой мутационное тестирование и существует. Другая версия mutmut
порождает другой набор мутантов, поэтому старое число несопоставимо с
новым.

Замер на ``arena/files/sandbox.py``: **146с вхолодную, 0с из кэша**, тот
же вердикт (89 выживших из 216).

Два правила не дают этому превратиться в способ тихо перестать
тестировать. **Промах кэша никогда не значит «пройдено»** — неизвестный
вход означает «прогнать», а битый или обрезанный файл кэша деградирует в
«проверить всё», а не в «всё хорошо». **Попадание всё равно проходит
храповик**: сохранённое число сравнивается с базой точно так же, как
свежий прогон, поэтому закешировать хороший результат и потом сломать код
не получится. Есть тест, утверждающий, что ветка с кэшем доходит до
храповика, а не возвращается раньше.

Один баг в самом кэше, пойманный осмотром того, что он записал: у mutmut
2.5.1 нет флага ``--version``, поэтому первая реализация записала в поле
версии его баннер справки — строку, которая не меняется между релизами, что
молча отключало эту треть ключа. Теперь версия читается из метаданных
установленного пакета.

`.mutmut-cache` (рабочий sqlite-файл mutmut) игнорируется;
`scripts/mutation_cache.json` коммитится — вердикт, который репозиторий уже
заработал, это ровно то, что CI и должно быть позволено переиспользовать.

### Ранжирование по «опасность x непокрытость» и первый файл, на который оно указало

Прежде чем выбирать между «догонять покрытие» и «прогнать мутационное
тестирование по всему», я замерил, нашло бы ли второе хоть что-нибудь.

Баг #48 (RCE в зеркале) был откачен в отдельную копию, и mutmut натравлен
на уязвимый файл с теми тестами, что существовали на тот момент:
**180 мутантов, убито 0**. И не потому, что тесты слабы каким-то
интересным образом — `coverage` выдал «No data to report» для
`mirror.py`. Сьют его вообще не исполнял. А сама уязвимость жила в
*отсутствующем* коде:
...
В отсутствующей проверке нечего мутировать. Ни один из багов #40–#50 так
бы не нашёлся: все они — отсутствующие проверки, проглоченные ошибки или
гонки.

Значит порядок такой: сначала покрытие, потом мутации, — а выбор *какого
именно* покрытия взвешен по опасности. Файлы оценены по их возможностям
(subprocess, rmtree, urlopen, chmod, ввод на устройстве), умноженным на
число непокрытых операторов. `mirror.py` до починки занял бы седьмое
место в этом списке — и в этом суть: непокрытость и опасность
коррелируют, потому что в опасный код лезть страшнее всего.

Первым в списке оказался `arena/workbench/runtimes.py` с 26% покрытия —
он скачивает Go, Deno, Zig, Wasmtime и Lua из интернета, распаковывает их
и делает исполняемыми.

### Скачанные рантаймы ставились без проверки, если отсутствовал digest

Все четыре пути установки проверяли так:

    expected = str(asset.get("digest") or "").split(":", 1)[-1]
    if expected and got.lower() != expected.lower():
        raise RuntimeError(...)

*Отсутствующий* digest полностью пропускал проверку, а три из пяти
резолверов ассетов могут его не дать — `asset.get("digest") or ""` — это
ровно то, что происходит, когда GitHub Release API не отдаёт поле.

Воспроизведено от начала до конца с заглушенной загрузкой: при
`digest: ""` архив с содержимым `#!/bin/sh; echo TROJAN` установился как
исполняемый файл `deno` и был записан в реестр рантаймов как управляемый.
Для байтов, скачанных по сети и затем исполняемых, «не смогли проверить»
обязано означать «не поставим», поэтому `_verify_digest` теперь
отказывает сразу.

Защиты распаковки проверены заодно и оказались исправны — zip slip через
`../`, абсолютный путь члена архива и tar-симлинк наружу отбиваются все.
Теперь это зафиксировано, чтобы так и осталось.

Два замечания из раунда саботажа, оба про мои же тесты. Храповик,
запрещающий старую fail-open форму, срабатывал на собственной
документации: докстринг `_verify_digest` цитирует ошибочный код, чтобы
объяснить, зачем нужен хелпер; теперь он разбирает AST, а не ищет по
тексту. А тест на неверный digest использовал шестьдесят четыре нуля,
поэтому саботаж со сравнением только `got[:8]` его проходил — digest
должен отличаться *только в хвосте*, чтобы отличить полное сравнение от
усечённого.

`runtimes.py` перешагнул предел в 600 строк во время починки.
Архитектурный храповик это поймал, и половина про скачивание-распаковку
переехала в `arena/workbench/runtime_fetch.py`, где ей и место: она
ничего не знает ни про один конкретный языковой рантайм.

### Почему live-зеркало никогда не работало: гонка, которую нельзя было проиграть медленно

Иван сообщил, что live в mobile не работает вообще — работает только
опрос скриншотов на ~0.4 fps. Он прав, и причина была не в видеоконвейере,
а в двух строках управления жизненным циклом.

`get_or_start` планирует `_pump_pipeline`, а подписка происходит после.
Условие цикла — `while not stop_event.is_set() and has_subscribers()`.
Итог зависел от того, кто выиграл гонку:

  * подписался до первой проверки цикла — стрим идёт;
  * опоздал на миллисекунды — подписчиков ноль, цикл выходит сразу, а
    блок `finally` удаляет сессию из `_SESSIONS`. Браузер держит мёртвую
    сессию, и повторная попытка тоже ничего не находит в реестре.

На WebSocket-пути между этими шагами стоит `await ws.prepare(request)`,
поэтому подписчик проигрывал гонку практически всегда. Замер с заглушкой
screenrecord: подписка через 0.30s оставляла `reader_task.done()` равным
True и пустой реестр; синхронная подписка работала. Теперь конвейер ждёт
явного события `first_subscriber` — с таймаутом, чтобы клиент, умерший на
рукопожатии, не оставил screenrecord работать на телефоне.

Написание ворот вскрыло вторую проблему. Цикл чтения был
`buf = await sr.stdout.read(65536)`, а проверки остановки стояли *после*,
поэтому обе срабатывали только при поступлении байтов. Сессия, от которой
ушёл последний зритель, держала screenrecord живым, пока телефон случайно
не выдаст следующий кадр, — а на статичном экране AVC-энкодер может
молчать долго. Замер: stop_event выставлен, подписчиков ноль, через три
секунды всё ещё работает. У `stop_all()` при выключении моста была та же
беда. Теперь чтение ограничено по времени и перепроверяет условия выхода
в простое.

Сам муксер оказался исправен. `mp4_muxer.py` — это 576 строк вообще без
тестов, поэтому он считался виновным до доказательства обратного: на
настоящих параметрах Annex-B он корректно разбирает 320x240, выдаёт
валидный ftyp+moov, строит фрагменты moof+mdat, помечает IDR ключевым
кадром и рассчитывает длительности семплов по стенным часам. Теперь это
покрыто 15 тестами, включая NAL'ы, разрезанные по произвольным границам
чтения, обе длины стартового кода и мусор перед первым из них.

Одно замечание по раунду саботажа — он поймал слабое утверждение, а не
слабый код. Удаление `first_subscriber.set()` тесты не уронило: конвейер,
припаркованный в `wait_for`, тоже «не завершён», так что проверять
`reader_task` было неправильно. Теперь сессии считают `segments_started`,
который растёт только при реальном запуске screenrecord, — и это число
выведено в `/v1/mobile/mirror/stats`, где отвечает на вопрос «эта сессия
действительно стримит?» тому, кто разбирается с чёрным видео.

### У зеркала экрана была незакрытая RCE, а мой собственный храповик её не видел

`arena/mobile/mirror.py` был на 35% покрытия. Он стримит экран телефона,
поэтому шёл следующим — и `?size=` попадал из строки запроса прямо в

    adb -s <serial> exec-out screenrecord --size <size> ...

вообще без какой-либо валидации. `exec-out` — это шелл устройства, то есть
удалённое выполнение кода; воспроизведено от начала до конца, затем
подтверждено против работающего моста:

    GET /v1/mobile/{serial}/mirror?size=720x1600; touch /tmp/PWNED %23
    -> файл создан на устройстве.

Неприятная часть — почему это выжило. В v4.162.0 ровно этот класс был
починен централизованно, внутри `arena.mobile.adb.run`, именно чтобы ни один
вызывающий не мог забыть, — а mirror `run()` не использует. И не может:
`run()` блокирующий и захватывает всё целиком, а зеркалу нужен живой поток,
поэтому оно запускает adb само. Это законное исключение.

Подвёл храповик, написанный как раз для отлова таких исключений. Он искал
`subprocess.run` и `subprocess.Popen`; mirror использует
`asyncio.create_subprocess_exec`. Централизованную починку обходили два
модуля, храповик знал об одном и отрапортовал «чисто». Детектор ровно
настолько хорош, насколько полон его список способов запуска процесса —
теперь их одиннадцать (run/Popen/call/check_output/check_call, обе
асинхронные формы, os.system, os.popen, os.spawn, os.exec), mirror вынесен
в исключения **поимённо**, чтобы третий такой модуль не унаследовал
поблажку молча, и отдельный тест проверяет, что асинхронные формы из списка
не пропали.

`validate_stream_params` теперь отвергает всё, что не `WxH`, и ограничивает
битрейт, WebSocket-обработчик отказывает до апгрейда (внятный 400 лучше
сокета, который открылся и умер), а `_screenrecord_cmd` перепроверяет
сам, вместо того чтобы доверять вызывающим, — забудет как раз следующий.
Шестнадцать инъекционных payload'ов зафиксированы рядом с шестью реальными
разрешениями телефонов, потому что валидатор, отвергающий `1080x2400`, —
это валидатор, который кто-нибудь удалит.

### Аудит npm-поверхности в день, когда Shai-Hulud забрал 1280 пакетов

4 августа 2026 червь захватил GitHub-аккаунт сопровождающего `keyv` и
выпустил вредоносные релизы более чем в 1280 npm-пакетах с суммарными
двумя миллиардами установок в месяц. Два свойства этой атаки стоит
назвать прямо, потому что они обходят защиты, к которым тянутся первым
делом:

  * Отравленные релизы несли **действительную provenance, подписанную
    GitHub Actions**. Их действительно собрал CI самого сопровождающего —
    скомпрометирован был аккаунт, а не подпись. Проверка подписи прошла бы.
  * Полезная нагрузка запускалась из хука **`preinstall`**, до того как
    дерево увидел бы человек или сканер. Одного `npm install` хватало,
    чтобы утекли учётные данные GitHub, npm, AWS, Kubernetes, Vault,
    Slack, Stripe и SSH.

Проект не задет, и причина структурная, а не везение. Всё npm-дерево —
это `oxlint` и его платформенные бинарники: 21 запись в lock-файле, ни
одной из поражённых семейств, ни одной с install-скриптом. `keyv` обычно
приходит транзитивно через eslint, которого у нас нет. Мы ничего не
публикуем в npm, поэтому и publish-токена, который ворует червь, не
существует. Подтверждено `npm audit` (ноль уязвимостей) и поиском
артефактов самого червя (`setup.mjs`, `Math_Symbol.js`) — отсутствуют.

Изменилось то, что ничто из этого не было закреплено. `npm ci` теперь
выполняется с `--ignore-scripts`, что полностью снимает класс атак через
install-хуки. Сегодня флаг не меняет ничего — ровно потому, что блокировать
нечего; это и есть аргумент добавить его сейчас, а не после того, как
обновление lock-файла тихо принесёт хук. Проверено исполнением: установка
с флагом, затем линт файла с намеренным `eval()` — oxlint по-прежнему
сообщает о нарушении.

`tests/test_npm_supply_chain_surface.py` удерживает свойства: никаких
install-скриптов, никакого голого `npm install`, точные пины вместо
диапазонов, integrity-хеш у каждой записи, `private: true`, синхронность
lock-файла с манифестом и потолок на число аккаунтов сопровождающих,
которым мы неявно доверяем. Каждое саботировано, каждое поймало свою
регрессию.

Не по теме, но нашлось при уборке: `requests.jsonl.2`, 10-мегабайтный
рантайм-лог, отслеживался в git начиная с c541f41e. В `.gitignore` были
перечислены `requests.jsonl` и `requests.jsonl.1`, но не `.2`, так что
вторая ротация logrotate проскочила. `make_release_zip.py` уже исключал
ротированные логи по префиксу — его научили этому после того, как один
попал в zip v4.83.0, — а git так и не научили. Теперь глоб по суффиксу, а
файл убран из индекса.

### Записи микрофона оставались на телефоне, и об этом врали

`arena/mobile/audio_capture.py` был файлом с самым низким покрытием в
мобильном дереве — 21%, — и именно он пишет через микрофон устройства. Так
что следующим смотрел его.

Очистка в конце `voice_record` выглядела так:
...
`run()` возвращает CompletedProcess и не бросает исключение на ненулевом
коде. Поэтому `rm`, ответивший «Permission denied», всё равно ставил
`cleaned_up: True`, и вызывающему сообщали, что запись микрофона удалена с
устройства, пока она лежала в /sdcard/DCIM. Это третий случай ровно такой
формы за релиз — `except: pass` вокруг adb-вызова и следом захардкоженный
успех, — и для записи голоса это заявление о приватности, а не поле
состояния.

Рядом обнаружилась проблема крупнее: ни один путь ошибки не убирал за
собой вообще. Капча по таймауту, ошибка от самого приложения-рекордера и
скачанные нулевые байты — все возвращались досрочно и оставляли аудио на
устройстве навсегда. Капча, которая ПРОВАЛИЛАСЬ, — как раз та, за которой
никто не возвращается, поэтому именно там осиротевшая запись важнее всего.
Все три теперь удаляют файл перед возвратом.

Путь неудавшегося `pull` намеренно делает обратное и сохраняет файл: байты
существуют только на телефоне, и сломалась именно передача, так что
удаление уничтожило бы ту самую запись, которую забирают. Чего он больше
не делает — так это не молчит об этом: возвращает `cleaned_up: false` с
путём на устройстве и причиной, в том же формате конверта, что и все
остальные исходы.

Именно это последнее свойство и фиксируют ворота: что бы путь ни решил,
вызывающий обязан всегда иметь возможность узнать, где лежит аудио с
микрофона. Удалять или сохранять — вопрос суждения; лишать вызывающего
возможности это выяснить — нет. В раунд саботажа я включил обратный
случай — добавление очистки на путь неудавшегося pull, — чтобы тесты
защищали решение в обе стороны, а не просто поощряли побольше удалений.

## v4.162.0 — 2026-08-05

### Stop и purge говорили «готово», когда телефон их не слышал

`handlers_recording.py` был на 31% покрытия, а под ним — две функции
одинаковой формы:
...
При недоступном adb `record_stop` сообщал `ok: True, status: "stopped"`,
пока screenrecord продолжал писать на телефоне, а `record_purge` сообщал
`ok: True`, пока все MP4 оставались на устройстве. Обе лжи направлены в
худшую сторону. Агент, считающий, что запись экрана выключена, будет вести
себя соответственно, а для purge «ok» — это заявление о приватности, а не
код состояния.

Ни один вызов теперь не бросает исключение — частично сработавшие stop или
purge не должны выглядеть как падение, — но результат сообщается честно:
`ok` отражает происшедшее, неудавшийся stop помечает запись в реестре как
`stop_failed`, а не `stopped`, а неудавшийся purge всё так же перечисляет
очищенное локально и прямо говорит, что файлы могли остаться на телефоне.

Стоит отметить всплывшее попутно: `purge_recordings` не работал никогда.
Он передаёт команду как `run(["shell", "sh", "-c", inner])`, и до
квотирования argv, добавленного ранее в этом релизе, adbd склеивал эти
аргументы в `sh -c rm -f /path/*.mp4 ...` — то есть шелл устройства
запускал `sh -c rm`, где `-f` становился именем аргумента, а остальное
отбрасывалось. `rm: missing operand`, ненулевой код, проглочено
`except: pass`, отрапортовано успехом. Квотирование починило удаление; эта
правка — то, что вообще сделало бы провал заметным.

Храповик, который это стережёт, считает нарушением `except Exception: pass`
вокруг изменяющего устройство вызова только тогда, когда причина не
записана. `_ensure_record_dir` глотает ошибку намеренно и объясняет это в
докстринге — screenrecord через мгновение выдаст ошибку понятнее, — так что
отмечать его было бы шумом, а детектор, придирающийся к задокументированным
решениям, — это детектор, который учатся игнорировать. Ещё отмечу: первая
версия этого храповика использовала очевидный регексп `try:...except:pass`,
который дал катастрофический бэктрекинг и подвесил сьют, пока его не убил
pytest-timeout. Теперь это линейный построчный проход.

### Загрузка отвечала «нет» и всё равно писала файл

`arena/mobile/handlers_devops.py` был на 29% покрытия — значит, он следующий.
У `POST /v1/mobile/apk/upload?filename=...` оказалась та же форма, что у бага
№37 (запись == выполнение): он проверял назначение *после* записи в него.

    dest = STAGING_ROOT / filename
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_bytes(data)
    return prepare(filename)      # проверка вложенности живёт здесь

`prepare` действительно отвергает пути вне staging, поэтому вызывающий
получал правильно выглядящий отказ — на шаг позже, чем нужно. Против
работающего моста:

    POST /v1/mobile/apk/upload?filename=/tmp/HTTP_PWNED.apk
    -> 200 {"ok": false, "error": "apk_path must live under the staging
            directory"}, и /tmp/HTTP_PWNED.apk существует, 504 байта.

Абсолютное имя доходило до этой строки, потому что единственной защитой было
`any(p in ("..", "") for p in Path(filename).parts)`, а `Path("/tmp/x").parts`
— это `("/", "tmp", "x")`: ни `..`, ни пустого сегмента. `STAGING_ROOT /
"/tmp/x"` затем даёт `/tmp/x`, потому что pathlib отбрасывает левый операнд,
когда правый абсолютный. Существующие файлы затирались, недостающие каталоги
создавались по пути. Текст ошибки обещал обратное: «Arbitrary host paths are
rejected on purpose so a hijacked token can't install anything on disk».

Пока писал ворота, выпали ещё два бага — оба про нетотальность.
`Path.expanduser()` бросает `RuntimeError` на ведущем `~неизвестныйюзер`, так
что `prepare("~nosuchuser/x.apk")` возвращал HTTP 500 вместо конверта; к тому
же это просто неверно по сути: `~` — легальный символ имени файла, и
`~foo.apk` должен лежать в staging, а не где-то в `/home`. И `Path.exists()`
тоже не тотальна — имя длиннее NAME_MAX бросает `OSError(ENAMETOOLONG)`,
встроенный NUL бросает `ValueError`. Эндпоинт, который fail-closed, обязан
быть таким и для враждебного ввода, а не только для того, который кто-то
предусмотрел.

Проверка вложенности теперь происходит до того, как что-либо касается
файловой системы, и перепроверяется на *разрешённом* пути, чтобы симлинк,
подложенный внутрь staging, не стал выходом наружу. Логика переехала в
`arena/mobile/apk_paths.py`, потому что `apk_install.py` перешагнул предел в
600 строк, пока это чинилось, — архитектурный храповик это поймал, и правильным
ответом было разрезать по естественному шву, а не поднять предел.

Из раунда саботажа стоит записать две вещи — обе мои ошибки, а не кода.
Удаление проверки `is_absolute()` не сломало ничего: проверка разрешённого
пути и одна закрывает побег, так что ранняя защита даёт лишь понятное
сообщение — и тесты теперь фиксируют именно это сообщение. А symlink-тест
проходил даже со снятой проверкой вложенности, потому что ссылка указывала на
*несуществующий* каталог: `mkdir` падал по посторонней причине и прятал побег.
С настоящим каталогом за ссылкой запись проходит насквозь. Тест-ворота,
который проходит по неверной причине, хуже, чем его отсутствие.

Постскриптум: первая версия этой починки покраснела только на Windows.
`~literal-tilde.apk` на POSIX — это имя файла (`expanduser()` бросает
исключение на неизвестного пользователя), а Windows раскрывает его в путь под
`C:\Users`, так что одна и та же загрузка была файлом в staging на одном
раннере и отвергнутым побегом на другом. Ошибка была в том, чтобы решать
«ссылка ли это на дом?» по факту исключения из `expanduser()`; теперь правило
чисто лексическое (`~` и `~/...` — ссылки на дом, всё остальное — имя файла).
Ворота симулируют поведение `expanduser()` обеих платформ, поэтому расхождение
падает локально за двенадцать секунд, а не в CI за двенадцать минут.

### `adb shell` был примитивом удалённого выполнения кода

`arena/mobile` — последний слабо покрытый блок, действующий на железо, поэтому
метод тот же: ранжировать по тому, что дотягивается до устройства, и гонять
против настоящего adbd, а не рассуждать. Нашлась живая RCE, воспроизведённая
от начала до конца.

Ошибка тонкая, и сделать её легко. На хосте
`subprocess.run(["ls", "a; rm -rf /"])` безопасен: argv уходит в `execve`, и
точку с запятой не видит ни один шелл. `adb shell` в Python выглядит так же,
но это не так: **adbd склеивает аргументы пробелами и отдаёт полученную строку
в `/system/bin/sh` на телефоне.** Список аргументов не даёт на стороне
устройства ровно ничего.

У `restricted_shell` был блоклист метасимволов, и выглядел он основательно:
`;`, `&&`, `||`, `|`, обратные кавычки, `$(`, перенаправления, переводы строк.
Одиночного `&` в нём не было. Против заглушки adbd:

    restricted_shell(serial, "ls /data & touch /tmp/PWNED")
    -> {"ok": True, ...}, и /tmp/PWNED создан.

Проверка головной команды по allowlist прошла (`ls` разрешён), блоклист не
увидел знакомого, телефон выполнил обе команды. Тем же путём проходили
glob-символы и `~` — их раскрывает шелл устройства.

У `input.type_text` та же дыра, но с другого входа. Замена пробелов на `%s` —
реальное требование CLI `input text`, но на неё же полагались как на границу
безопасности, а `$IFS` — это пробел, которого экранирование не видит:
`hi&touch$IFS/tmp/T3` создал файл.

Починка — одна функция в одном месте. `arena.mobile.adb.quote_shell_args`
квотирует argv каждого вызова к шеллу УСТРОЙСТВА и живёт внутри `run()`,
поэтому забыть про неё не может ни один из 39 вызовов. Включая `exec-out`,
который на первом проходе я пропустил: это тот же `shell`, только с сырым
двоичным потоком вместо pty — на телефоне это один и тот же путь кода, — а
screenshot, camera, recording и ui тянут данные через `exec-out cat <path>`,
где путь приходит от вызывающего. `run(["exec-out", "cat", "/sdcard/x; touch
/tmp/EO"])` оставался рабочим эксплойтом, пока его не закрыли. Вызовы не-
`shell` (`push`, `pull`, `install`, `forward`) до шелла устройства не доходят и
проходят нетронутыми. Блоклист тоже расширен — как fail-closed второй слой, — и
расширение проверено на пятнадцати настоящих диагностических командах
(`dumpsys battery`, `settings get ...`, `logcat -d -t 50` и прочих), потому что
валидатор, отвергающий законный ввод, выключают, а это хуже, чем его
отсутствие.

`tests/test_mobile_adb_shell_injection.py` ставит заглушку adbd, повторяющую
поведение «склеить и `sh -c`», так что ворота проверяют настоящую семантику
устройства, а не мок наших собственных допущений. Отдельный храповик роняет
сборку, если какой-нибудь модуль в `arena/mobile` запустит adb мимо `adb.run`.
Все три защиты саботированы по отдельности, каждая поймала свою регрессию.

### Inventory: 46 сборщиков и охота на канарейку по всем

`arena/inventory` — следующий слабо покрытый блок, касающийся машины:
`registry.py` на 17.7%, несколько проб ниже 5%. Он вызывает подпроцессы,
читает окружение, и его вывод уходит модели через `sys.inventory` — то есть
утёкшее значение не остаётся локальным: оно попадает в переписку и, возможно,
в логи провайдера.

Схема сокрытия оказалась правильной. `probe_agent_ctx` сообщает *имена*
секретов и никогда значения, с allowlist'ом, чтобы пути `*_TOKEN_FILE` не
засоряли отчёт. Проверено подсадкой канарейки в двенадцать переменных
окружения с «секретными» именами — включая собственный токен моста — и
прогоном всех 46 сборщиков: 46 отработали, ни один не упал, ни один не выдал
значение.

Не хватало того, что удержит это состояние.
`tests/test_inventory_never_leaks_secret_values.py` прогоняет канарейку через
каждый сборщик *и* каждый форматтер, потому что сборщик может скрывать
правильно, а соседний `format_lines` — вставить сырое значение в строку
отображения. Плюс проверяется, что имена секретов по-прежнему сообщаются
(проба, скрывающая всё, не безопасна, а сломана), что ни один сборщик не
падает (упавшая проба молча пропустила бы собственную проверку на утечку) и
что канарейка вообще наблюдаема.

Проверено саботажем: заставить пробу выдавать значения рядом с именами роняет
два теста.

### Одна предпушевая команда, потому что 12 минут — неправильная петля обратной связи

Замечание Ивана про укрупнение коммитов было верным, и замер сделал корень
проблемы очевидным. За последние 25 прогонов CI: 19 зелёных, 3 падения,
3 отменённых — 12% отказов при медиане **12.3 минуты**. При этом половина
последних коммитов в ветке — однострочные правки, чинящие ровно эти падения.

Все три падения ловились локально и не были пойманы, потому что локальные
проверки каждый раз собирались по памяти:

    17cec54e  actionlint прошёл локально и упал в CI — CI запускает его
              *вместе с* shellcheck (SC2012), у локального его не было в PATH
    6ba7b06a  тест читал /proc, которого нет на macOS
    7041c57d  ворота бейджа утверждали невыполнимое на релизном коммите

`scripts/preflight.py` — одна команда, запускающая все ворота, которые реально
роняли этот проект. Занимает **12 секунд** (148 с `--full`) и падает закрыто
при отсутствии инструмента: «проверка не запускалась» и «проверка прошла» не
должны выглядеть одинаково.

Проверено воспроизведением настоящих падений, а не доверием к замыслу:
возврат точной строки SC2012 из 17cec54e роняет preflight за 12 секунд с тем
же сообщением, которое CI выдавал за 12.3 минуты; бейдж впереди дерева тоже
ловится.

**Для платформенного класса испробованы две эвристики, и обе удалены.**
Подстрочный поиск пометил слово «systemd-run» внутри docstring, который
объясняет этот самый баг. AST-скан по строковым литералам затем пометил три
файла, зелёных на macOS в CI, потому что строки были
`monkeypatch.setattr(R, "_have", lambda c: c == "systemd-run")` и
`restricted_shell("dummy", "cat /proc/version ...")` — моки и фикстуры, а не
обращения к системе. Три ложных срабатывания из трёх находок — это не
проверка, а то, что учатся пропускать. Отличить «строка называет Linux-путь»
от «строка уходит в ОС» требует анализа потока данных, а не сопоставления с
образцом. Рассуждение записано в функции, которая теперь всегда возвращает
успех, чтобы следующий не пробовал в третий раз.

## v4.161.0 — 2026-08-04

Шесть живых дефектов, и все найдены одинаково: взять код, который действует
на машину и хуже всего покрыт, и не читать его, а запускать.

Два были доступны любому, кто может обратиться к мосту — запись в
`~/.bashrc` через upload и нажатие клавиши на десктопе, несущее shell-команду.
Один был невидим для всех 406 тестовых файлов, пока на него не указало
мутационное тестирование: один перевёрнутый оператор, инвертировавший
самозащиту моста.

Релизные артефакты впервые подписаны и проверяемы.

### Релизные артефакты теперь проверяемы

Замер на v4.160.0: страница релиза предлагала два zip и ничего, чем их
проверить — ни хеша в теле, ни подписи. Любой с правами записи или способный
перехватить загрузку мог подменить архив, и пользователь бы не заметил. Для
инструмента, который выполняет команды на машине оператора, «нельзя проверить
то, что скачал» — настоящий пробел.

`.github/workflows/sign-release.yml` подписывает каждый опубликованный ассет
через Sigstore keyless и публикует `SHA256SUMS-vX.Y.Z.txt` (тоже подписанный)
для тех, кто не хочет ставить cosign.

Keyless выбран намеренно: приватного ключа не существует — его нельзя ни
украсть, ни забыть ротировать, ни случайно закоммитить. GitHub выдаёт
короткоживущий OIDC-токен, Sigstore привязывает подпись к *этому workflow в
этом репозитории*, а сертификат попадает в публичный лог прозрачности.
Проверка доказывает, что артефакт вышел из этого пайплайна — утверждение
сильнее, чем «кто-то с ключом подписал».

Workflow проверяет собственный результат до публикации: шаг подписи, который
никто не видел работающим, может и не работать. И падает закрыто, если список
ассетов пуст — релиз с провалившейся загрузкой не может отрапортовать зелёной
подписью.

`tests/test_release_signing_workflow.py` стережёт свойства, на которых
держится это утверждение, потому что пайплайн подписи легко ослабить
случайно: право OIDC, шаг проверки, pinned SHA у действий, регексп личности,
привязанный к этому workflow и репозиторию, зафиксированный OIDC-issuer,
запасной вариант с хешами, strict mode в каждом блоке и наличие команды
проверки в RELEASE.md. Проверено саботажем четырьмя способами — удаление
`id-token`, удаление проверки, откат действия на подвижный тег и ослабление
привязки личности роняют по тесту каждый.

Это единственное, что стоило взять из предложений про «frontier-проверки».
Остальное — дуэли ИИ-редтимеров, телеметрия пространства эмбеддингов, cyber
range, SPIFFE/SPIRE — требует внешней модели, сети и порогов в CI, то есть
недетерминированного вердикта, который нельзя проверить саботажем. Именно
детерминизм делает ворота осмысленными.

### Живой баг: нажатие клавиши на десктопе могло быть shell-командой

`arena/desktop` — самый крупный из оставшихся блоков непокрытого кода,
действующего на машину (828 операторов), и это код, который двигает мышь и
жмёт клавиши на реальном десктопе оператора. Большая часть требует
X11/Wayland и в CI не запускается — но самое важное проверяется где угодно:
чистые функции, собирающие строку команды, которая уходит прямо в
`create_subprocess_shell`.

Замерено по всем шести сочетаниям билдер/бэкенд. Утекал ровно один:

    ydotool  key(key=)   ydotool key x; touch /tmp/PWNED     <- без экранирования
    ydotool  type        ydotool type --key-delay 50 '...'   <- в кавычках
    xdotool  key(key=)   DISPLAY=:0 xdotool key '...'        <- в кавычках
    xdotool  type        DISPLAY=:0 xdotool type ... '...'   <- в кавычках
    wtype    type        wtype '...'                         <- в кавычках

Запасная ветка ydotool была `f'ydotool key {key}'` без кавычек, поэтому
`POST /v1/desktop/key` с `key="x; touch /tmp/PWNED"` выполнял две команды.
Все соседние пути уже использовали `shlex.quote`; этот — нет.

Починено отказом, а не экранированием. `ydotool key` принимает только
известное имя клавиши или сырые пары `CODE:STATE`, поэтому закавычивание
произвольной строки закрыло бы дыру, но отправило бы команду, которая молча
ничего не делает — честная ошибка лучше рабочего на вид no-op. Легитимный
ввод не затронут: `Return`, `ctrl+c` и сырое `28:1 28:0` собираются как
прежде.

`tests/test_desktop_input_no_shell_injection.py` покрывает семь форм инъекции
по трём бэкендам и обеим формам key, плюс билдеры координат, плюс саму строку
регрессии. Проверено саботажем: возврат бага роняет 8 тестов.

У детектора внутри теста сначала был ложноотрицательный результат: сравнение
с `payload.replace("'", ...)` совпадает с *сырым* payload, если в нём нет
кавычки, поэтому точная баговая строка считалась безопасной. Теперь он
спрашивает `shlex.quote`. Ворота, не видящие баг, ради которого написаны,
хуже отсутствия ворот — поэтому строка регрессии закреплена отдельным тестом.

### Мутационное тестирование, потому что договорённость — это не принуждение

Возражение Ивана против отказа от MutMut было верным, и суть стоит записать:
*не каждый агент будет соблюдать обязательный саботаж без принуждения; а
красный CI игнорировать не станут.*

Замерено, и цифры решили спор. Тестовых файлов 406. Саботаж упомянут в шести.
В `AGENTS.md` слова «sabotage» не было вообще — правило, которое поймало
реальные баги в этом проекте, жило только в переписке.

Прогон mutmut по `arena/files/sandbox.py` дал 216 мутантов, **95 выживших**.
Большинство безобидны: формулировки сообщений об ошибке или вторая половина
эшелонированной защиты — применение мутанта, ломающего проверку `..`,
подтвердило, что traversal всё равно отклоняется следующей проверкой через
resolve. Один безобидным не был:

    -    if target_path.resolve() == bridge_py.resolve():
    +    if target_path.resolve() != bridge_py.resolve():

Один перевёрнутый оператор инвертирует самозащиту моста:

    правка самого моста     ->  200  (разрешено)
    правка обычного файла   ->  403  «cannot edit the bridge itself»

Агент мог бы переписать `unified_bridge.py` — все политики и все вызовы
аудита внутри — при том что обычная работа сломалась бы. **Весь набор тестов
проходил**, потому что никто не утверждал, что мост защищает себя, ни в одну
сторону. Проверять только отказ было бы недостаточно: инвертированная версия
тоже отказывает — просто всем остальным.

`tests/test_bridge_self_protection.py` утверждает обе половины по всем трём
валидаторам записи, плюс что одноимённый файл в другом каталоге — не мост, а
симлинк на мост — мост. Пять тестов ловят этого мутанта.

`scripts/mutation_gate.py` делает это механическим. Область узкая, а число
замерено, а не угадано: один файл ~2 минуты, всё дерево `arena/` — ~13 часов
ради цифры, которую никто не читает. Baseline работает как ratchet — сегодня
89 выживших вместо 95, потому что новые тесты уже убили шесть — и fail-closed,
если не убит ни один мутант: это ловит ошибку с указанием тестов, которые не
импортируют цель (была допущена однажды при разработке).

В `AGENTS.md` правило саботажа теперь записано явно, включая грабли, стоившие
здесь часа: восстановление файла через `cp` сохраняет mtime, поэтому Python
продолжает отдавать *мутантский* `.pyc`, пока исходник читается правильным.
После восстановления удалять `__pycache__`.

### Живой пробел: описание инструмента — это инструкция, и его не охранял никто

Предложено восемь продвинутых проверок (фаззинг, DAST, нагрузочное, лицензии
и четыре MCP-специфичных). Каждая замерена против этого кода, а не принята на
веру. Воспроизвелась одна.

**Confused Deputy — нет.** Оба пути принятия решения fail-closed:
`arena/control.py` при halt блокирует всё, что не помечено явно `safe`, а
расширение требует подтверждения всегда, когда `risk != "safe"` — поэтому 49
инструментов с `unknown` требуют подтверждения, а не проскакивают. Из 72
инструментов, действующих на машине, 20 помечены `safe` и все read-only;
`ship.smoke` выполняет фиксированный литерал кода, который вызывающий не
контролирует. «Legacy bare-name guard», который предлагалось тестировать,
удалён в v4.75.0.

**Отравление каталога внешним сервером здесь невозможно** — внешние
инструменты не попадают в наш `tools/list`, до них дотягивается только
`mcp.ext_call`.

**Но сами описания не были защищены ничем.** Это 24 570 символов, уезжающих
модели в `tools/list`, и модель на них **действует**. Снапшот хранит только
имена; schema-тест проверяет лишь, что описание — непустая строка. Замер с
отравленным `mobile.devices`:
...
Сделано намеренно **не так**, как предлагал ТЗ. Семантический сдвиг на
эмбеддингах требует модели, сетевого вызова и порога; вердикт был бы
недетерминированным, саботажем его не проверить, а качающиеся ворота
начинают игнорировать. Свойство сформулировано жёстче и проверяется точно:
описание должно *описывать*, а не *инструктировать агента*.

`tests/test_tool_descriptions_are_not_instructions.py` — 704 проверки по всем
234 инструментам: двенадцать паттернов директив, шесть исполняемых форм,
границы длины, невидимые символы (zero-width, bidi) и fingerprint всего
каталога, чтобы проза не менялась молча.

Ноль ложных срабатываний на реальном каталоге. Первая версия дала шесть — все
на markdown-бэктиках вокруг имён параметров, поэтому паттерн сужен до
бэктиков с настоящими shell-командами: детектор, кричащий на легитимный
текст, — это шум, а шум начинают игнорировать. Проверено саботажем на четырёх
payload'ах: исходный ловят 3 теста, «ignore all previous instructions» — 2,
невидимая zero-width инъекция — 3, «do not tell the user» — 2.

Остальные шесть предложений разобраны с цифрами в
`docs/github_apps_actions_survey.md`. `hypothesis` уже даёт property-based
фаззинг; MutMut дублирует обязательный саботаж, через который и так проходят
все новые ворота; ZAP ищет SQL/XSS/CSRF в сервисе без SQL, форм и cookie;
цифры нагрузки опубликованы в v4.159.0.

### Живой баг: загрузка файла была способом выполнить код

Проход «покрытие как поиск» перешёл от ``arena/exec`` к ``arena/files`` —
следующему по опасности слабо покрытому коду. Прогнано против живого моста,
а не прочитано.

**Защита от traversal действительно сильная.** Абсолютные пути вне root,
подъёмы через ``../``, URL- и двойное кодирование, и даже симлинки, лежащие
*внутри* root, но указывающие наружу — всё отклонено. ``token.txt``,
``.ssh/``, ``.aws/``, ``.gitconfig`` и история шелла защищены и на чтение, и
на запись.

**Но списки чувствительности отвечают только на вопрос «что не должно
утечь».** Вопрос «что не должно быть *подменено*» никто не задавал:

    POST /v1/upload?path=/home/user/.bashrc   ->   200 OK

Это не раскрытие файла — следующий же шелл, открытый оператором, выполнит
содержимое вызывающего, без какого-либо повышения привилегий. Это удалось
пронаблюдать вживую: после записи маркера следующая команда самой песочницы
напечатала ``/home/user/.bashrc: line 1: PWNED: command not found`` —
отравленный файл действительно был исполнен.

``.ssh/authorized_keys`` уже отклонялся. ``.bashrc``, ``.bash_profile``,
``.profile``, ``.zshrc``, ``~/.config/autostart/*.desktop`` и всё на PATH под
``~/.local/bin`` — нет.

Исправление сделано отдельным списком только для записи, а не дополнением к
существующим, потому что у двух вопросов разные ответы. Чтение конфигурации
шелла — легитимное поведение агента и остаётся разрешённым (проверено, а не
предположено). Список смотрят только ``uploading``, ``editing`` и
``creating``; ``downloading`` и ``viewing`` — нет. Закрытие одного upload
оставило бы тот же путь исполнения открытым через ``fs.edit`` и
``fs.create``.

Закреплено ``tests/test_write_is_not_code_execution.py`` (36 случаев):
тринадцать путей исполнения-при-записи по всем трём глаголам записи, что
чтение по-прежнему разрешено, что обычные файлы остаются записываемыми, что
прежние защиты секретов уцелели, и структурная проверка, что правило
подключено к глаголам записи и *не* подключено к чтению. Проверено саботажем
в обе стороны: снятие проверки роняет 14 тестов, избыточное применение к
чтению — 4.

### Покрытие как поиск, а не как число: два дефекта жизненного цикла в /v1/exec

Покрытие было 55.45%. Полезный вопрос не «какие строки тёмные», а «какие
*опасные* строки тёмные», поэтому непокрытые операторы отранжированы по
тому, может ли файл действовать на машине. ``arena/exec/handlers.py``
оказался худшим из всех: **11.3%**, все четыре обработчика выполнения
команд практически не проверены.

Прогнано против живого моста, а не прочитано. Все политики держатся —
аутентификация (401), пустой cmd (400), блок-лист (403), cautious-allowlist
(403), ограничение cwd (403), битый JSON (400), а потолок вывода реально
срезал запрошенные 99 МБ до 2 МБ. Жизненный цикл не держался.

**Команда по таймауту утекала процессом.** ``proc.kill()`` сигналит только
оболочке. При обычной форме ``bash -c "<что-то>"`` настоящая работа — это
внук, и он выживал: ``bash -c "sleep 300"`` с ``timeout=2`` оставлял
``sleep 300`` с PPID 1 на минуты после ответа на запрос. Для пользователя
это процесс, который он не запускал и не видит.

**И таймаут не был таймаутом.** Выживший внук наследует stdout-pipe, поэтому
``communicate()`` не мог завершиться, и 5-секундный drain всегда шёл целиком.
Замеренный перебег — ровно пять секунд:

    timeout=1 -> 6.0s     timeout=3 -> 8.0s
    timeout=2 -> 8.0s     timeout=5 -> 10.0s

Запросивший бюджет в секунду получал шесть. После фикса все попадают в
дедлайн с точностью до 3 мс.

Причина у обоих одна: ребёнок не запускался в собственной группе процессов,
поэтому убивать было нечего. Починено сразу для буферизованного и
потокового путей — это разный код с одним багом — с эквивалентом для
Windows (``CREATE_NEW_PROCESS_GROUP``), а не только для POSIX.

Попутно подтверждено, что серверный потолок таймаута работает: запрос на
999с при ``max_timeout=600`` был срезан на 605с.

Закреплено ``tests/test_exec_timeout_kills_the_tree.py``: проверяются время
и выживание процессов — то, что реально было сломано — плюс что вложенные
оболочки не убегают, что ребёнок действительно в своей группе (иначе
групповой kill утащил бы за собой сам мост), что обычные команды не
затронуты и что убийство уже мёртвого процесса не бросает исключение.
Проверено саботажем.

### Сценарий 1: может ли агент сам нарастить себе инструменты?

Это не заявка на фичу, а тест среды. Цепочку
``code_project.create -> write -> run -> validate -> publish -> вызов``
прогнали через настоящий диспетчер — так, как её ведёт модель. У каждого
звена были юнит-тесты. Цепочку целиком не запускали ни разу, и она не
работала.

**Петлю ломал словарь.** ``code_project.*`` называет проект ``name``
(7 инструментов), а ``tool_foundry.validate/publish`` тот же проект —
``project``. Агент, сказавший ``project`` инструменту ``write``, получал
*«project name must use only letters, digits, dot, underscore and dash»* —
ошибку валидации про аргумент, который он не отправлял, со ссылкой на
правило, которое его ввод не нарушал. Внутри обработчиков всё было
правильно; сама поверхность противоречила себе. По замеру таких
неймспейсов пять: ``code_project``, ``code_run``, ``code_session``
(name|project), ``capability_gap`` (id|gap_id), ``mission`` (q|query).

Починено один раз в диспетчере, а не в девятнадцати обработчиках — иначе
следующий добавленный инструмент вернул бы проблему. ``_accept_synonyms``
копирует использованное написание в остальные, только внутри
неймспейсов, которые реально противоречат себе, и никогда не затирает то,
что вызывающий указал явно.

**Fence отчитывался как включённый, хотя не мог запуститься.**
Поддержка песочницы определялась через ``shutil.which("systemd-run")`` —
а это ответ на вопрос «установлен ли», а не «работает ли». Без D-Bus
session bus бинарь есть, но каждый вызов падает, и отказ всплывал позже
невнятной ошибкой дочернего процесса. Fail-closed обязан означать, что
fence *работает*: теперь он проверяется исполнением, один раз на процесс, а
недоступный fence отказывает с причиной и маршрутом починки, а не в тупик.

С обоими фиксами петля замыкается: **234 инструмента -> 235**, и агент
вызывает инструмент, который сам только что написал, проверил тестами и
опубликовал — без человека в цепочке.

Закреплено ``tests/test_self_extension_loop.py``: в том числе что publish
по-прежнему отказывает инструменту с падающими тестами (ворота, публикующие
что угодно, обесценили бы всю цепочку) и что синонимы не переписывают явный
ввод. Проверено саботажем в обе стороны.

``arena/autonomy/runner.py`` при починке перешёл порог в 600 строк; probe
вынесен в ``arena/autonomy/fence_probe.py`` (615 -> 572), вместо поднятия
лимита.

### Разбор security-алертов: шесть ложных, и одна настоящая инъекция, которую они не увидели

Сканер сообщил о «command injection from dynamic arguments» в шести местах
плюс RCE через `pull_request_target` в `.github/workflows/check-glama.yml`.
Проверено до починки — по правилу, что находка это утверждение, пока не
измерено.

**У отчёта об RCE нет предмета.** Файла `check-glama.yml` в репозитории нет.
Единственный workflow, работающий с PR, — `relock-dependabot.yml` — уже
использует `pull_request` (не `_target`) с `permissions: {}` на верхнем
уровне, и там же записано почему.

**Пять из шести Python-мест вообще не содержат `shell=True`.** Они собирают
argv-списки и запускают `sys.executable`. Сканер споткнулся о `*paths` в
списке — а это и есть рекомендуемое исправление, а не баг: данные приходят
отдельными элементами argv, и ни один shell их не разбирает.

**Шестое указывает на заглушку.** `hwinfo_cim.py:145-148` конструирует
`subprocess.CompletedProcess`, возвращаемый при исчерпании бюджета прохода;
он ничего не запускает. Два текстовых совпадения `shell=True` в файле — это
проза в комментариях, причём один из них буквально требует, чтобы grep по
этой строке ничего не находил. Проверено исполнением: легитимные имена
классов доходят до `subprocess.run` с `shell=False`, а payload'ы с `;`, `&`,
бэктиком и `$( )` отклоняются whitelist'ом до запуска процесса.

**А потом ворота, написанные чтобы это закрепить, нашли настоящую инъекцию,
о которой сканер не сообщил.** `scripts/superpowers_manager.py`
интерполировал `REPO_DIR` — производный от `ARENA_AGENT_HOME` — в
shell-строку:

    subprocess.run(f"cd {REPO_DIR} && git pull", shell=True)

При `ARENA_AGENT_HOME="/tmp/x; touch /tmp/PWNED; echo "` это три команды.
Подтверждено запуском и появлением файла-маркера, затем подтверждена
безопасность после фикса (argv-форма, `cd` превратился в `cwd=`).

Шесть остальных настоящих `shell=True` — намеренные: очередь задач, чей
контракт и есть «выполни эту shell-команду», операторские CLI-хелперы, один
фиксированный PowerShell-литерал. Shell у них остаётся, но теперь с
письменным обоснованием `nosec`, а не молча.

`tests/test_subprocess_shell_policy.py` утверждает свойства, а не находки:
указанные файлы остаются argv-only, `argv[0]` никогда не вычисляется, каждый
`shell=True` в `arena/` и `scripts/` несёт обоснование, а whitelist hwinfo
отклоняет шесть инъекционных payload'ов, продолжая пропускать легитимные
запросы. Проверено саботажем в обе стороны.

## v4.160.0 — 2026-08-04

Две поправки, которые мог вскрыть только настоящий релиз: проект выходил
под чужой торговой маркой, а ворота, защищающие бейдж версии, не могли
пройти на релизном коммите.

### Переименование: Arena Unified Bridge -> Skainet Bridge

Проект назывался маркой компании, к сервису которой он же умеет
подключаться. В их условиях это сказано прямо: «Arena, Arena Intelligence и
все связанные стилизации ... не могут использоваться без разрешения в связи с
вашими продуктами или продуктами третьих лиц». С публикацией кода проблемы
не было — их условия прямо отказываются от прав на Output — а вот с
названием продукта была.

Переименовано, пока проект маленький: 4 звезды и два открытых PR в каталоги
стоят дня работы, год и сотня форков стоили бы куда больше.

Объём сознательно ограничен брендовым слоем, потому что именно он и есть весь
юридический риск:

    README.md / README.ru.md      отображаемое имя + новый раздел об аффилиации
    pyproject.toml                arena-unified-bridge -> skainet-bridge
    chat_extension/manifest.json  «Arena Chat Bridge MVP» -> «Skainet Chat Bridge»

Внутренние идентификаторы намеренно не тронуты — Python-пакет `arena` (его
импортируют 686 файлов), 119 переменных `ARENA_*`, имя Windows-сервиса
`ArenaUnifiedBridge` и строка `arena-unified-bridge` в `/health`. Это детали
реализации, а не брендинг, пользователю они не видны как название продукта, а
их переименование сломало бы регистрацию сервиса и путь к токену у всех
существующих установок — без единого юридического выигрыша. В README это
написано явно, а не оставлено на догадки.

Две вещи сознательно НЕ тронуты, хотя и попадают в поиск:
в `chat_extension/parser.js` строка «Arena Chat Bridge» входит в протокольные
маркеры, по которым матчится контракт промпта, и расширение по-прежнему
перечисляет arena.ai среди чат-сайтов. Последнее — номинативное
использование (описание совместимости), и теперь это прямо сказано в
дисклеймере рядом с ChatGPT, Claude и Gemini.

5251 тест по-прежнему проходит; отображаемое имя не было заассерчено нигде.
### Пост-релизный фикс: ворота бейджа не могли пройти на релизном коммите

Выпуск v4.159.0 покрасил всю матрицу тестов в красный — 15 из 35 джоб CI — на
репозитории, где ничего не сломано. Причина — ворота, добавленные в v4.158.0.

`test_badge_matches_the_shipped_version` утверждал `badge >= VERSION`: что
`docs/version.json` никогда не отстаёт от `arena.constants.VERSION`. Это
утверждение невыполнимо ровно на том коммите, который важен. Релизный коммит
поднимает версию и пушится *раньше*, чем успевает отработать
`version-badge.yml`, поэтому на нём бейдж по построению равен предыдущему
релизу:
...
Ворота, которые не могут пройти на здоровой системе, — это не ворота, а
сигнализация, которую придётся игнорировать; а игнорируемая сигнализация — это
способ пропустить настоящую. Свойство одностороннее: бейдж может отставать от
дерева между бампом и запуском workflow, но он не должен *обгонять* его
(объявляя релиз, которого не было — ровно это дала гонка в v4.158.0) и не
должен отставать больше чем на один minor (это означало бы, что workflow
бейджа молча не работает уже несколько релизов).

Проверено саботажем трижды: бейдж впереди дерева, бейдж на несколько minor
позади и `tag_name`, расходящийся с `semver`, — все три падают.


## v4.159.0 — 2026-08-04

Три живых дефекта, и все найдены использованием системы, а не чтением:
два — вызовом каждого MCP-инструмента по разу, один — попыткой убрать
рейт-лимитер с дороги, чтобы померить латентность.

У моста появились первые опубликованные цифры производительности, а сам
репозиторий наконец присутствует в хабах тем GitHub — у него не было ни
описания, ни топиков, поэтому он отсутствовал, а не был занижен в выдаче.

### Видимость: сначала замер, потом починка того, что действительно сломано

У репозитория было `description: null` и ноль топиков, поэтому он отсутствовал
во всех хабах тем GitHub — не занижен в выдаче, а отсутствовал. Через API
проставлены 16 топиков и описание.

Дальше ландшафт MCP-каталогов проверен по их собственной документации, а не по
предположениям:

- **`punkpeye/awesome-mcp-servers`** — запись подготовлена в ветке
  `IvanSkainet:add-arena-unified-bridge` в категории *OS Automation*: одна
  строка, легенда соблюдена, проверено что diff ровно `1 insertion(+)` и число
  заголовков `### ` не изменилось. Их CONTRIBUTING прямо разрешает агентские PR
  с маркером `🤖🤖🤖`. Сам PR требует клика человека: fine-grained PAT не может
  открывать pull request'ы в чужих репозиториях.
- **`modelcontextprotocol/servers`** — список сторонних серверов *упразднён* в
  пользу официального MCP Registry, и репозиторий теперь прямо пишет, что новые
  реализации серверов не принимает. PR туда был бы отклонён по политике.
- **Официальный MCP Registry** — канонический источник, из которого читают
  остальные каталоги, но он индексирует пакеты, а не репозитории: нужен артефакт
  на PyPI/npm/NuGet/OCI/MCPB плюс токен владения `mcp-name:`. Это не блокер, а
  предусловие — сначала пакет, потом реестр.
- **Glama** — добавлен `glama.json`, провалидированный через `jsonschema` против
  живой схемы, скачанной с `glama.ai`, чтобы индексатор использовал наши
  метаданные, когда сабмит состоится.

Разбор с обоснованиями записан в `docs/github_apps_actions_survey.md`.

### Живой баг: рейт-лимитер навсегда банил любого клиента с ретраями

Чтобы померить латентность моста, лимитер надо было убрать с дороги — так
его и прочитали внимательно. `check_rate_limit_v2` добавлял метку
*отклонённого* запроса в то самое скользящее окно, по которому решает об
отказе:

    if remaining <= 0:
        ep_store.append(now)      # <-- баг
        return 429

Окно чистится условием `now - t < window`, поэтому клиент, стучащий чаще
`window`, всё время обновляет хранилище, и самая старая метка никогда не
успевает устареть. Это не throttling, это вечный бан, и срабатывает он на
самом обычном поведении клиента. Замер, limit=10, window=60s:

    стук раз в 70с -> восстановился через 70с
    стук раз в 5с  -> всё ещё заблокирован через 1000с
    стук раз в 1с  -> всё ещё заблокирован через 600с

Восстановился только клиент, вежливо отступивший за пределы всего окна, —
ровно наоборот от нужного: дашборд опрашивает раз в секунду, а любой цикл
ретраев агента ещё быстрее. У `check_rate_limit` (v1, висит на том же
запросе) этого бага не было, то есть два лимитера по-разному понимали, что
такое окно.

Закреплено `tests/test_rate_limit_recovery.py`: часы инжектируются, а не
спятся; пять интервалов ретрая, проверка что лимитер вообще ещё отказывает,
и v1 как контроль. Проверено саботажем — возврат append роняет ровно те
интервалы, что короче окна.

### Производительность моста: измерена, утечек нет

Первые цифры латентности (loopback, один воркер, прогретый):
...
Поведение ресурсов на 40 принудительных таймаутах инструмента (`sleep 30`,
убитый на 1с): дескрипторов +0, потоков +0, RSS +0, осиротевших процессов
нет. Путь таймаута не течёт.

### Два живых бага от первого в истории вызова всех 234 MCP-инструментов

У MCP-поверхности было двое ворот, и ни одни ни разу не вызвали инструмент.
`test_mcp_tool_contracts.py` сканирует AST на литералы имён;
`test_mcp_input_schema_validation.py` проверяет JSON Schema деклараций. Оба
проходят на инструменте, чей обработчик падает при каждом вызове.

Поэтому инструменты вызвали — все 234, через настоящий диспетчер `call_tool`
из `make_mcp_tool_runtime`, с пустыми аргументами. Вопрос был не «отработает
ли» (без аргументов почти ничто не может), а «отвечает ли он словами». Двое
не отвечали.

**`capability_gap.promote` не работал никогда.** Лишняя пара скобок:

    cfg = (ctx.app_config() if ctx and hasattr(ctx, "app_config") else lambda: {})() if ctx else {}

Хвостовые `()` относятся ко всему условному выражению, поэтому на реально
исполняемой ветке вызывается *результат* `app_config()` — словарь. Каждый
вызов бросал `TypeError: 'dict' object is not callable`, его глотал общий
`except` диспетчера, и модель получала строку с ошибкой. Инструмент был
объявлен, показан в `tools/list`, классифицирован по риску — и мёртв в 100%
вызовов. Тестов на него не было; ратчеты его не видят, потому что это
валидный Python, который ломается только при исполнении.

**Пять `mission.autopilot_*` отказывали пустым предложением.** `_load`
бросает `FileNotFoundError(run_id)`, а при пустом run_id это исключение
вообще без текста, и диспетчер отдавал буквальное `ERROR: FileNotFoundError:`
— это не сообщает вызывающему ничего: ни какой ран, ни что не хватает
аргумента, ни что повторять бессмысленно. `status`, `report`, `cancel`,
`step` и `artifacts` теперь отвечают структурой `{"ok": false, "error": ...}`,
как и вся остальная поверхность.

### Ворота

`tests/test_mcp_tool_dispatch_contract.py` вызывает каждый объявленный
инструмент и проверяет три свойства: никто не отвечает `Unknown tool:`
(объявлен, но недостижим — ровно это даёт односторонний ренейм), ни один
отказ не пуст, и ни один `TypeError`/`AttributeError`/`NameError` не уходит
из обработчика на корректном вызове. Плюс проверка, что сама проба ещё умеет
падать — чтобы ворота, молча начавшие показывать ноль, не приняли за
проходящие.

Проверено саботажем трижды: возврат бага со скобками роняет
`capability_gap.promote`, возврат голого `_load` роняет
`mission.autopilot_status`, удаление одной ветки из цепочки диспетчера роняет
все четыре `git.*`.

### Ещё

`arena/mission_autopilot.py` при починке перешёл порог мини-монолита в 600
строк. Вместо того чтобы поднять лимит, слой хранения вынесен в
`arena/mission_autopilot_store.py` (615 -> 568 строк) с реэкспортом — ни один
вызов и ни один тест с monkeypatch не менялся.

Метаданные репозитория: у проекта не было `description` и было ноль топиков,
поэтому он отсутствовал во всех хабах тем GitHub — не занижен в выдаче, а
отсутствовал. Проставлены 16 топиков и описание.

## v4.158.0 — 2026-08-03

Пять живых дефектов, и каждый найден так: задать инструменту вопрос, а потом
проверить ответ исполнением, а не поверить ему.

Три пришли из замера MegaLinter (предлагался как замена ruff; оказалось, что
он ruff *запускает* внутри себя). Два — из работы по безопасности в прошлом
цикле: parser differential в SSRF, проявлявшийся только на macOS, и ворота
качества, которые никогда по-настоящему не работали.

PR с обновлением зависимостей теперь сами перегенерируют свои hash-locked
requirements, а `master` защищён от force-push и удаления.

### Живой баг: бейдж версии мог поехать назад

Выпуск v4.158.0 вскрыл гонку, которая жила в `version-badge.yml` с момента
написания. Релиз запускает workflow дважды — один раз на тег, который знает
своё имя, и один раз на push в master, который спрашивает у API
`releases/latest`. В этот раз второй запуск спросил на девять секунд раньше,
чем нужно:

    22:00:14  запуск на теге  -> записал 4.158.0
    22:01:13  запуск на push  -> перезаписал на 4.157.0
    22:01:22  релиз наконец стал "latest"

Оба запуска зелёные. Сломан оказался только сам факт, который публиковал
бейдж: читателю сообщалось, что проект откатился на версию назад, и заметить
это можно было, только открыв файл.

Чинить таймингом нельзя — порядок завершения двух запусков нам не подчиняется.
Теперь workflow отказывается записывать semver старее того, что лежит на
диске: гонка становится безобидной в любом порядке. При отказе он предупреждает,
а не падает (отстающий API — не ошибка). Под guard'ом оба шага, и запись, и
коммит: закрыть только один означало бы оставить ту же дыру.

Закреплено `tests/test_version_badge_monotonic.py`, включая случай
`4.9.0 -> 4.10.0`, который строковое сравнение считает движением назад, и
проверку, что бейдж не отстаёт от `arena.constants.VERSION`.

### Три живых бага из разового замера MegaLinter

MegaLinter предлагался как замена ruff. Заменой он быть не может: он
**запускает ruff внутри себя** — в его Python-дескрипторе десять линтеров, из
которых три у нас уже стоят, четыре это форматтеры или инструменты для
Jupyter, а сама обёртка *лишила* бы нас `pyrefly`, `semgrep`, `vulture` и
`import-linter`. Поэтому вопрос свёлся к «что видят два его действительно
новых линтера, чего не видят наши» — и ответ окупил заход.

* **`mission_manager.py run` выходил с кодом 0 на провалившейся миссии.**
  Результат терялся в трёх слоях: worker вычислял `ok` и ничего не возвращал,
  `cli.main()` звал подкоманду ради побочных эффектов, шим звал `main()` без
  `SystemExit`. JSON на stdout всегда честно писал `"ok": false` — человек
  видел провал, обманывалась только машина, ради которой этот CLI и
  существует. Воспроизведено запуском миссии с падающим шагом, починено во
  всех трёх слоях. (pylint `assignment-from-no-return`)

* **`_ensure_wm()` падал с `NameError` при первом же вызове.** Он объявляет
  `global _wm_started` и читает переменную, которую никто не создал на уровне
  модуля, — то есть `click`, `key` и `type_text` в CLI-пути десктопа умирали
  до начала работы. Тот же класс, что star-import находки v4.155.0.
  (pylint `used-before-assignment` и pyright `reportUnboundVariable`,
  независимо друг от друга)

* **Детектор блокировки CDP event loop ломался ровно при блокировке.**
  `_cdp_loop_healthy_ts` читается только в ветке `TimeoutError` и тоже не имел
  модульного биндинга, так что первый же таймаут убивал задачу детектора через
  `NameError` — сигнализация молча отключалась именно тогда, когда была нужна.
  Этот баг нашёл уже не сканер, а **ворота, написанные на класс** после
  предыдущего.

Оба класса теперь под воротами: `test_global_declarations_are_bound.py`
обходит все объявления `global` в `arena`/`scripts`/`bin` и падает, если имя
читается без модульного биндинга, а `test_type_checking_imports_resolve.py`
ловит `TYPE_CHECKING`-импорты несуществующих модулей — именно так
`network_har.py` импортировал `NetworkRequest` из отсутствующего
`network_types`, и аннотация была молча мертва.

Ни pylint, ни pyright не принимаются: 6852 находки на один настоящий баг, а
второй тайпчекер — это две очереди triage за один класс сигнала. Замер
записан в `docs/github_apps_actions_survey.md`.

### PR от Dependabot теперь сами перегенерируют свои lock-файлы

Dependabot открыл первое групповое обновление (7 runtime-зависимостей), и CI
завалил его на четырёх джобах. Падение было правильным: Dependabot правит
`requirements-*.in`, а CI ставит через `--require-hashes -r requirements-*.lock`
и `.in` не читает вообще — значит каждое обновление приезжало с протухшим
локом, о чём `check_lock_freshness.py` и сообщил. Без этих ворот PR выглядел бы
слитым, а на машине остались бы старые версии.

Очевидным решением был Renovate с его `postUpgradeTasks`, существующими ровно
для того, чтобы запустить `uv pip compile` после обновления. Проверено до
внедрения: **эта опция работает только при self-hosting**. Хостед-приложение
Mend её игнорирует, поскольку `allowedPostUpgradeCommands` — bot-admin
настройка, недоступная из конфига репозитория (renovatebot/renovate#16555,
#11206). То есть выбор был между «Dependabot плюс пятнадцать строк своего
YAML» и «поднять и обслуживать бота ради одной команды» — поэтому появился
`.github/workflows/relock-dependabot.yml`.

Его форма важнее самого факта:

* `pull_request`, а не `pull_request_target` — код PR исполняется с
  read-only токеном по умолчанию;
* write-права принадлежат одной джобе и тратятся на один push;
* гейт по `github.event.pull_request.user.login`, а **не** по
  `github.actor`, который zizmor справедливо считает подделываемым;
* `github.head_ref` попадает в скрипт через переменную окружения, а не
  подстановкой в строку: имя ветки — это текст, контролируемый атакующим;
* **fail closed**: если перегенерированные локи не проходят
  `check_ci_lock.py` и `check_lock_freshness.py`, джоба падает и не пушит
  ничего. Проверено саботажем — пином без `--hash=`.

Само обновление проверено исполнением, а не доверием: hash-locked установка в
чистый venv, полный сьют на новых зависимостях (5022 passed) и живой сервер из
этого venv, отвечающий `/health` 4.157.0, отклоняющий `/v1/exec` без токена и
отдающий 234 MCP-инструмента. `pathspec` пересёк мажорную границу
(0.12 → 1.1) и является только транзитивной зависимостью.

### master защищён от force-push и удаления

В репозитории не было ни одного ruleset. Добавлен один на `refs/heads/master`
с правилами `non_fast_forward` и `deletion`, проверен попыткой сделать и то и
другое: оба push вернулись с `remote rejected ... repository rule violations`.

Сознательно **не** включены требования pull request и status checks на master:
туда пушат напрямую и badge-workflow, и мейнтейнер, а правило, ломающее
релизный процесс, будет выключено при первой же помехе. Добавлены ровно те два
правила, которые предотвращают необратимый ущерб, не меняя способ работы.

### Исправлено: dependabot.yml ссылался на несуществующие labels

В каждом PR Dependabot писал: «The following labels could not be found:
`automated-pr`, `dependencies`». Конфиг просил метки, которых никто не
создавал, — то есть маркировка автоматических PR не работала с самого начала.
Метки созданы.

### Живой баг: на macOS SSRF-валидатор и HTTP-клиент видели разные хосты

Добавленный выше корпус атак сделал ровно то, ради чего пишут adversarial-тесты:
упал на пяти macOS-ячейках при зелёном Linux — и различие оказалось настоящим
дефектом, а не артефактом теста.

`socket.inet_aton` по-разному обходится с переполнением в зависимости от libc:
glibc отвергает значение больше 2**32, BSD/macOS молча **усекает** его до
младших 32 бит. `_coerce_ip` выходил из собственной числовой ветки на
`n > 0xFFFFFFFF` и проваливался в `inet_aton`, поэтому на macOS
`http://99999999999999/` декодировался в `16.122.63.255`, «приватным» не
считался и **проходил валидацию** — после чего HTTP-клиент с той же libc шёл
на 16.122.63.255. Проверяющий и сокет видели разные хосты. А то, что усечение
попало на публичный адрес, а не на loopback, — удача: `6425673729` это
`127.0.0.1 + 2**32`.

Починено отказом отдавать решение libc. `_coerce_ip` сам проверяет диапазон
каждого компонента (граница зависит от их числа: `inet_aton` читает последний
как остаток байтов), а `_validate_url` отвергает полностью числовой хост, если
тот не декодировался, — такая строка всегда попытка записать адрес, а не имя.
Проверено под эмуляцией усекающей libc, чтобы защита была протестирована на
платформе, до которой CI добирается реже всего.

### Ворота vulture никогда по-настоящему не работали

`quality_ratchet.py` фильтровал вывод vulture условием `": (" in ln`.
Vulture печатает `path.py:12: unused import 'x' (90% confidence)` — никакого
`: (` там нет. Фильтр не совпадал ни с чем, `vulture_count()` всегда возвращал
0, и каждое «vulture=0» за всю историю репозитория означало *«не считаю»*, а
не *«чисто»*.

Всплыло только потому, что ворота расширили на `scripts/` и `bin/`, а новый
scope проверили саботажем: мёртвый импорт в `bin/` оставил ratchet зелёным,
хотя `python -m vulture` строкой выше находку показывал. Ворота, которые
всегда говорят ноль, неотличимы от ворот, которые проходят, — ровно тот
green-checkmark fallacy, о котором предупреждает AGENTS.md, на этот раз в
нашем собственном инструменте.

Починено настоящим парсером плюс fail-closed: если vulture вышел с кодом 3
(«найден мёртвый код»), а не разобралась ни одна строка, ratchet аварийно
завершается вместо отчёта о чистом дереве. Закреплено
`tests/test_quality_ratchet_counts.py`, который скармливает счётчику реальный
вывод vulture: на чистом дереве и сломанный фильтр, и правильный дают 0, так
что разницу доказывают только синтетические находки.

Заодно ratchet перестал передавать пути в командной строке — теперь он и
голый `python -m vulture` читают один и тот же scope из `[tool.vulture]`.

### vulture теперь покрывает scripts/ и bin/

Причина та же, что у остальных ворот: `make_release_zip.py` кладёт оба
каталога в архив, значит пользователь этот код исполняет. 28 находок, из них
27 — `scripts/cdp_browser.py`, шим, чьи импорты и ЕСТЬ его контракт (исключён
по построению), и одна — `complete_event`, параметр, которого требует
интерфейс `Completer` из prompt_toolkit. Настоящего мёртвого кода ноль;
ценность в scope, а не в уборке.

### У SSRF-валидатора появился корпус атак (mutation testing отклонён)

Оценка mutation testing для nightly-слота закончилась замером, а не
инструментом. `mutmut` 3.7 не запускается здесь без второго, никем не
проверяемого `pyproject.toml` (он вызывает pytest своими аргументами и падает
на coverage-воротах), а его балл отвечает на вопрос «сколько искусственных
правок пережил сьют», тогда как полезен другой: «переживёт ли он правку,
которую сделает человек» — и его доктрина намеренного саботажа уже задаёт,
для каждых ворот и с письменным обоснованием.

Зато оценка кое-что нашла: у `arena/security_ssrf.py`, защиты, через которую
ходит **21 модуль**, не было ни одного прямого теста.
`tests/test_security_ssrf_bypass_corpus.py` теперь бросает в него стандартный
набор обходов — десятичные/шестнадцатеричные/восьмеричные записи 127.0.0.1,
`127.1`, IPv4-mapped IPv6, завершающие точки, регистр, userinfo-префиксы,
metadata-IP облаков, не-HTTP схемы — плюс легитимные URL, которые обязаны
проходить. Все 26 уже блокировались.

А саботаж научил тому, что стоит записать: удаление декодера целых чисел
оставляет все URL-уровневые проверки зелёными, потому что `getaddrinfo`
резолвит `http://2130706433/` и результат перепроверяется. Защита
трёхслойная (`ip_address` → `inet_aton` → DNS), и корпус, проверяющий только
исход, пропустил бы молчаливое удаление первого слоя. Теперь у каждого слоя
свой юнит-тест.

Затем тот же урок преподал CI. Один случай утверждал
`_coerce_ip("4294967296") is None` и упал на всех пяти macOS-ячейках:
поведение `inet_aton` при переполнении зависит от libc — glibc отвергает
значение, BSD усекает его до `0.0.0.0`. Оба исхода безопасны, но по разным
причинам, то есть проверялась деталь платформы, а не свойство безопасности.
Переписано как настоящий инвариант: либо значение не декодируется и резолвер,
общий с HTTP-клиентом, тоже его отвергает, либо декодируется — и тогда адрес
обязан быть заблокирован. В корпус добавлен `6425673729` — это
`127.0.0.1 + 2**32`, чьи младшие 32 бита равны loopback, лучший выстрел
атакующего по пути усечения; отвергается на обеих платформах.

## v4.157.0 — 2026-08-03

Все статические ворота репозитория теперь показывают ноль, а дашборд
проверяется исполнением, а не чтением исходника.

| ворота | scope | до цикла | сейчас |
|---|---|---|---|
| ruff | всё дерево | 35 | **0** |
| pyrefly | `arena`, `scripts`, `bin` | 213 | **0** |
| vulture | `arena` | 0 | 0 |
| oxlint | `dashboard/assets`, `chat_extension` | (линтера не было) | **0** |

По дороге найдено шесть живых дефектов, каждый закрыт воротами, проверенными
намеренной поломкой: битый файл расписания ронял `save_schedule_def`, fs-edit
маршруты отдавали невнятный 500 без проводки хуков, `bridgeFetch` мог отдать
`fetch()` тело на GET, `.ruff_cache/` уезжал в каждый релизный ZIP,
`smoke_mobile.py` показывал отказ WebSocket-хендшейка как успех, а `_check()`
записывал `None` вместо `False`.

Дашборд оказался здоров — смысл нового browser E2E в том, что «здоров» теперь
измеряется на каждом пуше, а не предполагается на 58 файлах, которые не
запускал ни один тест.

### Ворота теперь покрывают всё дерево, и pre-commit об этом знает

Расширив ruff-ratchet на `scripts/` и `bin/` ранее в этом цикле, я не задал
очевидный вопрос: а что ещё осталось снаружи? Ответ — 19 находок в 7 файлах, и
список стыдный в правильном смысле: сам `unified_bridge.py`, `_arena_helper.py`,
четыре `skills/*/run.py` и dev-скрипт. Ворота, исключающие главную точку входа,
— это не ворота, а предпочтение.

`TARGETS` теперь `(".",)`, а не список каталогов: новый файл в корне попадает
внутрь ворот по умолчанию, а не оказывается снаружи молча.

Одна из 19 находок долгом не была: `import resource` в `unified_bridge.py`
выглядит неиспользуемым, но он материализует Windows-мок в `sys.modules`
раньше, чем какая-либо зависимость импортирует настоящий модуль. Удаление
перенесло бы отказ в тот модуль, который импортирует `resource` первым, — на
платформе, которую тестируют реже всего. Теперь там аннотация с причиной.

Остальное механика (7 × `I001`, 8 × `E702`, 1 × `E701` через AST-проверенные
сплиттеры, 2 действительно мёртвых импорта удалены с AST-диффом). Проверено
запуском сервера: `/health` по-прежнему отвечает.

**pre-commit** был настроен, но появился раньше трёх из четырёх ворот. Теперь
он гоняет lint-, quality- и JS-ratchet в дополнение к `ruff check`, причём
JS-хук привязан к `types: [javascript]`, так что Python-коммиты не платят
ничего. Заодно перестал быть правдой его комментарий: он оправдывал
отключение whitespace-фиксеров ссылкой на долг W291/W293, которого больше нет.
Фиксеры всё равно остаются выключенными — по более весомой причине: хук,
молча переписывающий файлы, делает коммит, который никто не смотрел.

### Последние ворота долга на нуле: JS 15 -> 0

Теперь все статические ворота репозитория молчат: ruff 0, pyrefly 0,
vulture 0, oxlint 0. JS-порог оставался единственным ненулевым, и разбирать
его стоило вручную: три находки из пятнадцати были высказываниями о коде, на
которые нужен ответ, а не правка.

* **`refreshSettings` патчился дважды по имени.** Два модуля делали
  `const orig = refreshSettings; refreshSettings = async () => { await orig();
  ... }`. При исполнении в настоящем браузере оба слоя срабатывают — реестр
  вкладок зовёт `refreshSettings()` по имени, поэтому переприсваивание видно.
  Но держится это на удаче: любой вызывающий, сохранивший ссылку, или третий
  патч в том же стиле — и цепочка молча рвётся. Заменено явным списком
  `registerSettingsRefreshHook()`, где каждый хук изолирован; в Chromium
  проверено, что оба обновления по-прежнему срабатывают за одну активацию.

* **`[...map.entries()]` в `pruneMountedControls` — несущая конструкция.**
  Линтер счёл spread лишним; но цикл вызывает `mountedControls.delete()`, а
  расширение перемонтирует контролы, так что живой итератор Map обошёл бы и
  записи, добавленные во время итерации. Замерено: при повторном `set()` живой
  итератор продолжает (`a,gen1,gen2,...`), а снимок останавливается. Помечено
  этим замером, а не «исправлено».

* **`20-section.js` — две строки комментария и ни строчки кода.** Манифест
  строится сканом каталога, поэтому файл скачивался и разбирался при каждой
  загрузке дашборда. Удалён, добавлены ворота
  (`test_no_asset_is_only_a_comment_header`), чтобы пустой ассет не вернулся.

Остальное — настоящие упрощения, каждое перед применением доказано
эквивалентным: `{...(x || {})}` -> `{...x}` (spread любого falsy даёт `{}` —
проверено для null/undefined/false/0/""), `new Array(n).fill(null)` ->
`Array.from({length: n}, () => null)`, `/^\/agent\//.test(p)` ->
`p.startsWith('/agent/')` (14 путей, включая граничные, поведение не менялось),
и `x && x.close()` как инструкция -> обычный `if`.

Безопасным всё это сделал browser E2E дашборда, добавленный ранее в этом
цикле: он перезапускался зелёным после каждой правки, против настоящего
Chromium.

### Browser E2E для /gui: дашборд наконец исполняется, а не читается

Дашборд — это 60 файлов JavaScript и 17 тысяч строк. До этого релиза ровно
**два** из них когда-либо исполнялись тестом (Node-обвязки в
`tests/test_overview_*_js.py`). Всё остальное проверялось чтением исходника:
существует ли файл, сбалансирован ли `.main` в склеенном HTML, перечисляет ли
манифест то, что ждёт загрузчик. Всё это полезно и ни одно из этого не может
сказать, что страница рисуется.

У этого пробела есть история. В v4.96.0 уехал лишний `</div>`, вытолкнувший
шесть вкладок за пределы `.main` — тот самый баг «контент съехал вправо, в
середине пусто». Написанные тогда ворота разбирают HTML, поэтому повтор того
же симптома чисто через CSS или JS прошёл бы мимо них насквозь.

`tests/e2e/test_dashboard_browser.py` запускает настоящий Chromium против
настоящего сервера (переиспользуя фикстуру `bridge` из существующего live E2E,
чтобы контракт запуска был один, а не два) и проверяет наблюдаемое поведение:

* при загрузке ноль ошибок консоли, ноль ошибок страницы, ноль неудавшихся
  запросов, и грузится полный набор ассетов, а не аварийный список из пяти;
* каждая из 22 ссылок сайдбара показывает **ровно одну** видимую панель;
* обход всех вкладок — а это срабатывание `onShow` и его API-вызовов — не даёт
  ни одного 4xx/5xx и ни одного необработанного исключения (185 запросов, все 200);
* без токена страница — форма входа, не обращающаяся к API вообще;
* перезагрузка не задваивает сайдбар;
* задокументированный fail-soft исполняется во всех четырёх режимах отказа
  (HTTP 500 / пустые scripts / битый JSON / прерванный запрос): оболочка обязана
  отрисоваться **и** сказать об этом на экране, а не только в `console.warn`.

Каждая проверка перед тем, как ей доверять, была проверена намеренной поломкой:
снятие CSS, схлопывающего скрытые панели, опечатка в классе `active` в
переключателе вкладок и обнуление баннера fallback — каждая красила сьют
в ожидаемом месте.

Вердикт по самому дашборду: здоров. Чинить оказалось нечего — ценность в том,
что «здоров» теперь измеренный факт, перепроверяемый на каждом пуше, а не
допущение, опирающееся на 58 файлов, которые не запускал ни один тест.

### Рецепт генерации lock был неполным, и это стоило круга отладки

Добавление `playwright` в `requirements-ci.in` и регенерация командой,
напечатанной в шапке этого же файла, дали lock с **нулём** маркерных пинов:
`async-timeout`, `exceptiongroup` и `tomli` молча исчезли — ровно тот
«cold-cache marker-drop», ради которого существует `scripts/check_ci_lock.py`.
Он его и поймал.

Причина в том, что `uv pip compile` резолвит под тот интерпретатор, на котором
запущен, если не сказать иначе; `--python-version 3.10` обязателен, а не
косметичен. Теперь это написано в шапке вместе с замером (0 маркерных строк
против 6).

### scripts/ и bin/ входят в ворота: 433 ruff + 67 pyrefly до нуля

Эти два каталога никогда не проверялись ничем — считалось, что там служебные
скрипты сборки. Это не так: `make_release_zip.py` кладёт оба в релизный архив,
то есть пользователь их скачивает и исполняет. Теперь ratchet'ы покрывают
`arena`, `tests`, `scripts`, `bin` (ruff) и `arena`, `scripts`, `bin`
(pyrefly), все с порогом ноль.

Порядок, который делает уборку честной:

1. **Сначала зафиксировано поведение.** Все 82 точки входа в `scripts/` и
   `bin/` прогнаны с `--help`, коды возврата записаны до и после каждого шага.
   Итоговая разница: **0 из 82 изменились.**
2. **Механические слои — AST-проверенными инструментами**, написанными ещё для
   `arena/`: 167 разбиений E702 и 113 E701, файл принимается только если
   разобранный AST байт-идентичен. `e702_split_statements.py` получил `--paths`
   по образцу `e701_split_compounds.py`.
3. **F401 — вручную, никаких bulk-правок** (запрещено AGENTS.md). Каждое имя
   проверено на реальное использование; четыре оказались availability-пробами,
   где импорт и ЕСТЬ проверка (`prompt_toolkit`, `pytest`, `requests`) — они
   помечены, а не удалены.

Две находки стоит назвать отдельно:

* **`scripts/cdp_browser.py` — несущий шим, а не мёртвый код.** Реализация CDP
  переехала в `arena/browser/cdp_client/*`, но рантайм по-прежнему импортирует
  файл ПО ИМЕНИ: `arena/browser/cdp/loader.py` кладёт `scripts/` в `sys.path`,
  делает `import cdp_browser` и читает с модуля `CDPTabManager` и `list_tabs`.
  То есть «неиспользуемые» импорты — это его контракт. Удалено 12 реально
  мёртвых stdlib-импортов (с AST-доказательством, поверхность ре-экспорта
  проверена — 43 имени на месте), остальные объявлены в `per-file-ignores`
  с обоснованием.

* **`smoke_mobile.py` показывал провалившийся WebSocket-хендшейк как успех.**
  `except Exception as e` захватывал причину и возвращал голый `-1`, а проверка
  печатала для него «handshake completed». Теперь причина доносится и
  показывается. Там же `_check(cond)` получал `s == 200 and r.get("ok")` — при
  отсутствующем ключе это `None`, а не `False`, так что провал записывался ни
  как успех, ни как провал; приведение сделано на границе. Разделение `_http`
  (JSON) и `_http_raw` (бинарь) сняло 62 из 67 находок pyrefly одним симптомом:
  25 из 26 вызовов бинарную ветку никогда не хотели.

Попутно исправлено: `Popen(stdout=PIPE).stdout` итерировался без проверки на
None, `cwd: str = None`, и два предиката `-> bool`, охраняющих доступ к
атрибуту, стали `TypeGuard`.

### У JavaScript появился линтер — и он сразу нашёл баг

`dashboard/assets` + `chat_extension` — это 17 125 строк JavaScript, на
которые не смотрел ни один инструмент: semgrep запускается только по
`arena/`, а CodeQL — SAST-движок, а не линтер; ни тот, ни другой не сообщает
про `no-func-assign` или тело в GET-запросе. Теперь по обоим деревьям в CI
идёт oxlint (~40 мс на 72 файла) под тем же ratchet-паттерном, что ruff и
pyrefly: `scripts/js_lint_ratchet.py`, порог в
`scripts/js_lint_baseline.json`.

Первый же прогон себя окупил:

* **`bridgeFetch` мог отдать fetch() тело на GET.** Сигнатура —
  `bridgeFetch(path, {method = 'GET', body})`, а `bridgeFetchOnce`
  сериализовал `body` при любом истинном значении. По спецификации это
  `TypeError: Request with GET/HEAD method cannot have body`, который внутри
  service worker выглядит как невнятная «сетевая ошибка». Все четыре текущих
  вызова передают `method: 'POST'` явно, так что ружьё было заряжено, но не
  выстрелило. Guard теперь исключает GET/HEAD; проверено исполнением реальной
  функции против `fetch`-заглушки, соблюдающей спецификацию.
  (`tests/test_extension_fetch_body_guard.py`)

* **`.ruff_cache/` уезжал в каждый релизный ZIP.**
  `scripts/make_release_zip.py` исключал кеши перечислением
  (`__pycache__`, `.pytest_cache`, `node_modules`, `.mypy_cache`), поэтому
  кеш каждого нового инструмента исключался, только если кто-то вспомнит его
  дописать. Кеш ruff не вписали: в v4.156.0 уехало 17 файлов и 316 КБ
  хешированных блобов. Правило переписано по форме, а не по списку: любая
  dot-директория, оканчивающаяся на `_cache`/`-cache`, — остатки сборки.
  Архив похудел 1040 → 1023 файла.
  (`tests/test_make_release_zip_exclude.py`)

Линтер зафиксирован по хешам, как и всё остальное: `npm ci` из закоммиченного
`package-lock.json` (у каждой записи integrity-хеш), а не
`npm install oxlint@x` — zizmor указал на ad-hoc установку, и был прав.

### AGENTS.md: прогоняй проверки до завершения

Оба долговых гейта на нуле, а ноль нужно активно удерживать: Debt visibility
краснеет на первой же новой находке. В AGENTS.md добавлен явный ритуал перед
завершением задачи (ruff + оба ratchet'а + JS-ratchet, если трогали `.js`) —
взято из статьи Pyrefly «Adding Pyrefly Type Checking to Your Agentic Loop».
Их наблюдение совпало с нашим: установленный инструмент и привычка его
запускать — разные вещи.

### Обзор рынка обновлён

В `docs/github_apps_actions_survey.md` записан замеренный вердикт ещё по 20
Apps/Actions. Двое приняты (oxlint и директива в AGENTS.md); jscpd прогнан
один раз и оставлен отчётом, а не воротами (2.41 % дублирования — ниже любого
разумного порога); aislop замерен и отклонён: 3078 находок и ноль
подтверждённых дефектов, потому что он штрафует ровно то, что у нас
предписано — комментарии «почему», ре-экспортные фасады, fail-open handler'ы.
...
охоты, теперь **0**. Эта группа дала пять багов релиза, так что вычистить её
стоило, а не терпеть.

Последний проход нашёл ещё один настоящий дефект и исправил несколько
попросту неверных аннотаций:

* **`_fmt_displays` падал на числовом разрешении.** `' · '.join(parts)` по
  значениям прямо из словаря пробы даёт `TypeError: sequence item 1: expected
  str instance, int found` — и роняет весь текстовый отчёт инвентаря.
  Воспроизведено на реальной функции, исправлено через `str()`.
* **`res and res.get(...)` в двух методах `-> bool`** возвращал `None` при
  ответе None и `{}` при пустом словаре. Проверки на истинность это скрывали,
  `is False` и сериализация в JSON — нет.
* `launch_browser` объявлен `-> subprocess.Popen`, возвращая None при сбое.
  Единственный вызывающий значение игнорирует, так что неверна была только
  аннотация.
* `handle_sse` возвращает подготовленный StreamResponse, а не Response.
* `json.loads` может вернуть список или число; `tool_mission` теперь проверяет
  это перед `setdefault`.
* Два цикла прослушивания WebSocket утверждают то, что гарантирует их
  планировщик: задача создаётся только после успешного `ws_connect`.

pyrefly **229 -> 213**.

### Ещё два бага: null-путь ронял пять инструментов, а отсутствующий runtime — песочницу

**`{"path": null}` бросал TypeError вместо ошибки.** `args.get("path", "")`
выглядит как значение по умолчанию, но умолчание срабатывает только когда ключ
*отсутствует*. JSON с явным null возвращал None, который попадал в
`os.path.expanduser` и давал

    TypeError: expected str, bytes or os.PathLike object, not NoneType

изнутри обработчика. Пять мест с такой формой — fs.read / fs.write / fs.list,
fs.search, tree diff и git-инструменты — все доступны MCP-клиенту. Исправлено
через `or`, а новые ворота сканируют всё дерево на `.get(k, default)`,
подаваемый в функцию, не принимающую None.

**Отсутствующий runtime ронял вместо отказа.** `_resolve_win32_runtime`
возвращает None, когда на хосте есть только alias-заглушка WindowsApps. Ветка
AppContainer этот случай уже отвергала, но она достижима лишь для fenced-postures
— при нестрогой posture None уходил в `argv[0]`, и `subprocess.run` давал тот же
TypeError. Теперь отказ происходит до сборки argv, с переиспользованием
существующего сообщения.

Стоит записать: моя первая попытка продублировала существующий отказ и сломала
`test_build_win32_missing_runtime_refused_fail_closed` — тест был прав, у
fenced-пути сообщение точнее. Дубль удалён, настоящая дыра закрыта там, где она
и была.

pyrefly **233 -> 229**, группа «вызов не может выполниться» **12 -> 9**.

### Одиннадцатый баг: каждая отключённая вкладка считалась подключённой

`CDPTab.connected` был обычным методом, но шесть мест читают его как значение —
`if tab.connected:`, `return active_tab.connected`, `"●" if tab.connected`.
Объект связанного метода всегда истинен, поэтому вкладка с `_connected = False`
и без браузера сообщала о себе как о подключённой: путь авто-подключения
возвращал успех без соединения, очистка жизненного цикла считала живыми все
вкладки, а CLI рисовал закрашенную точку для каждой.

`connected()` не вызывался нигде — именно поэтому баг оставался невидимым: не
было TypeError, был лишь boolean, который никогда не бывал False. Превращение в
`@property` чинит все шесть мест сразу; перед этим дерево проверено на вызовы
`connected()`, которые бы сломались.

Кроме того: три обработчика аннотированы `-> web.Response`, возвращая
`web.FileResponse`, который является `StreamResponse` и **не** наследует
`Response`. aiohttp его принимает — неверна была именно аннотация.

pyrefly **265 -> 263**.

### Десятый баг: `agentctl chat` падал при каждом запуске

Сортировка находок по признаку «этот вызов не может выполниться», а не по
количеству, вывела наверх `bad-argument-count` / `unexpected-keyword` /
`missing-argument` — ту же форму, что раньше вскрыла баг колбэка. 61 находка,
и первая же оказалась настоящей:

    repl(args.session)          # repl(session_path: Path, mode: str)

Два дефекта в одной строке: `mode` не передавался вовсе, а `--session` — это
строка-слаг, тогда как REPL ждёт Path, который создаёт `open_session()`. Любой
запуск `scripts/chat.py` завершался TypeError. Теперь есть `--mode` с
умолчанием `safe`, а слаг проходит через `open_session()`; проверено запуском
CLI до чистого `/exit`.

### Один неверный алиас типа — 28 находок

`DesktopExec = Callable[[str, float], Awaitable[...]]` делает `timeout`
позиционным и обязательным, тогда как все 35 мест вызова передают его по имени
— и каждое отчитывалось дважды о коде, который вызывается правильно. Заменено
на `Protocol`, чей `__call__` совпадает с настоящим `_desktop_exec(cmd, timeout=10)`.

Кроме того, `zerotier.py` достиг нуля: словарю `payload` нужна аннотация (тип
значения выводился по первым строковым записям, из-за чего последующее
присваивание списка читалось как индексирование `str`), а построитель argv
теперь сужает Optional `network_id`, который guard этой же функции уже отверг.

pyrefly **295 -> 265**, а группа «не может выполниться» — с 61 до 38.

### `assert` там, где guard уже это доказал

pyrefly не сужает union кортежей при распаковке, поэтому после

    body, jerr = await parse_json_body(request, ctx)
    if jerr is not None:
        return jerr

каждое последующее `body.get(...)` читается как обращение к атрибуту None.
Ранее в этом цикле это было задокументировано и оставлено как есть, потому что
альтернативой было переписать ~50 обработчиков на индексирование кортежа вместо
распаковки.

Есть третий вариант ценой в одну строку и без потери читаемости: `assert body
is not None` — утверждение того, что guard выше и так гарантирует. Применено к
12 сайтам с однозначной формой. `-O` нигде не используется, а даже под ним
поведение не меняется: guard возвращает раньше.

pyrefly **380 -> 311**.

### Платформенные и опциональные заглушки больше не отравляют вызывающий код

Та же корневая причина, что и в фиксе aiohttp, ещё в трёх местах. `user32 = None`
в не-Windows ветке навсегда делает имя `Module | None`, поэтому каждое
`user32.GetForegroundWindow(...)` читается как обращение к атрибуту None — 59
находок только в `desktop/backends/windows.py`, и ни одной настоящей: модуль
достижим лишь за `_IS_WINDOWS`, а у `ctypes.windll` всё равно нет полезного
статического типа.

* `_win32_api.py`: `user32` / `gdi32` / `kernel32` / `dwmapi` /
  `EnumWindowsProc` объявлены `Any` до платформенной ветки.
  **windows.py 61 -> 1.**
* `input_helper/helper_server.py`: та же однострочная форма.
* `observability/live_metrics.py`: потребовалась другая форма — `import psutil`
  связывает тип модуля, а fallback связывает None, и объявление имени `Any`
  заранее это не перебивает. Импорт под приватным именем с последующим
  `psutil: Any` — перебивает. Проверено вызовом `live_metrics_snapshot()`,
  который по-прежнему возвращает настоящие данные о CPU.

pyrefly **558 -> 485**.

### Восьмой и девятый баги — в функции, которая должна была сообщать о багах

Объявление интерфейсов CDP-миксинов позволило pyrefly увидеть форму вызова,
которую он раньше не мог разрешить, — и он указал на
`CDPTabManagerCallbackMixin._log_callback_error`, обработчик, передаваемый в
`Task.add_done_callback` из трёх мест. Два независимых дефекта в четырёх
строках, оба подтверждены исполнением:

* Это был обычный метод, первый параметр которого называется `task`. При
  обращении `self._log_callback_error` он связывается, поэтому `self` занимал
  слот задачи, а настоящему Task места не оставалось: каждый вызов бросал
  TypeError, который asyncio проглатывал в обработчик исключений цикла. Теперь
  `@staticmethod`.
* `Task.exception()` **возвращает** исключение и бросает лишь для отменённой
  или незавершённой задачи. Поэтому `try: task.exception() / except Exception`
  никогда не входил в обработчик — и даже после починки связывания в лог
  по-прежнему ничего не писалось. Теперь проверяется возвращённое значение.

Итог: ошибки фоновых CDP-задач не логировались никогда — той самой функцией,
для которой это было единственной работой.
`tests/test_cdp_callback_error_logging.py` проверяет, что сообщение доходит до
лога, а не что «ничего не упало»: «ничего не упало» и было сломанным состоянием.

### Интерфейсы миксинов записаны явно

`arena/browser/cdp_client/` построен на миксинах, которые свободно читают
`self._tabs`, `self.port`, `self._browser_ws` — состояние, принадлежащее
конкретному классу и не объявленное нигде. 251 из 520 находок
`missing-attribute` были этим единственным упущением, и именно этот шум позволил
багу колбэка остаться незамеченным.

`scripts/mixin_interface_decls.py` записывает контракт в каждый миксин под
`TYPE_CHECKING` (только аннотации, рантайм не тронут). Типы не выдумываются:
каждый атрибут разрешается из конкретного класса, который его определяет, —
настоящая аннотация, аннотированное присваивание в `__init__`, аннотированный
параметр или однозначный литерал. Подошли 56 атрибутов в 13 миксинах; остальное
пропущено и поимённо названо, потому что приблизительная заглушка хуже её отсутствия.

pyrefly **742 -> 558**, `invalid-annotation` достиг нуля.

## v4.155.0 — 2026-08-03

**Шесть живых багов, найденных при выплате lint-долга.** Долг ruff прошёл путь
1980 -> 35, типовой 1035 -> 769, но суть не в цифре: `from X import *` никогда
не связывает имена с подчёркиванием, поэтому несколько мест годами ссылались в
пустоту — в путях, чьи исключения проглатывались. WebSocket push отправлял ноль
байт, рапортуя об успехе. WS-сервер не мог принять соединение. `skill run` падал,
не сделав ничего. Дыра path-injection в standalone MCP-диспетчере позволяла
прочитать `/etc/hostname` одной строкой. Ничего из этого не было видно, пока
имена не стали разрешимыми.

Кроме того: вендорский namespace `mumu.*` удалён и заменён провайдер-агностичным
`emulator.*` на основе таблицы данных.

### `mumu.*` удалён: управление эмуляторами стало провайдер-агностичным

В мосту жили восемь MCP-инструментов, приваренных к одному Android-эмулятору
(MuMu Player), к одной ОС (Windows), с домашним каталогом конкретного оператора
в значении аргумента по умолчанию — при том что skill `core/build-capability`
называл именно этот namespace своим контрпримером. Противоречие закрыто делом,
а не комментарием.

- **Новый namespace `emulator.*`, пять инструментов**: `emulator.providers`
  (какими менеджерами этот хост реально может управлять), `emulator.list`,
  `emulator.start`, `emulator.stop`, `emulator.attach`. Ни одного вендора в
  именах инструментов.
- **Провайдеры — данные, а не код** (`arena/emulator/providers.py`): AVD,
  Genymotion, MuMu и Waydroid — строки таблицы с путями к CLI и шаблонами argv.
  Добавить менеджер = добавить строку; MuMu теперь один из четырёх.
- **Хост может объявить свой провайдер, не трогая репозиторий** — через JSON в
  переменной `ARENA_EMULATOR_PROVIDERS`. Запись с совпадающим id заменяет
  встроенную, так что неверный well-known путь чинится без релиза. Битый конфиг
  игнорируется, а не роняет мост.
- **Дисциплина границы**: загрузка специфична для провайдера, всё после
  загрузки — обычный ADB. Старый namespace переизобретал adb
  shell/screenshot/devices, которые `mobile.*` уже делал кроссплатформенно; это
  дублирование удалено, а `emulator.attach` существует ровно для того, чтобы
  вернуть вызывающего в `mobile.*`.
- **Нигде нет shell**: каждый вызов — список argv из статического шаблона,
  поверхность квотирования вендорского CLI исчезла. Битый env-пин теперь
  *сообщается* (`broken_pin` + подсказка), а не выглядит как «не установлено».
- **Провайдер без глагола остановки так и говорит.** У AOSP-эмулятора его нет;
  `emulator.stop` возвращает `unsupported_operation` с прямым указанием
  ADB-пути вместо угадывания kill'а процесса.
- Новый `docs/emulators.md`; `core/build-capability` теперь учит приёму (найди
  категорию, вынеси вендора в конфигурацию, остальное отправь в уже
  существующие примитивы), а не просто признаётся в ошибке.

### hwinfo: внутренний worst case превышал внешний бюджет

`hwinfo.py --full` выполняет ~10 PowerShell-запросов `Get-CimInstance`, у
каждого таймаут 30 с, тогда как вызывающий тест давал 30 с на весь процесс.
Внешний бюджет меньше внутреннего worst case падает не громко, а на том
раннере, которому не повезло с нагрузкой, и выглядит как флейк. Он покраснел
дважды на windows-latest, прежде чем причину нашли.

- Таймаут одного PowerShell-вызова 30 с -> 8 с (`PS_TIMEOUT_S`).
- Новый общий бюджет прохода (`PS_PASS_BUDGET_S`, 20 с): после исчерпания
  оставшиеся запросы сразу возвращают пусто, а не копят таймауты, поэтому
  зависшая служба WMI даёт частичный, но быстрый ответ вместо подвисания.
- Новые ворота `tests/test_hwinfo_timeout_budget.py` фиксируют *соотношение*, а
  не константы: падают, если бюджет прохода перерастёт бюджет вызывающего, если
  таблица вызывающих разойдётся с их реальным исходником или если call site
  захардкодит завышенный таймаут. Каждые проверены саботажем — пять из пяти.

### Бюджеты `node -e`: одно число вместо пяти

Та же форма отказа, что и в hwinfo, только в тестовом сьюте. Пять модулей
копипастой держали каждый свой таймаут для Node (10 с или 15 с), подобранный на
прогретом Linux-раннере. На холодном windows-latest первый старт Node
оплачивает создание процесса, проверку Defender и многокилобайтный скрипт в
stdin; пара с 10 с покраснела дважды и оба раза выглядела как флейк.

- Новый `tests/_node_budget.py`: единый бюджет (60 с на Windows, 30 с на
  остальных), переопределяемый через `ARENA_TEST_NODE_TIMEOUT` — причём
  переопределение может только *повысить* его, потому что заниженный бюджет и
  есть устраняемая ошибка.
- Новые ворота `tests/test_node_budget.py` падают, если какой-либо
  node-модуль снова заведёт литеральный таймаут, если новый модуль начнёт
  вызывать `node -e` мимо списка или если override начнёт опускать планку.
  Проверено четырьмя саботажами.

### E701 устранён: 193 инлайновых составных оператора, доказано на уровне байт-кода

`if x: a; b` — тот слой, который `scripts/e702_split_statements.py` осознанно
не трогал: это не разделение по точке с запятой, а решение об отступах, и
ошибка здесь молча выносит операторы *из* ветки. Новый
`scripts/e701_split_compounds.py` делает это механически в 35 файлах.

- **Долг 1980 -> 1772.** E701 193 -> **0**, попутно E702 127 -> 112: инлайновые
  тела раньше блокировали сплиттер точек с запятой.
- **Аргумент безопасности — не «тесты прошли».** Файл принимается, только если
  AST идентичен до и после. Затем отдельно сверен скомпилированный байт-код
  всех **377 функций**, инструкция за инструкцией — 0 реальных различий после
  нормализации трёх артефактов CPython, каждый из которых сначала проверен на
  минимальном примере: `__firstlineno__` (метаданные класса в 3.13, зависящие
  от номера строки), NOP-заполнение для трассировки строк и склейка
  суперинструкций (`STORE_FAST_LOAD_FAST` — это ровно `STORE_FAST` +
  `LOAD_FAST`, склеиваемые только при смежности).
- Затем переписанные форматтеры были исполнены и их реальный вывод проверен:
  равенство байт-кода всё ещё ничего не говорит о том, был ли код верен изначально.
- Новые ворота `tests/test_e701_splitter.py` фиксируют пол на **нуле** E701
  (ratchet видит только общую сумму 1772, поэтому правило могло бы вернуться
  по одной строке) и покрывают опасные случаи: `else: a; b` с сохранением обоих
  операторов внутри ветки, двоеточия словарей и срезов, отступы табами,
  сохранение хвостовых `# nosec`, и depth-0 `if lambda: 1: go()` без скобок —
  легальный Python, где двоеточие lambda идёт перед заголовочным.

Два саботажа сначала прошли, и это само по себе находка: один использовал
lambda в скобках (её и так защищала глубина), другой — селектор `-k`, не
совпавший ни с одним тестом, так что pytest отчитался успехом по нулю тестов.
Оба переписаны; все семь саботажей теперь падают как задумано.

### Path-injection в standalone MCP-диспетчере (найдено при чистке E701)

`arena/mcp/standalone_tools.py` обслуживает отдельные HTTP- и
WebSocket-серверы MCP. Он не проходит через `arena/mcp/tool_fs.py`, поэтому и
не унаследовал его path-jail: `fs.read` / `fs.write` / `fs.list` брали путь от
вызывающего, применяли `expanduser` и открывали файл. Прочитать
`/etc/hostname` — одна строка, `~/../../etc/hostname` тоже работал.

**Это была настоящая дыра, а не артефакт сканера — подтверждено исполнением.**
И она была старой и молчаливой. CodeQL выдал py/path-injection только после
того, как разделение E701 вынесло `open()` с общей инлайновой строки и поток
данных стал виден сканеру. Молчание сканера никогда не было доказательством
безопасности — оно было доказательством строки, которую он не смог прочитать.

- `_jail()` повторяет правила главного диспетчера: чувствительные имена
  отвергаются, путь резолвится до проверки (чтобы `..` и симлинки не убегали),
  всё вне `$HOME` отклоняется через `PathJailError` и fail-closed `isError`.
- `tests/test_standalone_path_jail.py` (29 тестов) исполняет диспетчер на
  враждебных путях, а не инспектирует его, и проверяет, что содержимое секрета
  не появляется в ответе — а не просто что вернулась ошибка.

Написание самих ворот дало две находки. Тест на чувствительные имена сначала
использовал несуществующий файл, и `FileNotFoundError` удовлетворял его по
неверной причине; теперь файл создаётся в фальшивом home. А саботаж симлинка
сперва проходил, потому что `resolve()` есть в двух местах — побег пришлось
отключить в обоих, и только тогда тест упал, как и должен.

### Мелкий lint-долг закрыт: W291, W293, I001, E402, E741 — по нулям

В baseline осталось всего два правила. 1770 -> 1721, и каждое из 49 нарушений
было решено осознанно, а не автофиксом.

- **W291 (5)** — не исправлено, а *обосновано*. Все пять внутри `_SENSOR_DUMP`,
  дословного вывода `adb shell dumpsys sensorservice`, где устройство дополняет
  каждую строку события хвостовым пробелом. Измерено: удаление не меняет
  результат обоих парсеров, то есть пробелы не несущие — но ценность фикстуры
  именно в побайтовом совпадении с тем, что напечатал реальный телефон.
  Зафиксировано per-file ignore с этим обоснованием.
- **W293 (18)** — все внутри докстрингов. Доказано нормализацией всех
  докстринг-констант: AST идентичен в 9/9 файлах, из диффа удалены ровно 18
  пробельных строк, кода не добавлено.
- **E402 (6)** — пять подняты наверх после проверки каждого на цикл. Шестой
  поднять нельзя: `auto_update_windows` импортирует `_REPLACE_TARGETS` обратно
  из `auto_update`, и подъём даёт ImportError при инициализации. Подтверждено
  выполнением подъёма. `tests/test_e402_deliberate.py` повторяет этот подъём в
  копии на каждом прогоне, так что `# noqa` — проверяемое утверждение, а не
  комментарий; а если цикл когда-нибудь исчезнет, тест сообщит, что noqa можно
  снимать.
- **E741 (19)** — переименовано через `scripts/e741_rename_ambiguous.py`,
  который ведёт переименование от AST (только биндинги comprehension/цикла,
  правка Name-узлов по их собственной позиции, отказ при повторном связывании
  имени или отсутствии свободной замены). 16 автоматически, 3 генератора в
  f-строках вручную. Доказано нормализацией старого и нового имён в один токен:
  10/10 файлов идентичны с точностью до переименования.

Две находки по ходу. Переименователь сначала отверг строку вместо правки,
потому что `ast.col_offset` считает **байты UTF-8**, а инструмент резал `str` —
расходятся они только на не-ASCII строках, и ровно одна такая здесь есть
(русское слово внутри регекспа). Ворот поймал это вместо того, чтобы всадить
переименование в середину `re.I`. Отдельно: комментарии-обоснования раздули
`auto_update.py` до 602 строк и уронили ворот 600-строчного лимита — того
самого, из-за которого этот файл когда-то и разделили.

### E702 закрыт: guard отказывал ровно там, где должен был разрешать

Сплиттер точек с запятой два релиза стоял на 112 строках, объявляя их все
«небезопасными». Ошибка была в guard'е, а не в коде: он считал inline-suite
*любые* два оператора одной сюиты на одной строке — включая обычное
`home = tmp_path / "h"; home.mkdir()` в теле функции. Сужение до реально
опасного случая (сюита на одной строке со **своим заголовком**) освободило 110;
последние две — многострочный литерал, разделены вручную.

Долг **1721 -> 1609. В baseline осталось одно правило — F405.**

Сужение тут же породило настоящую дыру, и её обнаружение — самое ценное:
у `else` / `except` / `finally` нет собственного `lineno`, поэтому проверка
«та же строка, что у владельца» молча пропускала `else: a; b`, и инструмент
вынес оператор из ветки else. Поймано тем, что саботаж-тест был написан до
того, как я поверил правке; теперь guard дополнительно смотрит на ключевое
слово в начале физической строки.

Проверено так же, как E701: AST идентичен в 28/28 файлах, затем сверен
байт-код **384 функций** — 0 различий после нормализации NOP-заполнения,
склейки суперинструкций и перенумерации переходов. Сам нормализатор пришлось
чинить по ходу: он резолвил цель прыжка в *имя* инструкции, а оно меняется при
расщеплении склеенной пары, — теперь цель резолвится в индекс в полностью
развёрнутом потоке. После этого переписанные CLI-модули были исполнены.

### Четыре тихих рантайм-бага, найденных при удалении star-импортов (F405 433 -> 270)

Замена `import *` на явные имена перестала быть работой по линту на середине
пути. Star-импорт **никогда не связывает имена с подчёркиванием**, и не
связывает имена, которых в модуле-бандле вообще нет, — так что несколько мест
годами ссылались в пустоту, в путях, чьи ошибки проглатывались.

* **WebSocket push не доставлял ничего.** `ws_push._broadcast` вызывал
  несвязанный `_send_text`. NameError ловился `except Exception` на каждого
  подписчика, поэтому клиент попадал в `dead` и молча отписывался при первом
  же уведомлении. Функция рапортовала успех и отправляла ноль байт. Доказано
  исполнением `_broadcast` с фейковым сокетом: было 0 байт и подписчик
  выброшен, стало — кадр отправлен и подписчик жив.
* **WebSocket-сервер не мог принять соединение.** `ws_client._client_loop`
  вызывал несвязанные `_http_handshake`, `_recv_frame`, `_send_frame` — цикл
  падал на первой же строке.
* **Watcher уведомлений крутился вхолостую.** `_time.sleep(0.5)` / `(2)` —
  опечатка вместо `time` — давала NameError внутри обработчика повторов, так
  что задержка не срабатывала и поток жёг CPU.
* **`desktop shot` и `cdp close` были мёртвыми командами.** `shq` (экранирование
  пути скриншота) и `close_tab` не были связаны — оба падали с NameError.
  В `/health` у `standalone_http` вызывался `len(TOOLS)` с несвязанным `TOOLS`.

Существующий тест **описывал** первый баг в комментарии и обходил его —
подставлял `_send_text`, только если тот отсутствует, — вместо того чтобы
сообщить о нём. Этот обход молча превратился в no-op, как только импорт
починили, и тест перестал что-либо измерять; теперь подстановка безусловная.
`tests/test_ws_push_star_import_regression.py` проверяет доставку (байты в
сокете, подписчик остался), а не отсутствие исключений, — потому что прогон
без исключений и был сломанным поведением.

### Star-импорты выведены из обращения: F405 1609 -> 35, и пятый мёртвый путь

Оставшиеся bundle-импорты разобраны вручную там, где инструмент отказывался, и
каждый отказ оказался находкой, а не ограничением инструмента:

* **`skill run` был сломан целиком.** `arena/skills/cli_run.py` вызывал
  `_fire_hook` на *первой строке* `run_skill()`, доставая его через
  `from cli_common import *`, который никогда не связывает имена с
  подчёркиванием. Каждый вызов падал с NameError, не сделав ничего.
  Подтверждено вызовом (NameError), затем повторным вызовом после починки
  (код 2, «skill not found» — то есть дошло до поиска).
* **Stdlib, отмытый через соседей.** `recall_cli`, `agent_helpers/cli` и
  `missions_cli/commands` импортировали `argparse`, `os`, `json`, `textwrap`
  *из соседнего модуля arena*, а не из stdlib, — из-за чего у этих соседей и
  возникал видимый «контракт ре-экспорта». Исправлено на стороне потребителя,
  после чего контракт испарился.
* **Импорты только для аннотаций** (`CDPBrowser`, `CDPTab` в пяти файлах)
  переведены в `TYPE_CHECKING`, кроме `tab_connection.py`, который реально
  инстанцирует `CDPBrowser` в рантайме, — там сначала проверено отсутствие
  цикла, и импорт оставлен обычным.

Инструмент обзавёлся двумя воротами из собственных ошибок: он считал локальные
для функций связывания модульными (терял нужный импорт — поймано запуском ruff
по собственному выводу) и считал, что имена, связанные *только* внутри функций,
требуют модульного импорта (менял F405 на F401 — поймано трижды подряд).

`tests/test_star_import_policy.py` закрывает дверь: остались четыре
star-импорта, каждый — явный фасад ре-экспорта с письменным обоснованием, пол
F405 зафиксирован на 35. Ворота падают и если число *уменьшилось*, а пол не
опустили, — именно так пол в этом коммите прошёл путь с 58 до 35.

### Сканеры наконец увидели код — и им было что сказать

Удаление star-импортов позволило трём CI-сканерам разрешать `subprocess`,
`CDPBrowser` и прочее. Всё, о чём они сообщили, было верно годами — анализатор
просто не мог проследить имя через `import *`.

* **bandit: 9 MEDIUM (B604).** Семь существовали до этой работы — джоб был
  красным, а находки недостижимы. Пять — константные строки команд без единой
  подстановки; остальные уже несли обоснованный `# nosec B602`, не хватало
  идентификатора соседнего правила. Одна была настоящей ошибкой: в
  `arena/desktop/cli/input.py` подавление стояло на строке `time.sleep`, на
  строку ниже того `Popen`, который должно было закрывать, — и не закрывало
  ничего. Перенесено на сам вызов. **Теперь 0 MEDIUM/HIGH.**
* **semgrep: 2 находки** — та же промахнувшаяся аннотация.
* **pyrefly: `missing-attribute` +1 нетто, 34 новых в одном файле.** Это не
  баги, а необъявленный интерфейс. `CDPTabConnectionMixin` читает `target_id`,
  `ws_url`, `_browser` и вызывает `get_title` / `get_current_url`, которые даёт
  класс-наследник. Объявлено под `TYPE_CHECKING`: контракт зафиксирован,
  рантайм не тронут. Первая версия заглушек была неверной (синхронные, без
  аргумента timeout) — и checker сразу сказал об этом двумя `not-async` и двумя
  `inconsistent-inheritance`; сигнатуры теперь дословно совпадают с `tab_ops.py`.

Итог по типовому долгу: **pyrefly 905 -> 843**, причём `unknown-name` ушёл с 36
до 0, потому что у имён появилось разрешимое происхождение. Quality-baseline
перегенерирован при локально установленном pyrefly — прежний «0» в этой
песочнице был артефактом отсутствия инструмента, теперь это реальное измерение.

### Поля dataclass-контейнеров хендлеров получили настоящий тип

Пятьдесят `@dataclass`-контейнеров объявляли поля как `object`, храня в них
корутины-обработчики aiohttp — это не описание, а его отсутствие. Checker не мог
проверить ни один вызов через них, и отсюда шёл основной объём `bad-assignment`
(101 только в `arena/wiring/platform.py`).

`scripts/typed_handler_fields.py` меняет тип на `Callable[..., Any]`, но только
там, где каждое место конструирования во всём репозитории передаёт вызываемое.
Подошли 255 из 285; оставшиеся 30 нигде не создаются через именованный аргумент,
поэтому инструмент отказался гадать и назвал каждое.

Один тест проверял литерал `"update_token_set: object"` — он фиксировал
*отсутствие* типа и заблокировал бы ровно это улучшение. Теперь он проверяет
наличие поля, а не его аннотацию.

pyrefly 843 -> 769.

### Верификация
Четырнадцать новых тестов, четверо из них сначала проверены намеренным
саботажем: возврат имени `mumu.*`, захардкоженный домашний каталог оператора,
`shell=True` в слое control и прореживание таблицы провайдеров до одного
менеджера на ОС — каждый саботаж был пойман своими воротами и откачен.
Зелёное по-прежнему не означает, что эмулятор загружается: ни один CI-раннер
его не имеет, и в `docs/emulators.md` это написано прямо.

## v4.154.0 - Ворота качества, которые падают закрыто, и наконец-то работающее покрытие

### Доктрина верификации
- **В AGENTS.md прямо записано: ЗЕЛЁНОЕ != РАБОТАЕТ.** Пройденный сьют доказывает
  только то, что тесты прошли; уверенность даёт исполнение настоящего артефакта.
  Каждые ворота ниже сначала были намеренно сломаны, чтобы убедиться, что они ловят.

### Новые блокирующие ворота
- **Guard всей auth-поверхности** (`tests/test_auth_surface_guard.py`): поднимает
  реальное приложение, обходит все 510 зарегистрированных маршрутов, дёргает каждый
  без токена и требует отказ. Аутентификация вызывается внутри каждого handler'а, а
  не middleware, поэтому забытый `require_auth` в новом обработчике был невидим — и
  ни один SAST такое не поймает, это инвариант продукта, а не паттерн кода. Два
  маршрута отвечают открыто по замыслу (`/v1/version`, манифест ассетов дашборда) и
  внесены в allowlist с обоснованием. Доказано подсунутым незащищённым маршрутом:
  guard падает и называет его.
- **E2E установленного артефакта** (`e2e-installed`): собирает wheel, ставит в
  изолированное hash-locked окружение, запускает как живой сервер и проверяет health,
  отказ без токена, полный MCP-коннект, round-trip `fs.write` -> `fs.read` и
  path-jail — из нейтральной рабочей директории.
- **Packaging E2E**: `build` -> `twine check` -> `check-wheel-contents` -> установка
  в чистое окружение -> импорт. Закрывает класс «`pip install` сломан, а CI зелёный».
- **Свежесть lock-файлов** (`scripts/check_lock_freshness.py`): каждая зависимость,
  объявленная в `.in`, должна быть запинена той же версией в `.lock`, и у каждого
  пина должен быть хэш. CI ставит только из lock и `.in` не читает, поэтому
  незарегенерированный lock оставался невидим до ImportError в проде.
- **Контракты import-linter**: браузерная и десктопная автоматизация независимы;
  продакшн-код никогда не импортирует тесты.

### Ratchet-ворота долга
- **Lint-ratchet**: счётчики по каждому правилу заморожены в
  `scripts/lint_baseline.json`, любой рост красит CI. Долг снижен 3313 -> 1980 тремя
  слоями: механические безопасные фиксы, F401-проход с анализом реэкспортов (274
  импорта удалено, 90 оставлено с письменным обоснованием), F841 26 -> 0 и 189
  разделённых E702 — инструментом, который отклоняет любую правку, меняющую AST.
- **Quality-ratchet**: vulture (0) и pyrefly (1035, по видам ошибок) заморожены так же
  и печатают полную диагностику при росте.
- **Job debt visibility**: намеренно красный, пока долг больше нуля, и неблокирующий.
  Ratchet по построению зелёный при любом размере бэклога, а ИИ читает именно
  галочку — теперь у бэклога есть собственный сигнал.

### Безопасность и цепочка поставок
- **Harden-Runner** (режим audit) первым шагом во всех 29 CI-джобах.
- **requests 2.33.0** (CVE-2026-25645), hash-locked окружения для линта и упаковки.
- **Code scanning**: исправлен настоящий top-level `contents: write` в badge-workflow;
  сопутствующие сообщения о path-injection проверены вручную (resolve, затем
  containment, с учётом симлинков) и закрыты с письменным обоснованием.
- **Опциональные pre-commit хуки**: детект приватных ключей, валидация YAML/TOML и
  ruff из закреплённого окружения. Только удобство; источник истины — CI.

### Покрытие
- **Codecov снова принимает отчёты**, 52.6%. Их CLI ищет файлы отчётов по всему
  дереву и подхватывал закоммиченную тестовую фикстуру с именем `coverage.xml`
  девятидневной давности; воркер отбраковывает отчёты старше 12 часов и валил из-за
  неё каждую загрузку. Фикстура переименована, артефакт убран из индекса, загрузка
  сделана герметичной через `disable_search`.

### Тесты
- 4778 тестов, прогон со случайным порядком и таймаутами на тест.
...
### Безопасность
- **aiohttp >= 3.14.1**: закреплена минимальная версия, закрыто 62 CVE (2 critical).
- **idna >= 3.15**: закреплена минимальная версия, закрыто 2 CVE (влито из CodeRabbit PR #2).

### CI
- **pip cache**: добавлен `cache: pip` во все 17 шагов `setup-python` в `ci.yml` и `security-scan.yml`, сокращение времени CI-установки на ~30с на каждую job.

### Документация
- **README.ru.md**: полная синхронизация с README.md — добавлены 3 отсутствующие секции (Путеводная звезда, Миссия: бортовой компьютер, Текущий статус полёта v4.140.x), добавлены 18 отсутствующих tool namespace'ов в справочник, заголовки секций переведены последовательно, содержание обновлено.

### Прочее
- Удалено расширение VSCodium из репо (тонкий JSON-вьюер без самостоятельной ценности; план сохранён в docs).
...
### Исправлено
- Добавлено явное Semgrep-обоснование для loopback-only MCP dispatch в Mission Autopilot (`127.0.0.1:<bridge-port>/mcp`). Этот вызов намеренно локальный к тому же bridge-процессу и не принимает внешние URL.

## v4.145.0 - Mission Autopilot

### Добавлено
- Добавлены MCP-тулы `mission.autopilot_start`, `mission.autopilot_status`, `mission.autopilot_report` и `mission.autopilot_list`.
- Mission Autopilot выполняет bounded tool chain из цели и явных шагов (или safe default ship checklist), сохраняет progress в `ARENA_AGENT_HOME/autopilot/runs` и может автоматически создать scenario shell плюс `scenario.record` flight record.

### Заметки
- Это первый маленький автономный flight loop: ещё не LLM-планировщик, но уже durable executor, который превращает длинные ручные цепочки в run со status/report/flight-record artifacts.
...
### Исправлено
- `desktop_app.click_window_relative` теперь фокусирует/восстанавливает найденное окно и затем заново читает его геометрию перед переводом относительных координат в абсолютные. Живая проверка на Windows с Cheat Engine показала, что свернутые/восстановленные окна сначала могут быть видны в parking-координатах вроде `(-21333, -21333)`; по такой устаревшей геометрии клик уходит за экран.
- `desktop_app.screenshot_window` тоже фокусирует/восстанавливает окно и заново resolve-ит геометрию перед crop, чтобы оконный скриншот не снимал parking-прямоугольник свернутого окна.
...
### Исправлено
- `GET /v1/autonomy/posture` теперь отдаёт `ok: true`. Refresh карточки кубиков
  в Settings guarded по `ok`, которого не было, поэтому выходил раньше времени и
  не строил селекты по осям — карточка выглядела пустой, а кнопки пресетов /
  Apply будто ничего не делали. Клиентский гард теперь ключуется по `axes` (то,
  что он реально рендерит), а эндпоинт отдаёт `ok` для согласованности с
  остальными. Добавлен регресс-тест.

## v4.103.0 - Settings-UI для постурy выполнения («кубики»)

### Добавлено
- Карточка в Settings с кубиками постурy: селекты по осям (sandbox / network /
  privilege / filesystem / runtime), заполняемые с сервера, кнопки пресетов
  (strict / balanced / fenced-yolo / naked), живой бейдж риска, красная панель
  «эта postura оголяет: ...» для рискованных комбинаций, ack-чекбокс + confirm
  для high/critical. Apply мерджит выбранные оси с сохранёнными
  runtimes/resources (UI их в этом срезе не правит) и POST'ит в
  `/v1/autonomy/posture`; сервер пере-валидирует и требует ack-фразу.
- `GET /v1/autonomy/posture` теперь также отдаёт допустимые значения осей и
  ack-фразы, чтобы UI не хардкодил их (без дрейфа). Клиентский превью риска
  зеркалит серверную формулу только для живой подсказки; безопасность от него
  *не* зависит (сервер пере-валидирует на Apply).

### Замечания
- Агент по-прежнему не может читать или менять постурy (сеттер master-only +
  блок-лист чувствительных файлов). Клиентский превью риска — совет; сервер —
  инстанция истины.
- Карточка честно отмечает, что на Windows fenced-выполнение кода отказывается
  до более позднего среза (там сейчас бежит только экстремум `off`).

## v4.102.0 - Компонуемая postura выполнения («кубики») + fail-closed code.run

### Цель
Оператор компонует управления по осям — «кубики»: sandbox / network / privilege
/ filesystem / runtime — которые определяют, как выполняется код, сочинённый
агентом; агент не может их ни читать, ни менять. Это первый срез направления
«произвольный код в YOLO», построенный fail-closed и честно относительно того,
что каждая платформа реально принуждает. YOLO убирает только шаг одобрения; он
никогда не убирает забор и не перебивает HALT.

### Добавлено
- `arena/autonomy/posture.py` — модель постурy: оси, пресеты
  (strict / balanced / fenced-yolo / naked), уровень риска, ack-фразы по риску,
  персист-конфиг.
- `arena/autonomy/runner.py` — fail-closed runner. Linux fenced -> строгий
  фикс-забор systemd-run (приватная сеть, сброшенные привилегии, запись только в
  scratch, кап памяти); Windows fenced -> **отказ в этом срезе** (AppContainer
  требует живой валидации гранта доступа к файлу, которую мы не повезём
  нетестированной); `sandbox=off` -> unfenced (помеченный экстремум, = прежний
  exec-риск). Секреты вычищаются из env, а wall-timeout + кап вывода действуют
  на *каждом* пути, включая unfenced.
- Тул `code.run` (dangerous; под YOLO): гонит код агента под активной постурой
  оператора и отвергает любые оси постурy в аргументах, так что агент не может
  расширить себе забор. Результат несёт честную карту `enforced`.
- Операторские эндпоинты `GET/POST /v1/autonomy/posture`; сеттер — только под
  мастер-токен (токены агентов отклоняются). Каталог `autonomy/` в блок-листе
  чувствительных файлов.
- YOLO перенесён в пакет `arena/autonomy` (теперь там же posture + runner) и
  реэкспортирован, чтобы прежние импорты продолжали работать.

### Тесты
Модель постурy (валидация, пресеты, риск, ack, персист, блок-лист); runner
`build_command` по платформам + fail-closed отказы + выполнение на off-пути +
allowlist рантаймов; отвержение переопределения осей в `code.run` + классификация
dangerous.

### Отложено (отслеживается)
Потенциальная гранулярность осей сверх строгого Linux-забора; грант доступа к
файлу для Windows AppContainer code-exec; микроВМ-изоляция; Settings-UI
«кубиков».

## v4.101.0 - Перенос /v1/mcp/* в MCP-слой (связность)

### Цель
Закрыть отмеченный ранее долг связности: эндпоинт мониторинга MCP-серверов
(`/v1/mcp/servers`) и эндпоинт управления авторскими тулами
(`/v1/mcp/custom/remove`) жили в *системном* слое хендлеров (рядом с
version/status/config/doctor) — неправильный дом для MCP-задач. Теперь они живут
в MCP-модуле хендлеров (`arena/mcp/handlers.py`) и зарегистрированы вместе с
MCP-транспортами (`/mcp`, `/sse`, `/ws`), так что MCP-слой владеет своей
HTTP-поверхностью целиком.

### Изменено
- Реализации хендлеров перенесены из `arena/system/handlers.py` в
  `arena/mcp/handlers.py`; `McpHandlerContext` получил нужный ему `executor`;
  маршруты перенесены из группы `core` в группу `compat` (MCP).
- Без изменения поведения: эндпоинты отвечают идентично (проверено вживую до и
  после). Чистый рефактор слоёв / связности.

## v4.100.0 - Проход укрепления: управление оператором само-построенной библиотекой + чистка flaky CI

### Цель
Дозреть то, что построили в v4.96–v4.99: сделать само-построенную агентом
библиотеку *управляемой* оператором прямо из cockpit, и перестать падать в CI на
тестах, зависимых от окружения. Чисто внутренний перенос `/v1/mcp/servers` в
отдельный MCP-домен маршрутов отложен в собственный аккуратный рефактор — он
невидим оператору и трогает boot-wiring, поэтому отслеживается как долг, а не
делается вслепую внаспех.

### Добавлено
- `POST /v1/mcp/custom/remove` — оператор может отозвать авторский инструмент
  прямо из Dashboard (авторизация токеном, без гейта одобрения: человек,
  управляющий библиотекой, и есть инстанция одобрения). Карточки авторских тулов
  в MCP-табе теперь также показывают свои шаги и кнопку Remove.

### Исправлено (надёжность CI)
- `test_probe_tcp_timeout_short` пропускается, когда сетевой стек хоста
  принимает коннект к неразмещаемому TEST-NET (permissive NAT / песочница, где
  путь таймаута ненаблюдаем), и принимает быстрый ICMP-error как валидный
  «prompt failure» на реальных сетях.
- `test_disabling_auto_refresh_clears_timer` получает тот же `win32`-skip, что
  уже несут соседние toolbar-timer тесты (Node child-process харнесс там
  зависает); логика по-прежнему крутится на Linux/macOS.

### Отложено (отслеживается)
- Перенести `/v1/mcp/servers` (и новый эндпоинт управления custom) из системного
  слоя в отдельный MCP-домен маршрутов.

## v4.99.0 - Библиотека авторских инструментов: авторские тулы переиспользуют авторские

### Цель
В v4.98 композит умел звать встроенные тулы. Теперь шаг или одиночный вызов
может целивать в *другой авторский инструмент* (`custom.<name>`), так что агент
строит переиспользуемые абстракции и компонует их — настоящую библиотеку, а не
только плоские макросы. Это следующая ступень само-расширяющейся среды.

### Добавлено
- Тулом шага / `call` может быть встроенный ИЛИ уже определённый авторский
  инструмент. Ссылки валидируются снизу-вверх (ссылаемый тул должен
  существовать), а граф ссылок держится ацикличным (проверка цикла при
  создании; порядок создания делает циклы невозможными кроме случая
  remove+recreate, который проверка ловит).
- Унаследованный риск обходит всё дерево ссылок (МАКСИМУМ), поэтому цепочка,
  трогающая dangerous-тул где угодно, сама dangerous и требует одобрения на
  внешнем вызове.
- Ограничение глубины рекурсии (`MAX_CUSTOM_DEPTH`) — рантайм-страховка, чтобы
  патологическое дерево ссылок никогда не ушло в бесконечность; каждый вложенный
  вызов по-прежнему проходит через `call_tool`, поэтому HALT и политика рисков
  работают на каждом уровне.

### Безопасность / совместимость
- Произвольного кода нет; каждый вложенный вызов рекурсит через обычный
  диспетчер.
- Одиночный вызов, композиция и все прежние сигнатуры не изменились.

### Тесты
Переиспользование библиотеки + проброс риска; запрет самоссылки / висячей
ссылки / цикла; реальный вложенный прогон; страж глубины рекурсии.
