## v4.98.0 - Компонуемые авторские инструменты (агент пишет многошаговые способности)

### Цель
В v4.96.0 агент умел создавать авторский инструмент, оборачивающий ОДИН вызов.
Реальные задачи — это пайплайны, поэтому авторский инструмент теперь может быть
*композицией*: упорядоченным списком шагов с потоком данных между ними. Это
следующая ступень само-расширяющейся среды — агент растит более богатые
способности, а не только тонкие алиасы.

### Добавлено
- `custom.create` принимает тело `steps` (наряду с существующим `call`):
  `[{id, tool, args}, ...]`. `args` шага могут ссылаться на входы инструмента
  (`{serial}`) и на результат любого предыдущего шага через точечный путь
  (`{steps.<id>.<field>}`). Композит возвращает `{ok, steps, step_ok}`, так что
  один авторский вызов может собрать несколько показаний сразу (напр. состояние
  PC + телефона).
- Шаги идут continue-on-error (один оффлайн-источник не прячет остальные), и
  каждый шаг рекурсит через `call_tool`, поэтому политика рисков и полный стоп
  агента (HALT) работают *внутри* композита — HALT блокирует прямо посередине
  пайплайна.
- Унаследованный риск композита = МАКСИМУМ по шагам (композит, трогающий
  dangerous-тул, сам dangerous и требует одобрения на внешнем вызове; внутренние
  шаги повторно не спрашивают, по аналогии с `scenario.run`).
- Карточка MCP в Dashboard показывает шаги композита.

### Безопасность
- Шаги могут использовать только встроенные тулы (рекурсии по custom/management
  нет); имя входа `steps` зарезервировано; id шагов валидируются. Обратная
  совместимость: одиночный `call` и все прежние сигнатуры не изменились.

### Тесты
Сьют композиции: точечная подстановка + поток данных, вывод риска как максимума,
continue-on-error, HALT посередине пайплайна через chokepoint, создание/персист
композита, все проверки ошибок, диспетч и обратная совместимость одиночного
вызова.

## v4.97.0 - Автономия оператора: полный стоп агента (HALT) + YOLO-режим; фикс раскладки cockpit

### Цель
Два ортогональных рычага управления агентом плюс фикс раскладки, который видел
наблюдатель. Бридж уже давал агенту действовать; теперь у человека есть
настоящая «большая красная кнопка», чтобы остановить его *везде*, и явный
тумблер «пустить в свободное плавание» — вместо старой паузы только десктопа,
которая оставляла не-десктопные действия работать.

### Добавлено
- **Полный стоп агента (HALT).** `POST /v1/control/halt` / `/v1/control/unhalt`
  и кнопка HALT / Resume-stop в Dashboard. При halte весь инструментарий агента
  блокируется в единой точке `call_tool` (покрывает MCP `tools/call` И
  `/v1/extension/execute`): работают только policy-`safe` (read-only) тулы; всё
  мутающее отклоняется fail-closed (`mcp.ext_call` тоже блокируется — он
  классифицирован safe, но мутирует внешнее состояние; unknown-тулы тоже).
  Десктопный гейт `@controlled` теперь чтит HALT, так что GUI-ввод тоже
  замирает. Прямая консоль оператора (Terminal / Mobile / прямой HTTP в
  Dashboard) намеренно НЕ гейтится: HALT стопит *агента*, а не человека,
  возвращающего машину себе. Модель угроз проверена: ни один `safe`-тул не
  умеет POST в плоскость управления (`browser.fetch` только GET), поэтому
  halted-агент не может сам себя unhalt'нуть. По умолчанию выключено; хард-стоп
  всегда сильнее YOLO.
- **YOLO-режим** (`GET`/`POST /v1/control/yolo` + тумблер в Settings с красным
  предупреждением «никто не несёт ответственности»). Пока включён,
  `/v1/extension/execute` авто-апрувит каждый тул независимо от риска
  (бриджевый `--dangerously-skip-permissions`), чтобы автономный цикл не ждал
  клика человека. Включение требует явный ack-токен; Dashboard дополнительно
  требует чекбокс ответственности + подтверждение. **Не персистится**: рестарт
  бриджа возвращает безопасный дефолт. Агент не может включить YOLO сам себе
  через шину тулов (такого тула нет; чтобы дотянуться до эндпоинта, нужен
  dangerous-вызов, который до YOLO требует одобрения человека).
- Вкладка Control показывает состояние HALT; вкладка Settings — YOLO.

### Исправлено
- **Сдвиг cockpit вправо / табы «через раз».** Лишний `</div>` в конце
  `body-15-settings.html` преждевременно закрывал `.main` шелла; поскольку
  загрузчик склеивает body-файлы в порядке имён, шесть численно последних табов
  (mobile, live, zerotier, proposals, transports, mcp) оказывались прямыми
  детьми `<body>`, и `body{display:flex}` раскладывал их как соседи справа от
  пустого растянутого `.main` — ровно те табы, про которые сообщали «съехало /
  через раз». Лишний close удалён; добавлен
  `tests/test_dashboard_tab_containment.py`, чтобы несбалансированный body-файл
  в будущем валил CI, а не сдвигал cockpit.
- **Консольная ошибка Mobile info** (`mobileInfoRememberOpenState is not
  defined`): `<details ontoggle>` теперь страхует коллбэк проверкой `typeof`.

### Тесты
30 тестов на логику решения kill-switch (read-only проходит; мутающие и
`mcp.ext_call` блокируются; unknown fail-closed; HALT перекрывает desktop-lease;
утечки глобального состояния между тестами нет) и 9 на YOLO (ack-гейт,
default-off, флаг в снапшоте политики, байпас одобрений в `execute_sync`).

## v4.96.0 - Само-расширяющаяся среда: агент сам создаёт себе инструменты (custom.create)

### Цель

Индустрия прошла путь: prompt engineering -> context engineering -> harness
engineering (строим всю операционную среду агента) -> фронтир
*динамической / самоэволюционирующей среды*, где агент уже не просто
пользуется готовой средой, а расширяет и создаёт её сам. Arena уже является
harness'ом (один рантайм объединяет сенсоры, актуаторы, подключаемые
MCP-подсистемы и политику рисков). Этот выпуск делает первый конкретный шаг по
последней ступени: агент теперь может сочинить новый инструмент во время
выполнения — способность, которой мгновение назад не существовало, — общим
способом и в рамках существующей модели безопасности. В README теперь записана
организующая миссия: бридж как бортовой компьютер, и куда движется
проектирование агентских сред.

### Добавлено

- **`custom.create` (medium)** — агент создаёт НОВЫЙ именованный инструмент:
  обёртку с JSON-схемой поверх ОДНОГО вызова встроенного инструмента, с
  подстановкой `{param}` из своей входной схемы в оборачиваемый вызов. Никакого
  произвольного кода: оборачиваемый вызов идёт через обычный диспетчер и
  политику. Custom-инструмент может оборачивать только встроенный инструмент
  (не другой custom — без цепочек композиции), а его риск НАСЛЕДУЕТСЯ от
  оборачиваемого (обёртка над dangerous-инструментом сама dangerous) — та же
  идея, что `derive_scenario_risk`. Хранится в `mcp/custom_tools.json`.
- **`custom.list` (safe)** / **`custom.remove` (medium)** — список / отзыв
  авторских инструментов.
- Авторские инструменты появляются в `tools/list` (статические инструменты
  управления зарегистрированы в каталоге; динамические авторские добавляются
  при выдаче списка) и вызываются через обычную шину инструментов.
- **MCP-таб Dashboard** теперь также показывает авторские custom-инструменты
  (имя, оборачиваемый инструмент, параметры, унаследованный риск) в секции
  «Custom tools».

### Исправлено

- **Раскладка активной вкладки Dashboard** — активная вкладка сохраняла
  `contain:layout style paint` при переходе на `content-visibility:auto`, из-за
  чего Chromium периодически откладывал/пропускал её рендер и держал заглушку
  800px: вкладки загружались «через раз», контент мог обрезаться/съезжать с
  пустым промежутком. Активная вкладка теперь рендерится как обычный блок
  (`content-visibility:visible; contain:none`); неактивные сохраняют дешёвый
  пропуск.
- **Дока `mobile.voice_record`** звала `asr.transcribe(path=<local>)`, а
  настоящий аргумент — `file=<local>`.

### Документация

- README **«Mission: the flight computer»** — бридж как бортовой компьютер
  корабля; траектория prompt -> context -> harness -> dynamic; ориентир для
  ИИ-сопровождающих (строить бортовой компьютер, а не кучу пультов).

### Тесты

20 unit-тестов на custom-инструменты (валидация схемы/аргументов, подстановка
`{param}`, CRUD + персистентность, наследование риска через реальную политику,
диспетч через оборачиваемый инструмент, управление); неймспейс управления
проходит guard'ы связности диспетча, dead-dispatch, сигнатур хендлеров и
покрытия документацией; весь сьют зелёный.

## v4.95.0 - MCP-юзабилити: добавление любого сервера, использование тулов без одобрения на каждый вызов, MCP-таб в Dashboard

### Цель

v4.94.0 добавил MCP-клиент, но перестраховался: `mcp.ext_call` был
`dangerous`, поэтому КАЖДЫЙ вызов внешнего тула требовал ручного одобрения —
агент не мог реально пользоваться установленным сервером. А сервера
добавлялись только через курируемый маркетплейс. Модель доверия теперь
корректна: доверие принимается при ДОБАВЛЕНИИ сервера; после добавления агент
свободно пользуется его тулами. Плюс таб в Dashboard для мониторинга
MCP-серверов.

### Изменено

- **`mcp.ext_call` переклассифицирован `dangerous` -> `safe`.** Добавление
  сервера — решение о доверии; одобрение на каждый вызов было непригодно для
  агента.

### Добавлено

- **`mcp.add` (medium)** — зарегистрировать ЛЮБОЙ внешний MCP-сервер в
  `mcp/mcp.json`: передать явный `command` (+ `args`/`env`) для любого
  сервера, напр.
  `{"name":"desktop-commander","command":"npx","args":["-y","@wonderwhy-er/desktop-commander"]}`,
  или только `name` для установки из реестра маркетплейса (git-venv сервера
  вроде `screenpilot` клонируются + venv автоматически).
- **`mcp.remove` (medium)** — убрать зарегистрированный сервер (и остановить).
- **Таб "MCP" в Dashboard** — мониторинг внешних MCP-серверов
  (зарегистрированные, статус, имена тулов) через новый эндпоинт
  `GET /v1/mcp/servers`, с reload + auto-refresh.

### Тесты

Покрыты реклассификация политики + `mcp.add`/`mcp.remove`; MCP-таб Dashboard
проходит guard'ы манифеста ассетов и no-hardcoded-theme-colors; реестр роутов
обновлён.

## v4.94.0 - MCP-клиент: использование ВНЕШНИХ MCP-серверов через мост

### Цель

Мост давно был MCP-*сервером* (отдавал свои тулы), а маркетплейс умел
*регистрировать* внешние серверы в `mcp/mcp.json` и зондировать их одноразово
(`tools/list`). Не хватало MCP-*клиента*, который держит сервер живым и реально
ВЫЗЫВАЕТ его тулы (`tools/call`) — чтобы агент мог использовать внешние
MCP-серверы (Desktop-Commander, ScreenPilot, официальные filesystem/fetch/git и
т.д.) через мост. Этот релиз закрывает дыру.

### Добавлено

- **`arena/mcp_client/`** — persistent stdio MCP-клиент:
  - `McpStdioClient`: спавнит один MCP-сервер, делает initialize-handshake,
    матчит ответы по JSON-RPC `id` (уведомления сервера пропускаются),
    поддерживает `tools/list` + `tools/call`.
  - `McpClientManager`: лениво стартует/кэширует один процесс на имя сервера из
    `mcp/mcp.json`, поэтому повторные вызовы переиспользуют соединение, а не
    спавнят `npx` каждый раз.
  - Allowlist команд (`npx`/`uvx`/`uv`/`python`/`python3`/`node` или полный
    путь до одного из них), чтобы подделанный конфиг не мог запустить
    произвольный бинарник.

- **Мостовые тулы `mcp.ext_*`**:
  - `mcp.ext_servers` (safe) — список зарегистрированных внешних серверов + статус.
  - `mcp.ext_tools` (safe) — подключиться и показать тулы сервера.
  - `mcp.ext_call` (dangerous) — вызвать тул внешнего сервера (выполняется на
    хосте моста с правами пользователя моста, поэтому требует одобрения).
  - `mcp.ext_stop` (medium) — остановить сервер.

- **Маркетплейс**: в реестр добавлены `desktop-commander` (npx, работает из
  коробки) и `screenpilot`. `screenpilot` — сервер типа `git-venv`:
  `marketplace install screenpilot` клонирует репо в `mcp/servers/`, создаёт
  venv, ставит его зависимости и регистрирует интерпретатор venv + entry-скрипт
  в `mcp.json` (идемпотентно).

### Тесты

`tests/test_mcp_client.py` прогоняет клиент end-to-end на in-process мок-MCP-
сервере (handshake, tools/list, tools/call, переиспользование процесса,
менеджер и хендлеры `mcp.ext_*`) плюс allowlist команд. Таблица неймспейсов
README документирует новый неймспейс `mcp`.

## v4.93.1 - CI-хотфикс: детерминированный тест wait-for-file

### Исправлено

- `test_wait_for_file_appears_after_delay` был flaky на новой macOS 3.13
  CI-ячейке (добавлена в v4.93.0): тест создавал файл через `write_bytes`
  (создать-затем-писать), позволяя `_wait_for_file` увидеть транзиентный файл
  в 0 байт и упасть на `assert 0 == 42`. Теперь тест создаёт файл атомарно
  (запись во временный файл, затем `Path.replace`), поэтому «появился» означает
  «появился с контентом». Изменение только в тесте; релизный артефакт идентичен
  v4.93.0.

## v4.93.0 - Честные уведомления, инсталлятор предлагает (а не навязывает) обновления, починен install.sh, полная macOS-матрица CI

### Исправлено

- **Уведомления больше не рапортуют ложный `ok`.** Windows-тост создавался с
  незарегистрированным AppUserModelID («Arena Unified Bridge»), который
  Windows молча дропает при exit 0 у PowerShell — поэтому `sys.notify` /
  `POST /v1/notify` возвращали `ok: true`, хотя на экране ничего не
  появлялось. Тост теперь использует собственный *зарегистрированный* AUMID
  PowerShell (реально появляется в области уведомлений), выполняется с
  `$ErrorActionPreference=Stop` + try/catch (настоящая ошибка даёт ненулевой
  exit), экранирует заголовок/текст и для XML, и для одинарных кавычек
  PowerShell (проверено живьём на кириллице + `&` + `'`). При ошибке результат
  теперь несёт `detail` с причиной. notifier'ы macOS/Linux тоже возвращают
  `(ok, detail)` и экранируют аргументы.

- **install.sh имел фатальную синтаксическую ошибку и вообще не выполнялся на
  Linux/macOS.** Дублирующий блок `elif [ -d .../skills/browseract ]` (с
  misplaced-комментарием «# Step 7») оставлял висячий `elif` — `bash -n`
  падал. Дубликат удалён; скрипт теперь парсится и выполняется.

### Изменено

- **Инсталляторы ПРЕДЛАГАЮТ обновления, а не навязывают.** SuperPowers и
  BrowserAct раньше обновлялись автоматически (`git pull --ff-only` /
  `uv tool upgrade`) без спроса. Теперь и install.bat, и install.sh спрашивают
  «Check for … updates? [y/N]» (по умолчанию N) перед обновлением, поэтому
  неинтерактивный запуск ничего молча не меняет. SuperPowers также предлагает
  re-sync из upstream, если копия без `.git` (zip-копии поставляются без
  `.git`, поэтому старый путь `git pull` не выполнялся и SuperPowers
  оставался stale).

- **Шаги опциональных компонентов install.bat усилены.** SuperPowers,
  BrowserAct и Camoufox теперь подпрограммы `call`/`exit /b` — убраны
  последние мины `goto`-из-блока (та же конструкция, что оборвала инсталлятор
  на SpecKit-шаге в v4.91.x). Проверено живьём на реальной Windows-установке.

- **Camoufox сделан менее навязчивым.** Это ОПЦИОНАЛЬНЫЙ анти-детект-слой;
  инсталлятор теперь сообщает, что обычный Chrome/Chromium через BrowserAct
  обычно работает нормально и может выглядеть человечнее для анти-бот-систем,
  а все действия Camoufox — opt-in (по умолчанию N).

- **CI: macOS-матрица теперь зеркалит полную Python-матрицу ubuntu/windows**
  (3.10 / 3.11 / 3.12 / 3.13 / 3.14) вместо одной ячейки 3.12 —
  кроссплатформенность проверяется на каждой поддерживаемой версии Python.
...
- **install.bat больше не прерывается на SpecKit-шаге.** Опциональный блок
  SpecKit (добавлен в v4.60.19) использовал `goto :speckit_done` ИЗНУТРИ
  parenthesised-блоков `if`. cmd.exe не разрешает `goto` на метку изнутри блока
  `( ... )` (усугубляется `for /f 'command'` в том же блоке), поэтому
  инсталлятор падал с *«Не удается найти указанную метку пакетного файла -
  speckit_done»* сразу после отчёта о статусе SpecKit — не доходя до установки
  сервиса и старта моста.
- SpecKit-шаг переписан как подпрограмма `call :speckit_step` / `exit /b` —
  надёжный паттерн batch, корректно возвращающийся из блоков. Шаг по-прежнему
  опционален (по умолчанию N), поведение не изменилось.
- `install.sh` (bash) был корректен изначально (обычный `if/else`); без изменений.

### Примечание

v4.92.0 починил сам мост (он стартует); v4.92.1 чинит Windows-инсталлятор,
который его доставляет. Ставьте/обновляйтесь на v4.92.1.

## v4.92.0 - Восстановлен старт моста: фикс синтаксической ошибки проводки уведомлений, отзыв v4.91.x

### Цель

v4.91.0 добавила кроссплатформенные визуальные уведомления (`sys.notify`), но
коммит проводки внёс фатальный `SyntaxError` — дублирующиеся keyword-аргументы
(`play_beep_sync` и `send_notification_sync` повторно) в сборке контекста MCP
task runtime (`arena/wiring/mcp_task_runtime.py`). Повтор keyword-аргумента —
ошибка компиляции, поэтому `arena.runtime_deps` (а значит и `unified_bridge.py`)
не импортировался вовсе: мост не запускался.

Этот один дефект был корнем трёх симптомов:

1. **«Уведомления сломали мост»** — мост падал при старте.
2. **«Инсталлятор падает в конце»** — финальный шаг инсталлятора «waiting for
   bridge to start» отваливался по таймауту, потому что только что установленный
   мост не мог подняться.
3. **«Автообновление плодит процессы bore/ngrok/cloudflared»** — падающий мост
   перезапускался watchdog'ом, и autostart-хуки каждого рестарта спавнили новые
   tunnel-процессы, которые не убирались (осиротевали при следующем падении).

### Изменения

* **Исправлен фатальный `SyntaxError`** в `arena/wiring/mcp_task_runtime.py`
  (убраны задвоенные `play_beep_sync` / `send_notification_sync`). Мост снова
  импортируется и стартует; проверено `tests/test_app_factory.py` и живым
  смоук-стартом (поднимаются все сервисы: bridge, MCP, SSE, WS, gateway,
  dashboard, task-runner, watchdog).
* **Визуальные уведомления сохранены и работают**: MCP-тула `sys.notify` и
  `POST /v1/notify` (Windows toast / `notify-send` / `osascript`), теперь
  корректно проведённые через контекст системных хендлеров.
* **Убрано дублирующееся поле `send_notification_sync`** в
  `arena/wiring/platform.py` (`SystemWiringContext`).
* **Гигиена репо**: удалены случайно закоммиченные `.coverage` (~1.2 МБ бинарник)
  и scaffolding `.specify/` (spec-kit); оба добавлены в `.gitignore`, чтобы не
  попасть повторно.

### Примечание

Этот релиз замещает бракованную серию `v4.91.0`-`v4.91.10`, которая отзывается
(релизы и теги удалены). Обновляйтесь сразу на v4.92.0.
...
Исправляет первый реальный macOS CI pass и оставшийся Windows 3.14 timeout
в Node harness. Новый macOS runner корректно вскрыл Linux-specific
предположения в тестах.

### Изменения

* KDE/Wayland `kwin_journal` capability test теперь Linux-only, а не generic
  POSIX.
* macOS firewall probe теперь возвращает явный `error`, если `socketfilterfw`
  недоступен.
* Overview toolbar rejection-path Node harness skip-ается на win32, как соседний
  wrapper test, чтобы избежать timer-handle timeouts на Windows runners.
...
Добавляет semantic structuring слой, который возник после обсуждения OCR/vision.
У моста уже есть разные способы получить text-like input (OCR, ASR, browser,
files, мультимодальные агенты). Этот релиз добавляет deterministic tools,
которые превращают сырой текст в стабильный JSON без внешних LLM-вызовов.

### Изменения

* Новая MCP-тула `document.extract_tasks` (SAFE): извлекает checklist-like
  tasks из OCR/ASR/text в `{tasks:[...]}` с `due_text` hints.
* Новая MCP-тула `document.structure` (SAFE): поддерживает `kind=auto`,
  `task_note` и `physics_homework`; для физики извлекает variables, formulas,
  find lines и answer lines.
* Новый helper module `arena.document.structure` с deterministic parsers.
* README namespace coverage и MCP contract snapshot обновлены под `document.*`.
...
Добавляет quality-layer, который вскрыл живой OCR сценарий с книжной страницей:
generic OCR работает, но реальные фото с камеры требуют подготовки изображения
(resize, grayscale, autocontrast, threshold, optional deskew) перед OCR.

### Изменения

* Новая MCP-тула `image.health` (SAFE): показывает Pillow и optional OpenCV runtime.
* Новая MCP-тула `image.preprocess_for_ocr` (MEDIUM): создаёт OCR-friendly PNG
  из любого image-файла на хосте.
* `ocr.extract(preprocess=true)` теперь запускает preprocessing перед Tesseract
  и возвращает metadata preprocessing.
* README namespace coverage и MCP contract snapshot обновлены под `image.*`.

## v4.86.4 - OCR Windows UTF-8 stdout hotfix

### Цель

Исправляет живой путь OCR extraction на Windows. Tesseract отдаёт UTF-8 TSV,
а Python `subprocess.run(text=True)` декодировал stdout локальной Windows
code page, из-за чего `proc.stdout` мог становиться недоступным после
UnicodeDecodeError в reader-thread.

### Изменения

* `ocr.extract` теперь читает stdout Tesseract с `encoding="utf-8"` и
  `errors="replace"`, так что русский TSV корректно парсится на Windows.
...
Исправляет CI-регрессии, появившиеся в сценарных релизах v4.85.0/v4.86.0.
Runtime-фичи были рабочими, но CI поймал недообновлённые guard’ы: старое
ожидание registry mobile-ext, README namespace coverage и security scan
annotations.

### Изменения

* В README namespace table добавлен `ocr`, чтобы проходил namespace-doc coverage.
* Legacy v4.59 mobile-ext registry test теперь принимает additive tools вроде
  `mobile.voice_record`, а не требует точного исторического множества.
* Добавлены security annotations для одобренных bootstrap downloads
  (`asr.bootstrap`, `ocr.bootstrap`) и owner-only `voice-captures` chmod.
* Локально проверено: namespace-doc coverage, targeted tests, semgrep, bandit и
  ruff blocking rules.
...
Укрепляет голосовой сценарий после v4.84.0, где мост доказал, что умеет
слышать. Живой прогон вскрыл следующий слой: ASR-среда не была ванильной —
`ffmpeg`, `whisper-cli` и полноценную модель пришлось ставить вручную, а
прерванная загрузка модели выглядела как существующий файл, но падала в
runtime. Этот релиз превращает ручную настройку в диагностику и bootstrap
на уровне моста, а запись голоса делает удобнее для координации с человеком.

### Изменения

* **Новая MCP-тула `asr.health`** (SAFE): показывает `ffmpeg`,
  `whisper-cli`, найденные модели, выбранную модель, partial/corrupt
  detection, PATH и подсказку по bootstrap.

* **Новая MCP-тула `asr.bootstrap`** (DANGEROUS, требует одобрения):
  Windows-bootstrap ASR runtime — скачивает `ffmpeg`, официальный Windows x64
  binary whisper.cpp и ggml-модель в пользовательские каталоги
  (`~/.local/bin`, `~/.whisper`). Загрузка модели атомарная (`.tmp` -> final),
  поэтому оборванный partial-файл больше не считается валидным.

* **Поиск моделей укреплён:** явный `model` / `ARENA_WHISPER_MODEL` остаются
  приоритетными, но авто-выбор теперь предпочитает `ggml-small.bin` вместо
  `base` для лучшего русского и отказывается от известных partial/corrupt
  файлов.

* **UX `mobile.voice_record`:** новый аргумент `pre_delay_ms` задерживает
  старт записи после установки/грантов и прямо перед включением микрофона —
  ровно тот паттерн, который понадобился в живом голосовом тесте.

* **Тесты:** покрытие ASR registry/policy/health/partial-model, guard для
  bootstrap на non-Windows и проводка аргумента `pre_delay_ms`. Контрактный
  MCP-снепшот обновлён под `asr.health` и `asr.bootstrap`.

## v4.84.0 - Мост слышит: первоклассный захват микрофона (Arena VoiceRecorder)

### Цель

Даёт мосту «слух» в пару к «зрению» (скриншот / UI-дамп). Крошечное
подписанное приложение-помощник пишет звук с микрофона на устройстве, а
новая тула `mobile.voice_record` ведёт весь цикл: установка один раз,
запись заданной длительности, перенос MP4 на хост — готовый файл для
`asr.transcribe`. Это открывает голосовые сценарии (голосовая заметка ->
транскрипция -> действие) прочным, не зависящим от приложений компонентом:
захват полностью под контролем моста, системный Диктофон НЕ драйвится.

### Изменения

* **Новая MCP-тула `mobile.voice_record`** (риск MEDIUM, требует одобрения):
  - Пишет `duration_ms` (500..600000) звука с микрофона в AAC-in-MP4.
  - Авто-установка встроенного приложения-рекордера при первом использовании
    (идемпотентно, проверка через `pm path`), гранты `RECORD_AUDIO` и
    `MANAGE_EXTERNAL_STORAGE`.
  - Запуск записи через `am start`, поллинг маркера `<output>.done`, который
    приложение пишет по завершении, перенос файла в
    `~/.arena/voice-captures/` и возврат
    `{ok, remote, local, size_bytes, status, record_ms, mime}` (плюс
    `bytes_b64` при `return_bytes=true`).
  - Транскрипция остаётся отдельным вызовом `asr.transcribe(path=<local>)` —
    агент компонирует две общие способности (слух + транскрипция).

* **Новый модуль ядра `arena/mobile/audio_capture.py`:** оркестрация
  установка / гранты / запись / перенос плюс чистые тестируемые хелперы
  (`pm_path_installed`, `parse_done_marker`, `default_output_path`,
  `bundled_apk_path`, `voice_capture_dir`).

* **Встроенный `assets/apks/arena-voicerecorder.apk`:** минимальное
  подписанное (APK Signature Scheme v3) Android-приложение —
  `com.arena.voicerecorder`, `MediaRecorder` с `AudioSource.MIC` ->
  MPEG_4/AAC — собрано из исходников пайплайном aapt2 + javac + d8 +
  zipalign + apksigner.

* **Политика:** `mobile.voice_record` отнесена к MEDIUM (меняет состояние и
  затрагивает приватность; требует одобрения, как прочие mobile-действия).

* **Тесты:** `tests/test_mobile_audio_capture.py` (логика хелперов +
  гарантия того, что APK реально поставляется и является валидным zip);
  контрактный снепшот MCP обновлён под новую тулу.

## v4.83.0 - Сценарное укрепление мобильной поверхности: честная способность звука, корректная длительность записи, надёжный поиск adb

### Цель

Первый релиз по методологии сценарной разработки: реальный сложный кейс
(«записать голос на телефоне -> перенести на ПК -> расшифровать ->
доставить в чат») прогнали против ванильного моста, а вскрытые им ОБЩИЕ
дефекты закрыли в ядре (а не патчем под задачу). Найдено и закрыто три
проблемы; неопубликованный v4.82.1-хотфикс RECT для Windows включён в
этот релиз, чтобы наконец попасть в публикуемую ветку.

### Изменения

* **Честная способность звука (`mobile.record_start` / `mobile.record_sync`):**
  - CLI `screenrecord` пишет **только видео** — опции источника звука у него
    нет (проверено по AOSP `frameworks/av/cmds/screenrecord`). Поверхность
    тулов раньше заявляла «captures device audio+mic on API 31+», но молча
    выбрасывала аргумент `audio`, выдавая немой MP4, который транскрипция
    затем превращала в пустоту.
  - Запрос `audio=true` теперь возвращает явный
    `{"ok": false, "error_type": "unsupported_capability"}` с подсказкой
    про рабочий путь захвата голоса, вместо лжи умолчанием. Ложный
    комментарий тулы исправлен.

* **Корректная длительность записи (`resolve_duration_ms`):**
  - MCP-поверхность шлёт `time_limit` (секунды, семантика screenrecord), а
    хендлер читал только `duration_ms`, из-за чего запрошенная длительность
    молча игнорировалась (всегда 30 с). Теперь разрешаются оба поля,
    `duration_ms` (мс) в приоритете над `time_limit` (с).

* **Надёжный поиск adb для мобильных тулов v4.59.0 (`tool_mobile_ext`):**
  - `mobile.launch_app` / `pull_file` / `push_file` / `list_files` искали adb
    через `shutil.which("adb")` (только PATH) и падали с «adb not found on
    PATH» на машинах, где adb лежит в каталоге Android SDK platform-tools,
    но не в PATH — при том что `mobile.ui` / `mobile.shell` (использующие
    кросс-платформенный `find_adb`) продолжали работать.
  - Эти тулы теперь используют тот же `arena.mobile.adb.find_adb`, а ошибка
    «не найден» несёт платформенную подсказку по установке.

* **Хотфикс RECT для Windows (включён из неопубликованного v4.82.1):**
  - `perform_window_action` (move/resize) теперь использует
    `ctypes.wintypes.RECT` вместо самодельного `ctypes.Structure` с полями
    `c_long`, что чинит несовпадение типов RECT на 64-битной Windows.

* **Тесты:** `tests/test_mobile_recording_contract.py` (честность звука +
  разрешение длительности), `tests/test_mobile_ext_adb_discovery.py`
  (проводка поиска adb) и `tests/test_make_release_zip_exclude.py`
  (исключения релизного zip) фиксируют новое поведение.

* **Релизный инструментарий (`scripts/make_release_zip.py`):** ротированные
  рантайм-логи (`requests.jsonl.2`, `audit.jsonl.N`, `bridge.log.N`) теперь
  исключаются из релизного zip — раньше матчились только точные базовые
  имена, из-за чего tracked-`requests.jsonl.2` (~10 МБ) попадал в архив.

## v4.82.0 - Безопасная работа с бинарными файлами, сырые скриншоты по умолчанию и нативный Window Action API на Windows

### Цель

Этот релиз приносит три крупных улучшения в подсистемы работы с файлами и десктопной автоматизации:
1. **Безопасная работа с бинарными файлами:** Добавлена поддержка Base64-декодирования в эндпоинт `POST /v1/fs/create` и MCP-инструмент `fs.create`, что позволяет агентам записывать любые бинарные файлы (включая архивы, картинки и исполняемые файлы) напрямую без вызова консоли.
2. **Сырые скриншоты по умолчанию:** Поведение эндпоинта `GET /v1/desktop/screenshot` изменено — теперь по умолчанию он возвращает сырые байты картинки (PNG/JPEG/WebP) вместо Base64-JSON-конверта, что колоссально экономит ресурсы. Формат Base64 по-прежнему доступен при явной передаче `format=base64`.
3. **Нативный Window Action API на Windows:** Реализована нативная поддержка управления окнами (свернуть, развернуть, закрыть, восстановить, изменить размеры/позицию) на Windows через вызовы ctypes в `perform_window_action`, исключая необходимость в использовании PowerShell-скриптов.

### Изменения

* **Бинарная запись файлов (`/v1/fs/create` и `fs.create` MCP):**
  - Реализован параметр `"encoding": "base64"`, активирующий декодирование Base64 и бинарную запись на диск.
  - Обновлены схемы входных параметров для документации новой опции.

* **Сырые скриншоты по умолчанию (`/v1/desktop/screenshot`):**
  - Изменено дефолтное значение формата с `"base64"` на `"raw"`.
  - Запросы без параметров теперь мгновенно отдают бинарный поток с правильным HTTP Content-Type.

* **Нативные действия над окнами (`perform_window_action`):**
  - Добавлена поддержка действий `minimize`, `restore`, `maximize`, `close` и `move_resize` на платформе Windows через прямые ctypes вызовы к Win32 API.
  - Сохранена полная совместимость с механизмами верификации и каталогизации окон.

## v4.81.1 - CI-хотфикс v4.81.0 (сплит модуля + гейты Windows-тестов)

### Цель

v4.81.0 привёз Windows-бэкенд десктопа, но все 10 test-cells
(5 версий Python × 2 ОС) упали на
`test_runtime_modules_stay_below_mini_monolith_threshold`.
Виновник: `arena/desktop/backends/windows.py` — 658 строк
при потолке 600 в "modular runtime". На Windows дополнительно
падали 4 старых KWin/xdotool юнит-теста, потому что мой новый
шорт-серкит `sys.platform == "win32"` перехватывал их моки.
Coverage на Linux просел до 50.35% при пороге 51%, потому что
новый бэкенд — это 490 строк NotImplementedError-заглушек на
Linux.

v4.81.1 — точечный follow-up: никакого нового поведения ни на
Windows, ни на Linux, только рефакторинг и перекладка моков,
чтобы CI v4.81.0 стал зелёным.

### Изменения

* **Сплит `arena/desktop/backends/windows.py`** на два
  модуля, чтобы уложиться в лимит 600 строк:
  - `arena/desktop/backends/_win32_api.py` (239 строк) —
    все ctypes-сигнатуры user32/gdi32/kernel32, все
    Windows-константы (`SW_*`, `SWP_*`, `MOUSEEVENTF_*`,
    `KEYEVENTF_*`, `SM_*`, `SRCCOPY`, `VK_MAP`), и два
    BMP-хелпера (`hbitmap_to_png_bytes` /
    `raw_bgra_to_bmp`). Чистая обвязка API, никакой
    политики.
  - `arena/desktop/backends/windows.py` (440 строк) —
    высокоуровневый API (`is_available`,
    `virtual_screen_rect`, `capture_screenshot`,
    ...
    `sync_click_and_verify`). Ре-экспортит константы и
    старые имена `_hbitmap_to_png_bytes` /
    `_raw_bgra_to_bmp`, чтобы существующие тесты не
    сломались.

* **`arena/desktop/focus.py`** — win32-шорт-серкит теперь
  гейтится по `env.get("has_win32_windows")`, а не по
  `sys.platform == "win32"`. Тесты, которые подкладывают
  свой `detect_env` через параметр
  (`test_focus_window_uses_kwin_script_for_uuid_targets` в
  `test_desktop_windows.py`), могут форсить Linux-ветку
  даже на Windows-раннере CI. `import sys` убран.

* **`arena/desktop/active_window.py`** — win32-ветка теперь
  проверяет модульный флаг `_USE_WIN32_ACTIVE_WINDOW`
  (инициализируется один раз из `sys.platform == "win32"`).
  Три старых KWin/xdotool теста
  ...
  перед прогоном, что чинит их на обеих ОС.

* **`pyproject.toml`** — конфиг coverage обновлён:
  - `omit`: `arena/desktop/backends/windows.py` и
    `arena/desktop/backends/_win32_api.py` исключены на
    Linux (там это на 99% NotImplementedError; настоящие
    пути покрывают Windows-only live тесты).
  - `exclude_lines`: добавлены паттерны для win32-гейтов
    (`if env.get("has_win32_...")`,
    `if _USE_WIN32_ACTIVE_WINDOW`,
    `if sys.platform == "win32"`), чтобы Linux CI перестал
    считать их за непокрытые бранчи.

* **`arena/desktop/{input_handlers,screenshot,active_window,focus,window_catalog}.py`**
  — на строках win32-гейтов проставлен маркер
  `# pragma: no cover` в дополнение к regex-исключениям
  (старые версии `coverage.py` учитывают либо одно, либо
  другое в зависимости от структуры бранча).

### Числа после фикса (локальный Linux)

* pytest: 615 pass, 10 skip (7 Windows-live + 3
  до-существовавших), 1 флейк
  (`test_probe_tcp_timeout_short`, сеть/тайминг, старый).
* Coverage: **51.06%** при пороге 51% (Linux).
* `test_runtime_modules_stay_below_mini_monolith_threshold`:
  зелёный (windows.py = 440, _win32_api.py = 239).
* Все 7 структурных guards (catalogue-harden,
  catalogue-consistency-check, dead-dispatch-check,
  json-schema-check, handler-signature-check,
  handler-namespace-consistency, legacy-name-guard):
  зелёные.
* version_sync: OK на 4.81.1. changelog_freshness: возраст 0d.
* ruff check `--select F821,F811`: чисто.

### Заметки по миграции

Нет. Публичный API-surface (`/v1/desktop/*`) и импортируемые
имена в `arena.desktop.backends.windows` не изменились.
Downstream-код, импортирующий
`arena.desktop.backends.windows.user32` /
`arena.desktop.backends.windows.VK_MAP`, продолжает
работать — эти имена ре-экспортируются из `_win32_api`.
...
Windows runner был отключён в v4.61.0 потому что 4
pre-existing platform-specific test failure были пойманы в
первом CI run (mission-schedule bash probe, cp1251 decode,
sound-device assertion, sqlite file locking). Три из них
были починены в v4.61.1 (добавлены `skipif(sys.platform ==
"win32")` маркеры) и matrix сократили до
`os: [ubuntu-latest]`. План тогда был: "Windows runner
отложен на v4.62.x после того как platform-specific
test failure починены или skipif'нуты". v4.68.0 — релиз
который это делает, теперь когда:

- v4.62.0 добавил actionlint + contract job separation
  (Windows-only test failure больше не нужно прятаться
  в 5 одинаковых test cells).
- v4.65.0 добавил coverage-diff + dependabot + stale bot
  (CI поверхность больше и полезнее, так что добавить
  6-й runner к matrix — это маленькая маржинальная стоимость).
- v4.66.0 добавил version-sync + changelog-freshness
  (Windows-specific failure Python тестов не рискует
  спрятать release-process regression).
- v4.67.0 добавил catalogue-harden (структурные guard'ы
  которые не зависят от платформы).

Первый CI run после этого commit'а выявит новые
Windows-specific failure; v4.68.0 follow-up commits
добавят `skipif` маркеры для каждого, и matrix достигнет
green на Windows за небольшое количество follow-up commits.

### Изменено

1. **`.github/workflows/ci.yml`** — `os: [ubuntu-latest]` →
   `os: [ubuntu-latest, windows-latest]`. Matrix теперь
   запускает каждую версию Python (3.10-3.14) и на Linux,
   и на Windows, так что 10 test cells на PR вместо 5.

### Вне scope (намеренно, в очереди)

* **Deprecate the bare names** в `MCP_TOOLS`
  (`ping` / `echo` / `exec` / `snapshot`) — v4.67.0
  отгрузил namespaced siblings и оставил bare names для
  backward compat. v4.70.0 должен пометить их
  deprecated в descriptions и v4.75.0 должен удалить
  их. В очереди.
* **Coverage gate 50% → 60%** — нужны новые behavioural
  тесты, не только структурные. В очереди.
* **Mutation testing** (mutmut/cosmic-ray) — нужно
  зрелое покрытие. В очереди.
* **Mypy strict rollout** за пределами
...
### Follow-up (коммиты acd85a2, 250de6f8)

Два коммита которые не меняют поведение, но фиксируются здесь
чтобы цепочка "что сломалось и почему" осталась трассируемой.

* **`acd85a2` — fix(version-matrix):** релиз v4.65.0 использовал
  тот же paren-counter-based хелпер для вставки новой версии в
  `BRIDGE_VERSIONS`, что и релиз v4.64.0. Хелпер считает `(`/`)`
  чтобы найти закрывающую скобку tuple, но в docstring модуля
  есть `(` внутри backticks (пример `chat_extension/`), которая
  сбивает счётчик, так что новая запись приземлилась в
  docstring а не в tuple. Баг тот же что в `710b84b0`. Фикс тот
  же: переписать файл с master HEAD и пере-вставить в правильном
  месте, с AST-validate шагом перед commit.
  *Долгосрочный фикс:* paren-counting хелпер должен быть
  переписан чтобы пропускать docstring'и (настоящий
  `bump_version.py` скрипт в очереди на v4.66.0; это
  follow-up, не блокер).
* **`250de6f8` — fix(stale):** actionlint поймал два невалидных
  имени input'ов в `.github/workflows/stale.yml` на первом PR
  v4.65.0. `only: "issues and pulls"` не существует в
  `actions/stale@v9` (action по умолчанию работает с обоими), и
  `exempt-draft-prs` должно быть `exempt-draft-pr` (без
  завершающего 's'). Оба исправлены в одном коммите. Это
  третий actionlint follow-up в серии v4.62.0 / v4.64.0 /
  v4.65.0 и подкрепляет кейс для actionlint job: он ловит
  опечатку до того как она запустится на реальном PR.
...
Задокументировать и **сделать видимой** проблему Edge-headless,
которую мы впервые увидели в smoke-прогонах v4.60.18. Причина —
upstream-политика Chromium (`Edge is running elevated: 1`), а не
баг моста. Этот релиз добавляет actionable-диагностику, чтобы
пользователь видел **workaround** вместо `Browser process exited
(rc=0) stderr=`.

### Добавлено

1. **`arena/browser/diagnose_elevation.py`** — новый модуль,
   который проверяет stderr на известные Chromium/Edge-маркеры
   отказа (сейчас: `running elevated: 1`, `elevation is not
   supported`). Возвращает структурированный `isError` с
   конкретным next-step: либо запустить мост от non-admin
   пользователя, либо использовать BrowserAct cloud, либо
   поставить Camoufox. Диагностика строго аддитивна — если
   маркер не найден, возвращается `None` и поведение
   идентично прежнему.

2. **`docs/browser-headless-on-windows.md`** — полный статус
   ситуации с Edge headless на Windows, три workaround'а с
   trade-off'ами, и явная секция «что мы НЕ сделали» чтобы
   не преувеличивать фикс в будущих изменениях.

### Изменено

3. **`arena/browser/cdp_client/tab_manager_connect_launch.py`** —
   `_raise_browser_exit` теперь импортирует `diagnose_elevation`
   и добавляет actionable-сообщение к `ConnectionError` при
   обнаружении известного маркера. Оригинальный raw-stderr
   сохранён рядом (defensive: ничего не теряется).

### Тесты

`tests/test_diagnose_elevation.py` (10 гардов):
- пустой stderr -> None
- здоровый stderr -> None
- `running elevated: 1` детектируется
- альтернативный `elevation is not supported` детектируется
- сообщение actionable (длина + упоминание workaround'а)
- `diagnose_browser_exit` -> None на нормальном запуске
- пустой stdout + rc=0 детектируется
- elevated-warning приоритетнее empty-output
- substring-mismatch детектируется
- комбинированная логика приоритетов

### Live smoke

После коммита диагностика отрабатывает на реальном запуске
`msedge.exe --headless --dump-dom`, который упал в v4.60.18.
Корректно показывает: «Edge is running elevated: 1. Headless
mode from an admin process is blocked by Chromium's security
policy. Workarounds: (1) run the bridge as a non-admin service;
(2) use BrowserAct's cloud browser; (3) install Camoufox
(Firefox-based, no elevation block) and use
`browser.launch --type=camoufox` instead of Edge. See
docs/browser-headless-on-windows.md for details.» Проверено
локально до релиза.

### Out of scope (намеренно, зафиксировано в docs)

- Сама проблема Edge-headless **не починена**. Диагностика —
  и есть фикс. Три workaround'а в docs-файле — это реальные
  фиксы; пользователь выбирает.
- Non-elevated browser-worker процесс — принципиальный фикс,
  отслеживается для v4.61.x.
...
### Назначение
Первый релиз, продиктованный методологией "сценарий как стресс-тест ядра".
Сценарий - это реальная, сложная пользовательская задача, которую ванильный
мост либо выдерживает, либо нет. Если не выдерживает - расширяем ядро
общей возможностью (не хаком под задачу) и шлифуем релиз.

Сценарий "голос на телефоне -> перенос на ПК -> транскрибация -> чат" -
первый драйвер. Стресс-тест вскрыл три реальные общие дыры, все закрыты
в этом релизе; онлайн-транскрибация через браузер подключена и проверена
живьём.

### Исправлено

1. **CDP / браузерный путь падал на Windows при импорте** (`process_discovery.py`).
   `os.getuid()` (только POSIX) и `pwd` вызывались безусловно на уровне
   модуля, поэтому импорт `cdp_browser` на Windows взрывался с
   `module 'os' has no attribute 'getuid'` и убивал весь роут-поверх
   `/v1/browser/*`. Теперь функция кросс-платформенная: на не-Linux
   возвращает унаследованный env без изменений; на Linux `os.getuid()`
   защищён `hasattr` (оборона в глубину).

2. **stdlib `http` затенялся на Python 3.14** (`arena/http.py` ->
   `arena/web_utils.py`). Файл в 29 строк с именем `arena/http.py` (только
   `cors_json_response` + `CORS_HEADERS`) затенял stdlib `http` на
   Python 3.14, где stdlib `http` теперь - пакет. Импорт aiohttp ->
   `from http import HTTPStatus` подхватывал наш файл и делал circular
   import, ломая запуск сервера. Файл переименован в `arena/web_utils.py`,
   обновлены 3 абсолютных импорта
   (`exec/handlers.py`, `observability/handlers.py`,
   `runtime_deps/observability_system_wiring.py`). Содержимое не тронуто,
   только имя модуля.

### Добавлено

3. **`fs.write_base64` MCP-инструмент** (`mcp/tool_fs.py`). Ванильный
   `fs.write` пишет только текст. Нужно для материализации бинарных
   пейлоадов (например, записи с телефона через `mobile.record_pull`,
   который возвращает base64) в реальные файлы на диске, которые
   downstream-инструменты (`asr.transcribe`, `browser.interact`) могут
   прочитать. Декодирует base64 (сначала строгий, затем мягкий fallback)
   и пишет бинарно; на невалидном вводе возвращает аккуратный `isError`,
   а не падает.

### Тесты
`tests/test_fs_write_base64.py` - 2 гарда: round-trip бинарной записи
(включая не-UTF8 байты `\x00\x01`) и корректный отказ на не-base64
входе как `isError`-дикт.

Live smoke: `cdp_browser` import OK на Windows; `CDPTabManager`
конструируется; ошибка `os.getuid` при импорте больше не воспроизводится.

### Расширение
Байт-в-байт идентично v4.53.1 - релиз только моста.

## v4.60.17 - Живой target для v4.60.16 auto-update detach fix

### Назначение
Никаких функциональных изменений vs v4.60.16. Существует чтобы v4.60.16 (auto-update mover действительно detached через wscript+VBS шим) можно было проверить end-to-end из бегущего v4.60.16 моста. Dashboard -> Install -> v4.60.17 должен на этот раз пройти полный цикл.
...
### Добавлено
Две диагностики, чтобы будущие сбои auto-update можно было разобрать по одному audit trail'у, без ещё одного круга итераций:

1. **`admin.update.apply.restart_scheduled` audit event.** `handlers_update.py` теперь эмитит это событие прямо перед вызовом `restart_process()`. Наличие события = "handler дошёл до restart point"; отсутствие = "handler вылетел раньше, смотри `admin.update.apply` на ok=False причину". v4.60.13 убрал гейт ``platform != "windows"``, но не было простого способа доказать по audit trail, что handler реально вызывает `restart_process`. Теперь есть.

2. **`.arena-update-apply.log` рядом с mover'ом.** Windows mover .cmd теперь пишет пофазный лог в sibling-файл (в install root, переживает cleanup payload temp). Логируемые фазы:
   - `mover-start pid_target=<pid>` — mover запустился, ждёт исчезновения bridge PID
   - `bridge exited, starting copy` — прошёл wait loop
   - `copy done, launching relaunch` — robocopy/copy закончены
   - `relaunched via schtasks` / `relaunched via start_hidden.vbs` / `relaunched via start_bridge.bat` — какой relaunch реально сработал
   - `WARN no relaunch mechanism found` — bridge не поднимется сам
   - `mover-done` — чистый exit

Любой будущий "auto-update не сработал" разбирается чтением файла по `<install-root>\.arena-update-apply.log`.

### Тесты
`tests/test_auto_update_diagnostics_v4_60_14.py` — 4 guards: handler эмитит restart-scheduled событие, mover пишет phase log с минимум 4 echo-строками, ветка "no relaunch mechanism" существует, log path в install root (не в payload temp).
...
### Исправлено
Две ``echo``-строки в ``install.bat`` печатались с изувеченными ``!``, потому что ``setlocal enabledelayedexpansion`` съедает любой неэкранированный ``!`` внутри аргумента (ищет ссылку на несуществующую переменную):

- ``(Trojan:Win32/Wacatac.B!ml)`` -> печаталось как ``(Trojan:Win32/Wacatac.Bml)``, скрывая настоящее имя malware-семейства (документация про Defender false-positive)
- ``[ERROR] Python not found!`` -> хвостовой ``!`` молча отваливался

Заэкранированы через ``^!``.

### Изменено
Ветка отсутствующего Camoufox раньше просто печатала команду и уходила дальше. Теперь она активно запускает ``uv tool install --python 3.12 --with camoufox browser-act-cli`` (идемпотентно, добавляет camoufox рядом с существующим BrowserAct без полной пере-установки) и проверяет импортируемость. Огорожена ``where uv``, чтобы машины без uv получали чистое manual-fallback сообщение вместо ошибки.

### Тесты
``tests/test_install_bat_v4_60_10.py`` — 4 guards:
1. В каждой ``echo``-строке ``!`` либо часть ``!VAR!``, либо экранирован как ``^!``.
2. Wacatac конкретно проверен.
3. Camoufox-ветка пытается ``uv tool install --with camoufox`` (не просто печатает hint) и предварительно ``where uv``.
4. Python-not-found хвостовой ``!`` экранирован.
...
### Исправлено
`dashboard/assets/body-20-transports.html` — сетка карточек транспортов использовала `repeat(auto-fit, minmax(340px, 1fr))`, что растягивает каждую карточку на всю ширину, когда транспортов мало. Когда активных транспортов меньше, чем колонок, последняя карточка расширялась и весь ряд визуально смещался вправо. Заменено на `auto-fill` + `justify-content:start` — пустые треки остаются пустыми, карточки сохраняют заданную ширину независимо от их количества.

### Улучшена диагностика
`arena/lifecycle.py` — сбои autostart писались уровнем `debug` и по умолчанию не показывались. Операторам, сообщающим «галочка сохранена, но транспорт не поднялся», не по чему было ориентироваться. Поднято до `warning`:
- `[<label>] Autostart hook not wired (skipped)` — hook = None
- `[<label>] Autostart hook returned None (dependencies not wired?)` — hook отработал, но не вернул outcome
- `[<label>] Autostart FAILED: <причина> (<длительность>s)` — попытка была, но не удалась
- `[<label>] Autostart raised <ExceptionType>: <message>` — сам hook кинул исключение

Старый OK-путь на `log_info` сохранён без изменений. `LifecycleContext` получил опциональное поле `log_warning`; при отсутствии откатывается на `log_info`, чтобы старые вызывающие не сломались.

### Расширение
Байт-в-байт как v4.53.1 — только мост.

## v4.60.1 — ZeroTier: LAN-only подключение теперь признаётся честно

### Исправлено
`_zerotier_snapshot()` в `arena/admin/tunnels.py` раньше использовал `connected` из CLI напрямую. Команда `zerotier-cli status` возвращает **planet-connectivity** — может ли демон достучаться до корневого сервера. На хостах за жёстким NAT (или при временно недоступной планете) это поле пишется как `OFFLINE`, даже когда демон спокойно peer'ит с локальной сетью через кэшированные roots / LAN discovery, и у ноды есть назначенный IP.

Итог: транспорт помечался `active=False`, карточка Overview рисовалась как `○` (installed, но не active), Transports расходилась с Overview — при том, что `curl http://<zt-ip>:port` работал.

Фикс: распознаём «LAN-only connected» — активная сеть + назначенный IP достаточны, чтобы считать транспорт up. Статус планеты вынесен в отдельное поле `planet_connected`.

### Изменение семантики
- `connected` теперь `zt.connected OR (active_nets AND assigned_ip)` — надмножество старого.
- `active` теперь `active_nets AND assigned_ip` — соответствует «URL работает».
- `planet_connected` — новое поле, старый raw `zt.connected` для тех, кому важна именно planet-достижимость.

### Контрактные тесты
Три новых теста в `tests/test_tunnels_probe.py` закрепляют три состояния: LAN-only (новое), planet-online (старое поведение сохранено), planet-offline-без-сетей (по-прежнему честно inactive).

### Расширение
Байт-в-байт как v4.53.1 — только мост.

## v4.60.0 — прогон реальных багов (список Ivan'а: #2, #7, seed #9) + CI зелёный

Ivan вернулся с практики Windows / GNU/Linux + ZeroTier с 10 конкретными полевыми багами. Это не фича-релиз — это *слушающий* релиз. Три пункта закрыты здесь, остальные ушли в отдельные задачи.

### Исправлено

**#7 — Иконка Hooks не отображается на Windows 10 LTSC 2021.**
`dashboard/assets/00-tabs-registry.js` использовал 🪝 (U+1FA9D, Emoji 14.0, вышел в сент. 2021). Базовый Segoe UI Emoji в Windows 10 LTSC 2021 поддерживает максимум Emoji 13.1 — глиф рисовался пустым квадратом. Заменил на 🎣 (U+1F3A3, Emoji 3.0 — везде есть). Новый контрактный тест `test_tabs_registry_avoids_emoji_14` не даст молча вернуть Emoji 14 обратно.

**#2 — Состояние ZeroTier расходилось между Overview и Transports.**
`dashboard/assets/20-transports.js` проверял `ztRaw.available !== false` при определении «ZT установлен». Но `zerotier_status()` возвращает `installed`, а не `available` — фантомное поле = `undefined`, `undefined !== false` = `true`, поэтому Transports **всегда** уверял что ZT установлен, независимо от реальности. Overview же смотрел `/v1/zerotier/peers` — другая проверка. Исправил на `ztRaw.installed`. `active` теперь также консультируется с `ztRaw.active_count` — на хостах с CLI-бекендом (нет `.zerotier.online` bool'а) видит подключение по наличию активных сетей.

**#9 seed — Чекпоинт сессии агента.**
`arena/agent_session.py`: минимальный `write_checkpoint / read_checkpoint / append_note / clear_checkpoint` в `~/.arena/agent_session.json`. Это ещё не полная «возобновляй с чекпоинта» логика (гораздо более крупная задача) — просто место, где текущий агент оставит крошки следующему после перезагрузки/потери контекста. Переопределяется через `ARENA_AGENT_SESSION_FILE`.

### Полоса зелёного CI восстановлена

v4.59.1 закрыл ruff F811, который молча падал с v4.54.1, и bandit B108 из v4.59.0. Начиная с v4.59.1 оба пайплайна (**Security scan** и **CI (ruff+tests)**) зелёные. Мои прошлые «CI green» рапорты для v4.56-v4.58 покрывали только Security scan и технически были неверны — новое личное правило: инспектировать имена **всех** workflow для тега перед словом «RELEASED».

### Пока не исправлено (из списка Ivan'а — в очереди)

- **#1** — CI падал несколько релизов → закрыт в v4.59.1 и здесь.
- **#3** — Autostart транспортов не работает (галочка сохраняется, но игнорируется).
- **#4** — Auto Update на Windows 10 LTSC 2021 ничего осмысленного не делает.
- **#5** — inventory.py и full inventory не грузятся на Windows.
- **#6** — Layout Transports смещается вправо, когда транспортов запущено мало.
- **#8** — Задержка вкладок в Dashboard на Windows хуже, чем на GNU/Linux.
- **#10** — sudo (Linux) / runas (Windows) / macOS coverage.
...
line 309 использует `urllib.request.urlopen(user-supplied URL)`
в `_wait_for_http`. `# nosec B310` был на месте, а вот
`# nosemgrep: dynamic-urllib-use-detected -- <rationale>` я
забыл — CI security gate отверг релиз. Добавил nosemgrep
аннотацию с тем же обоснованием. Функционально ничего не
поменялось.

pytest suite: 2998 passed. Bridge live на 4.55.1.
...
sidebar-раскладки MCP SuperAssistant
(github.com/srbhptl39/MCP-SuperAssistant, MIT-license). Релиз
затрагивает только UI — парсер, адаптеры и код collapse
байт-в-байт идентичны v4.51.4.

### chat_extension `sidepanel.html` + `sidepanel.js` + `popup.css` (0.14.33 → 0.14.34)

Side panel теперь имеет **четыре таба**:

1. **Status** — здоровье bridge, dump политик, connectivity-
   badge в заголовке (зелёный `v<версия>` когда bridge
   доступен, красный `offline` когда нет). Кнопки те же, что
   в v4.51.x.

2. **Tools** — поисковый, отфильтрованный по категории каталог
   инструментов, загружаемый из
   `/v1/extension/instructions?category=…`. Каждый tool —
   раскрывающаяся карточка:
   * заголовок: моноширинное имя, risk badge
     (`safe`/`medium`/`dangerous`, цветокодированный), topic
     pill, описание
   * тело: JSON Schema, CSN однострочник, пример аргументов,
     два action-а per-tool:
     * **Copy call template** — кладёт в буфер обмена готовый
       `arena-tool` fenced block с pre-filled example
       arguments
     * **Copy CSN line** — кладёт однострочное описание
       (`name (risk) — description; schema: <csn>`)
   * Категории: `safe`, `medium`, `dangerous`, `all` и
     тематические `fs`, `mission`, `memory`, `browser`,
     `desktop`, `git`, `system`.

3. **Instructions** — Copy Instructions с **live preview**.
   Category selector (тот же список плюс режим «preamble
   only, no catalog») + format selector (`arena` / `jsonl` /
   `both`). Summary-строка показывает
   `<N> chars · <M> tool(s) · fmt=…` — видно точный payload
   до копирования. Кнопка **Copy to clipboard**.

4. **History** — wiring не изменился с v4.51.x. Все
   command-lifecycle-группировки, фильтры по kind/site/adapter
   и per-card actions (Inspect Payload, Inspect Result,
   Replay Preview, Replay Execute, Copy Payload, Copy Result)
   сохранены. Хелперы `lifecycleSummary`, `lifecycleKinds`,
   `groupCommandHistory`, `commandGroupFromEvents`,
   `scanDiagnostics`, `bridgeDiagnostics`, `versionDiagnostics`,
   `insertionDiagnostics`, `cardMetaParts` — все на месте.

Табы **lazy-loaded**: Tools, Instructions и History дёргают
данные только при первой активации, так что открытие side
panel мгновенное даже через медленный tunnel.

### Тесты

* Добавлен `tests/test_extension_v4_52_0.py` — 18 ассертов:
  bump версий (manifest/content.js/insert_strategies.js/
  README/constants.py/pyproject.toml), 4-tab HTML структура,
  per-tab контролы (Tools, Instructions, History), header
  connectivity badge, JS wiring (tab activator, lazy-load
  hooks, tool catalog cache, instructions live preview),
  per-tool copy actions, History-tab wiring сохранён
  (regression guard), CSS tab + tool + risk-badge стили.
* Legacy `tests/test_chat_extension_sidepanel_flow.py`
  по-прежнему зелёный — History-tab wiring не тронут.
* Полный suite: **2779 passed** (2761 baseline + 18 v4.52.0).
  Zero regressions.

### Что не вошло

* **Mistral duplicate-mount loop** — по-прежнему отложено
  согласно указанию Ivan из v4.50.17.
* **browser-agent-bridge server-driven mode** — изучено в
  v4.51.2, остаётся отмеченным как возможное будущее
  направление.
...
Три фикса из сканов Ивана после v4.51.1 + существенный рефактор
`instructions.py` после изучения MCP SuperAssistant.

## 1. z.ai регрессия (критично)

**Симптом:** скан z.ai показывает цикл `mounted →
sweep_orphan_shadow_removed` каждый scan. Toolbar не появляется.

**Причина:** orphan sweep v0.14.27 требовал
`shadow.previousElementSibling === mounted host`. Но `controlsHost`
на z.ai возвращает DIV, `attachControls` использует `appendChild`
— shadow host становится ЧАДОМ host'а, не sibling'ом. Каждый
валидный z.ai toolbar помечался orphan-ом и удалялся.

**Фикс (`chat_extension/content.js::sweepDuplicateToolbars`):**
`isAnchored` теперь принимает ЛИБО prev-sibling anchor (PRE
паттерн) ЛИБО parent-element anchor (appendChild паттерн).

## 2. Collapse tool results не работает на Gemini/Mistral/Kimi/Qwen/DeepSeek

**Симптом:** после Insert + Send блоб остаётся raw на большинстве
сайтов. Fold только на ChatGPT/T3/OpenRouter.

**Причина 1:** HTML-comment sentinel `<!-- arena:tool-result -->`
strip'ается syntax highlighter'ами (shiki/prism/monaco) во время
токенизации. `textContent` после highlighting не содержит
comment → `collapseToolResultsInHistory` не fires.

**Причина 2:** селектор блоков покрывал только `pre / code /
[class*="code-block"]` — пропускал Gemini `<code-block>` custom
element, Qwen `.qwen-markdown-code`, Kimi `.language-jsonl`,
Gemini `.formatted-code-block` wrapper.

**Причина 3:** оборачивание innermost PRE оставляло chrome
fence'а (copy button, language label) СНАРУЖИ `<details>`.

**Фиксы:**
- **`formatInsertText`** теперь стемпает **visible-text sentinel**
  `ARENA_RESULT_V1` первой строкой fenced block. Выживает любой
  highlighter. Legacy comment sentinel всё ещё детектится для
  сообщений в старых чатах.
- **Расширенный селектор**: добавлены `code-block`,
  `[class*="language-"]`, `[class*="qwen-markdown-code"]`,
  `[class*="segment-code"]`, `[class*="formatted-code-block"]`.
- **Fence-root walk**: при попадании на inner element — walk up
  до enclosing fence container (custom element или wrapper class),
  чтобы весь fence уходил в `<details>`.

## 3. Мерцание collapse

**Симптом:** blob виден ~300-600мс перед сворачиванием.

**Причина:** `collapseToolResultsInHistory` работал в конце
`scan()`, а `scan()` за 300мс throttle.

**Фикс:** MutationObserver callback теперь вызывает
`collapseToolResultsInHistory()` **синхронно**, до `scheduleScan()`.
Fold происходит на том же frame когда sentinel-containing block
попадает в DOM.

## 4. Редизайн инструкций (адаптировано из MCP SuperAssistant)

**Фидбек Ивана:** каталог v4.51.1 был неэффективный (raw JSON
schemas жрут токены) и AI часто игнорировал формат. Изучение
референсной имплементации в
`github.com/srbhptl39/MCP-SuperAssistant/pages/content/src/
components/sidebar/Instructions/` (MIT license) выявило 2
паттерна, которые стоило перенять:

### CSN — Compressed Schema Notation

3-5× экономия токенов vs raw JSON Schema без потери
выразительности. Пример:
```
До (JSON Schema, ~128 токенов):
  {"type":"object","properties":{"path":{"type":"string"},
   "depth":{"type":"integer","default":2}},"required":["path"],
   "additionalProperties":false}
После (CSN, ~28 токенов):
  o {p {path:s r; depth:i d=2} ap f}
```

Новый публичный helper
`arena.extension_bridge.instructions.json_schema_to_csn()`.
Обрабатывает enums, unions, arrays, nested objects, constraints,
defaults, `additionalProperties: false`.

### Явная SYSTEM prompt структура

Заменяет расплывчатый "Useful safe tools include..." параграф
на структурированный `<SYSTEM>` блок с чёткими правилами:
- Точный tool-call формат (Arena или MCP-compatible JSONL).
- Один вызов на response, STOP после emit, ждать реальный result.
- Никогда не invent результаты или destructive параметры.
- CSN notation quick-guide inline (AI читает `o {p {...}}` без
  доп. документа).
- Response format: один параграф → tool block → STOP.

### Rendering каталога

Компактная per-tool строка:
```
- **fs.view** (safe) — View file contents with line numbers. ...
  schema: `o {p {path:s r; view_range:a[i]} ap f}`
```
Потом ОДИН worked example (Arena или JSONL зависит от `format`
param). Предыдущий v4.51.1 дампил полный arena-tool code fence
на каждый tool — прожигало токены и путало AI где template а
где live call.

### Атрибуция

Оба паттерна credited MCP SuperAssistant (MIT) в comment block
вверху `arena/extension_bridge/instructions.py`.

## Файлы

- `chat_extension/content.js` — orphan sweep parent-anchor фикс,
  formatInsertText sentinel, collapse selector + fence-root walk,
  MutationObserver-hooked collapse.
- `arena/extension_bridge/instructions.py` — полный переписал с
  CSN + MCP-SA-style system prompt + компактный каталог.
...
- Новый `tests/test_extension_instructions_v4_51_2.py` — 15
  asserts на все 3 content.js фикса + CSN edge cases +
  MCP-SA-style preamble + catalog CSN rendering + back-compat.
- Перепиннены v0_14_* на `0.14.31`.

## Изучено но не адоптировано (пока)

- **browser-agent-bridge** — server-driven WebSocket архитектура.
  Другой дизайн; заметил для потенциального bridge-driven agent
  mode в будущем.
- **chrome-extension-bridge** (mcpmarket) — тонкий popup MCP
  server discovery. Не подходит под наш use case.
...
Два прямых root-cause фикса из v4.50.14 сканов Ивана. Без гаданий
в этом раунде — диагностика строго по тому что показали scan
reports.

## 1. T3 chat дубль — причина в `attachControls`, не в sweep

**Симптом (Иван, v4.50.14 скан):** T3 chat первое сообщение в
новом чате всё ещё показывает 2 toolbar на fingerprint
`arena_msg_1326293718`. Оба mounted event сработали.
`mounted_diagnostics` содержит 2 shadow host по путям
`DIV:0/DIV:0/DIV:0/DIV:0/DIV:3/DIV:0` и `.../DIV:1` — это **прямые
siblings** одного и того же PRE.

**Причина:** `attachControls()` вызывал
`host.insertAdjacentElement('afterend', bar)` дважды на гонке.
Sweep v0.14.24 бежит в конце scan; но обе попытки mount внутри
ОДНОГО scan pass успевают прикрепить bar до того как sweep что-то
увидит. И map хранит только 1 entry (`mountedControls.set(fp,...)`
перезаписывает), поэтому sweep не может понять что оставлять.

**Фикс (`chat_extension/content.js::attachControls`):** перед
`insertAdjacentElement` обходим `host.nextElementSibling` и
**удаляем** любую arena bar (`[data-arena-tool-controls="1"]`) или
shadow host (`[data-arena-shadow-host="1"]`) которая уже висит.
Аналогично для ветки `appendChild` — чистим arena-детей. Sweep
v0.14.24 остаётся second line of defence для дубликатов
между scans.

## 2. Arena.ai Battle — carousel top-up в candidate discovery

**Симптом (Иван, v4.50.14 скан):** в Battle обе AI колонки видны
в браузере, но toolbar только на column[1]. Диагностика v0.14.24
показала carousel в DOM с обеими колонками
(`carousels: 3, columns: [...has_ai_bar:false, has_ai_bar:true...]`).

**Причина:** `arenaCandidateNodes()` возвращал только 2 кандидата
(user PRE + column[1] AI PRE). Column[0] AI PRE был в DOM, но
съеден `arenaPruneAncestorCandidates` — эта политика prune убирает
любой узел который **содержит** другой candidate, а column[0] PRE
случайно содержит вложенный `<code>` который также подхватил
селектор `code`. Column[1] PRE выжил из-за особенностей DOM
layout'а.

**Фикс (`chat_extension/adapters.js::arenaCandidateNodes`):**
после стандартного prune, если мы на arena.ai — **carousel top-up
pass**:
- Обходим каждый контейнер `[class*="carousel"] / [class*="battle"]`
  и его детей (колонки).
- В каждой колонке ищем PRE, у которых `textContent` содержит
  `function_call_start`.
- Добавляем такой PRE в список кандидатов ЕСЛИ он ещё не там И
  не пересекается (contains-or-contained-by) с существующим.

Плюс расширил cap кандидатов с 5 → 8, чтобы Battle 2 AI-колонки +
6 предыдущих turns помещались.

Обогащён диагностический `arenaai_hint.carousel.columns[]` полями
`has_pre`, `pre_count`, `has_tool_text` — любой оставшийся Battle
miss можно root-cause за 1 scan без переписки:
- `has_pre=false` — модель ответила только параграфами, без code
- `has_pre=true` + `has_tool_text=false` — модель не выдала JSONL
  или Arena.ai post-процессила block
- `has_tool_text=true` + `has_ai_bar=false` — extension miss (то
  что фиксим этим релизом)
...
- Новый `tests/test_chat_extension_v0_14_25.py` — 14 asserts.
- MAX_PRODUCT_FILE_LINES 1100 → 1200
  (content.js 1119 LOC, adapters.js 1103 LOC).
- Перепиннены v0_14_* + assets + adapter_flow на `0.14.25`.

## Всё ещё отложено

- Mistral flaky mount — Иван говорит не воспроизводится стабильно,
  возможно model-side.
- **Дальше:** v4.51.0 (collapse tool results in chat history).
...
Три повторные попытки после тура Ивана по v4.50.8. Каждый фикс
заменяет эвристику v4.50.8, которая либо дала визуальную регрессию
(Kimi), либо не сработала вовсе (z.ai, arena.ai).

## 1. Kimi — огромная пустая колонка toolbar в сохранённых чатах

**Симптом (Иван):** "На Kimi toolbar появляется теперь, но там
...
**Причина:** v0.14.18 `controlsHost` выходил из
`.toolcall-container.thinking-container` и заякоривался на
внешнем `.segment-assistant`. В streaming-чате сегмент короткий →
нормально; в сохранённом чате сегмент DIV тянется на всё сообщение
вертикально → shadow toolbar рисует огромную пустую колонку.

**Фикс:** удалил hop из `controlsHost` полностью. Копия в
thinking-widget теперь скипается через `arenaWhyUserAuthored`
(matched=true, reason `kimi:thinking-widget@DIV`). `mountControls`
уже умеет для matched=true добавлять fingerprint в
`dismissedControls` и делать early return — видимый sibling
`.segment-content` PRE (уже отдельный parsed candidate) становится
единственным хостом toolbar без визуальных side-effects.

## 2. z.ai — toolbar всё ещё в конце сообщения

**Симптом:** "На Z.ai ничего не изменилось, всё тоже самое,
также отображается в конце сообщения, а не под вызовом в сообщении."

**Причина:** walker v0.14.18 искал Kimi-специфичные токены классов
(`.code-block`, `.syntax-highlighter`, `.segment-code`), которых на
z.ai нет. Walker возвращал null → toolbar оставался на внешнем
`.markdown-prose`.

**Фикс:** walker расширен: также ищет `<pre>`, `<code>`,
`[class*="language-"]`, `[class*="hljs"]` **И** требует, чтобы
`textContent` элемента содержал `function_call_start` /
`function_call_end`. Cap 200 → 300 узлов. На хите — `.closest('pre,
[class*="code"], [class*="language-"]')`, чтобы toolbar сидел под
всем блоком, а не тесно к одному `<code>` span.

## 3. Arena.ai — User всё ещё ловится, AI не ловится

**Симптом:** "На Arena.ai ничего не изменилось, кроме
displayName, который стал нормальным, а user также ловит и AI не
ловит."

**Причина:** v0.14.18 ключился на `.chat-user` / `.chat-assistant`
— это классы **z.ai**, не arena.ai. Live-скан arena.ai показывает
реальные wrapper-токены — это Tailwind design-system классы:
`bg-surface-raised` (AI, часто с `w-fit`) и `bg-surface-primary` +
`no-scrollbar` (user pill). Также присутствует явный AI-маркер
`#response-content-container`.

**Фикс:** переписал ветку `arenaai` в `arenaWhyUserAuthored`:
- AI fast-return: `node.closest('#response-content-container, [class*="bg-surface-raised"]')` → not-user.
- User marker: `.no-scrollbar` / `[class*="user-message"]` / `[class*="chat-user"]` ancestor, чей ancestor также несёт `bg-surface-primary` или `no-scrollbar`. Reason: `arenaai:user-wrap@DIV`.

Плюс новый **диагностический блок `arenaai_hint`** в каждом
snapshot (заполняется только на `arena.ai`): `surface`
(agent/chat/battle/other), `response_container_ancestor`,
`bg_surface_raised_ancestor`, `bg_surface_primary_ancestor`,
полная цепочка wrapper-классов (топ 12). Если `/agent/` снова
промахнётся в следующем туре — причина будет видна из scan-report
без DevTools.
...
- Новый `tests/test_chat_extension_v0_14_19.py` — 11 asserts на
  все 3 фикса + regression guards для v0.14.16..v0.14.18.
- Перепиннены `tests/test_chat_extension_assets.py`,
  `tests/test_chat_extension_adapter_flow.py` и исторические
  `tests/test_chat_extension_v0_14_{7..18}.py` на `0.14.19`.
- Ожидаемо: 2554 passed (2543 → 2554).

## Всё ещё отложено на v4.50.10

- Multi-block per message (1 toolbar на host; на OpenRouter 6
  tool-вызовов в одном сообщении получают только 1 toolbar).
- Одинаковый call_id когда ИИ не инкрементирует.
- T3 chat дубль toolbar во время stream.
- Mistral flaky mount.
- Arena.ai `/agent/` fine-tune (жду scan-report с новым
  `arenaai_hint` блоком).
