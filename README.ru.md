...
### 🆕 Что нового в v3.30.0

- **Появился side-panel result inspection** — browser chat extension теперь умеет смотреть сохранённые execution results отдельно от payloads.
- **Фильтрация history стала сильнее** — side panel теперь фильтрует по kind, site и adapter, что делает multi-site debugging практичнее.
- **ChatGPT-oriented adapter flow стал плотнее** — latest-candidate helpers, node-id extraction и более строгая assistant-filtering логика улучшают поведение на больших chat DOM без изменения bridge-side protocol.
- **Extension debug loop продолжает усиливаться** — payload inspection, result inspection, copy actions, replay и filtering теперь работают вместе в одной browser surface.
