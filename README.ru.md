...
### 🆕 Что нового в v3.69.0

- **Content-script version diagnostics**: Scan Page теперь показывает manifest/content/insert-script versions, чтобы stale tab injections были видны после reload extension.
- **Composer diagnostics**: Scan Page включает тип composer и Auto insert plan (`rich_textarea`, `prose_mirror`, `auto_plan`).
- **Insert status version tag**: toolbar insert/send сообщения показывают активную версию extension/content-script.
