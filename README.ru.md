...
- **ReAct/reflection runtime стабилизирован** — `/v1/react`, `/v1/reflect`, `react.run` и `reflect.run` теперь корректно читают конфиг установленного bridge после AppKey migration, что исправляет live runtime issue, найденный сразу после `v3.5.0`.
