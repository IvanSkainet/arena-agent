...
- **Появилось OCR-to-window resolution** — `POST /v1/desktop/resolve_text_target` и MCP `desktop.resolve_text_target` теперь связывают видимый текст не только с координатами клика, но и с содержащим его окном.
- **`desktop.focus` теперь умеет работать от text query** — агент может сфокусировать окно по видимому тексту, а не только по id или window metadata.
- **`desktop.window_action` теперь умеет работать от text query** — placement и window-management action'ы могут целиться в окно, содержащее распознанный текст.
- **Desktop workflows становятся composable** — OCR, windows, displays, focus и actions теперь сцепляются напрямую, а не живут изолированными примитивами.
- **616 тестов проходят**, без регрессий.
