...
| **Встроенные AI-инструменты** | MCP-сервер с 50+ инструментами, интеграция BrowserAct, репозиторий навыков Superpowers (14 навыков), стелс-браузер Camoufox |
...
### 🆕 Что нового в v3.15.0

- **Появились API для mission composition** — у Arena теперь есть first-party mission endpoints для listing templates, composing planner-backed mission drafts, creating reusable mission artifacts и запуска missions через bridge.
- **Mission composition теперь доступна и через MCP** — `mission.templates`, `mission.compose`, `mission.create` и `mission.run` позволяют агентным фронтендам работать с missions без собственного REST glue.
- **Следующий большой roadmap-блок реально начался** — после закрытия текущего D2/D3 desktop maturity среза Arena теперь вошла в слой deeper agent loops / mission composition, а не только подготавливается к нему.
- **Desktop maturity никуда не делась** — новая mission-работа строится поверх уже завершённого OCR/window/focus/semantic desktop stack, а не вместо него.
- **622 теста проходят**, без регрессий.
