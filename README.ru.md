...
- **Bounded ReAct loop (`A2`)** — в Arena появился `POST /v1/react`, который запускает безопасный reason → act → observe цикл поверх planner'а, memory profiles, system checks, task state, file watchers и опциональных browser HEAD checks.
- **Reflection endpoint (`A3`)** — `POST /v1/reflect` критикует предыдущий run и возвращает positives, concerns, missing evidence, confidence и suggested next steps.
- **MCP `react.run` + `reflect.run`** — те же agentic surfaces теперь доступны и через MCP-слой Arena.
- **OpenAPI обновлён** — `/v1/react` и `/v1/reflect` теперь документированы в публичной API-спеке.
- **586 тестов проходят**, без регрессий.
