...
- **Sidepanel Command Center lite**: history теперь показывается compact cards с kind/status/count badges вместо raw log list.
- **Чище status area**: Test Bridge и Policies показывают короткие summaries, а raw JSON остаётся доступен в selected result panel.
- **Больше metadata в cards**: sidepanel cards показывают adapter, site, tools и replay/copy actions там, где это уместно.
