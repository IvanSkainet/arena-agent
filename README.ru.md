...
### 🆕 Что нового в v3.14.0

- **Появились high-level text-driven desktop workflows** — `POST /v1/desktop/text_action` и MCP `desktop.text_action` теперь умеют разрешать видимый текст и потом фокусировать окно, кликать или запускать semantic window action в одном workflow.
- **`D2 / D3` desktop maturity теперь можно считать завершённой достаточно, чтобы двигаться дальше** — у bridge уже есть более сильный OCR ranking, click-by-text, OCR-to-window resolution, query-driven focus/window actions, display-aware placement, snap/tile placement и non-interactive KDE/Wayland window control.
- **Продукт может двигаться дальше по roadmap** — после этой вехи центр тяжести можно переносить на deeper agent loops и workspace UX, а не на базовую desktop plumbing.
- **И без возврата KWin focus-stealing regression** — более богатые workflow остаются на non-interactive KDE/Wayland path.
- **619 тестов проходят**, без регрессий.
