...
| **14 desktop + 4 control эндпоинта** | Автоматизация рабочего стола Wayland/X11: скриншот, display/output discovery, OCR, text-target detection, OCR-to-window resolution, semantic click-by-text, клик, layout-safe ввод, нажатие клавиш, движение мыши, список окон, active window, focus, window actions, плюс pause/resume/revoke/status для control lease |
...
### 🆕 Что нового в v3.13.1

- **OCR-to-window resolution стала практичнее на live KDE/Wayland bridge** — query-driven text workflow теперь может ограничивать OCR активным окном вместо сканирования всего desktop каждый раз.
- **`desktop.focus` и `desktop.window_action` лучше работают от text query** — когда нужный текст уже находится в активном окне, bridge разрешает его с меньшим шумом и меньшим риском timeout.
- **Composable desktop workflows продолжают усиливаться** — OCR, window resolution, display-awareness, focus, snap/placement action'ы и click-by-text теперь складываются в более полезные high-level flow.
- **И без возврата KWin focus-stealing regression** — более богатое text-driven поведение остаётся на non-interactive KDE/Wayland path.
- **617 тестов проходят**, без регрессий.
...
### Автоматизация рабочего стола (14 эндпоинтов + 4 control lease эндпоинта)
