...
### 🆕 Что нового в v3.27.0

- **Появился extension history replay** — side panel теперь умеет переисполнять сохранённые preview/execute payloads из structured history для debugging и повторных запусков.
- **Появился Insert & Submit flow** — detected `arena-tool` blocks теперь могут вставлять результаты и пытаться отправлять их через adapter-aware composer/button helpers.
- **Extension всё больше становится usable daily bridge UI** — popup config, side panel, structured history, replay actions и более сильный ChatGPT-oriented insertion path теперь существуют вместе.
- **Это продолжает двигать Arena выше tool-aware чатов** — browser-chat путь становится практичным local execution layer для обычных web chats.
