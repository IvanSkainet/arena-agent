...
- **Добавлены KWin active-window retry** — `/v1/desktop/active_window` теперь несколько раз кратко перепроверяет `queryWindowInfo`, прежде чем падать в fallback, что сглаживает редкий live-case с пустым ответом на Plasma/Wayland.
