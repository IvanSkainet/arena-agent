...
### 🆕 Что нового в v3.12.0

- **Появились snap/tile-style placement action'ы** — `POST /v1/desktop/window_action` и MCP `desktop.window_action` теперь поддерживают `snap_left`, `snap_right` и quadrant snap'ы.
- **Размещение окон стало более policy-driven** — мост уже умеет выражать типовые tiling-намерения, а не заставляет каждого агента вручную считать координаты относительно монитора.
- **Dry-run остаётся first-class** — semantic placement action'ы заранее показывают exact geometry, которая будет применена.
- **И без возврата KWin-регрессии** — более богатая placement-семантика остаётся на non-interactive KDE/Wayland path и не возвращает focus-stealing запросы.
- **613 тестов проходят**, без регрессий.
...
| `POST` | `/v1/desktop/window_action` | Двигать, ресайзить, центрировать, снапать в типовые tiling-позиции, переносить на другой display, минимизировать, максимизировать, восстанавливать, закрывать или переключать fullscreen у окна, найденного по id или semantic-фильтрам; поддерживает `dry_run` |
