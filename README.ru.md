...
### 🆕 Что нового в v3.33.0

- **Исправлена надёжность popup в browser extension** — сохранение bridge URL/token теперь использует callback-compatible runtime messaging и явную обработку ошибок вместо зависания на `Loading...`.
- **Проверка сохранения** — после Save popup сразу перечитывает данные из extension storage, чтобы пользователь видел, сохранилась ли конфигурация.
- **Более понятная диагностика** — ошибки загрузки config/history теперь показываются прямо в popup.
