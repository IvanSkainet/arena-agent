...
### 🆕 Что нового в v3.11.0

- **Появилось display-aware размещение окон** — `POST /v1/desktop/window_action` и MCP `desktop.window_action` теперь поддерживают `center` и `move_to_display` поверх уже существующих low-level move/resize action'ов.
- **Dry-run стал полезнее** — для display-aware action'ов Arena теперь возвращает planned geometry и информацию о source/target display до реального выполнения.
- **Desktop control становится более семантическим** — мост движется от сырых координат к политикам вроде «перемести окно на этот монитор» и «отцентрируй окно на дисплее».
- **И без возврата KWin-регрессии** — новый placement behaviour остаётся на non-interactive KDE/Wayland path и не возвращает focus-stealing запросы.
- **612 тестов проходят**, без регрессий.
...
| `POST` | `/v1/desktop/window_action` | Двигать, ресайзить, центрировать, переносить на другой display, минимизировать, максимизировать, восстанавливать, закрывать или переключать fullscreen у окна, найденного по id или semantic-фильтрам; поддерживает `dry_run` |
