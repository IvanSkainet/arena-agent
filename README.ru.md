...
| **Встроенные AI-инструменты** | MCP-сервер с 68 инструментами, интеграция BrowserAct, репозиторий навыков Superpowers (14 навыков), стелс-браузер Camoufox |
...
### 🆕 Что нового в v3.24.0

- **Появился backend foundation для chat bridge extension** — у Arena теперь есть extension-facing execution surfaces поверх существующего bridge core.
- **Новые extension endpoints** — `GET /v1/extension/policies`, `POST /v1/extension/preview` и `POST /v1/extension/execute` позволяют браузерному расширению валидировать, классифицировать и исполнять структурированные Arena tool blocks из обычных чат-сайтов.
- **Добавлен MVP scaffold браузерного расширения** — в репозитории теперь есть первый прототип `chat_extension/` с generic fenced-block detection для `arena-tool` payloads и localhost bridge execution.
- **Продукт получил путь выше нативно tool-aware чатов** — Arena движется к тому, чтобы стать universal local execution layer для обычных browser chats, а не только сред с built-in MCP или code execution.
- **633 теста проходят**, без регрессий.
...
| `GET` | `/v1/mission/family?name=…` | Посмотреть всю mission family, rooted в mission chain, включая members, leaves и branch summaries |
...
| `GET` | `/v1/mission/schedules?action=&enabled=&due_only=&limit=` | Посмотреть recurring mission schedules и due-state summaries |
| `GET` | `/v1/mission/schedules/state` | Посмотреть состояние recurring mission schedule worker |
...
| `POST` | `/v1/mission/schedules` | Создать или обновить recurring mission schedule |
| `DELETE` | `/v1/mission/schedules` | Удалить recurring mission schedule по id |
| `POST` | `/v1/mission/schedules/tick` | Вручную исполнить due mission schedules |
| `GET` | `/v1/extension/policies` | Посмотреть execution policies, risk classes и payload examples для browser chat extension |
| `POST` | `/v1/extension/preview` | Валидировать и классифицировать структурированный `arena-tool` payload перед выполнением |
| `POST` | `/v1/extension/execute` | Выполнить одобренный `arena-tool` payload через локальный bridge |
...
| **Workspace** | Companion-style поверхность для активного profile context, planner output, bounded ReAct runs, reflection, управления file watcher'ами, profile notes, important lessons, recent activity и mission loop studio для lineage / family / schedules / schedule-state / follow-up / iterate flows |
