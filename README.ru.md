...
### 🆕 Что нового в v3.29.0

- **Появились side-panel replay и payload inspection** — browser chat extension теперь умеет смотреть сохранённые payloads, переигрывать preview и переигрывать execute прямо из side-panel history.
- **ChatGPT-oriented detection стал сильнее** — fingerprinting assistant messages и более строгая фильтрация candidate nodes уменьшают шум на больших chat DOM.
- **Extension debug loop стал практичнее** — popup + side panel + history filters + payload inspection теперь образуют более сильный browser-chat workflow.
- **Это продолжает двигать Arena к real universal chat execution layer** — extension становится полезным не только для one-shot запусков, но и для повторяющихся inspect/replay/debug циклов.
- **637 тестов проходят**, без регрессий.
