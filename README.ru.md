...
### 🆕 Что нового в v3.75.0

- **Более консервативная lifecycle grouping**: sidepanel command cards теперь группируют только реальные multi-stage flows, а не повторные `detected` дубли.
- **Чище badges в cards**: grouped command cards показывают `command`, lifecycle и event count без дублирующих status badges.
- **Filters стали live**: kind changes применяются сразу, а site/adapter filters auto-refresh с debounce или по Enter.
