...
### 🆕 Что нового в v3.65.0

- **Settled insert verification**: Insert/Send теперь показывают success только если composer всё ещё содержит маркер вставки после короткой паузы.
- **Более безопасный Send flow**: Insert & Submit больше не нажимает Send после неподтверждённой или откатанной вставки.
- **Direct DOM pre-wrap diagnostic**: добавлена стратегия `directDomPreWrap` для проверки быстрой многострочной вставки без `execCommand` с сохранением визуальных переносов.
