...
### 🆕 Что нового в v3.37.0

- **Стабильный smoke test для chat extension** — copied Arena/JSONL instructions теперь по умолчанию используют `sys.status`, который работает даже на пустой установке.
- **Structured mission errors** — mission tools сохраняют JSON body для 404 вроде `mission not found`, а не возвращают голый `HTTPError`.
- **Аккуратнее placement в AI Studio** — nested rendered-code matches сходятся к ближайшему `pre` / `code` перед mounting controls.
