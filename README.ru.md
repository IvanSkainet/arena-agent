...
### 🆕 Что нового в v3.2.14

- **Чистка репозитория** — из проекта удалены мёртвые per-release scratch файлы (`release_v*.md`) и устаревший helper `bump_v323.py`.
- **Улучшена release hygiene** — будущие `release_v*.md` и `bump_v*.py` теперь игнорируются, а `RELEASE.md` явно говорит использовать временные/untracked notes files для GitHub release.
