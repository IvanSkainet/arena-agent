...
- **Insert/submit history events**: toolbar Insert и Send теперь записывают `insert` / `submit` events с timing и version diagnostics.
- **Более полный command lifecycle**: sidepanel lifecycle cards теперь могут показывать `detected → previewed → executed → inserted → submitted`.
- **Insertion metadata в cards**: insert/submit cards показывают strategy, total time, verification time и активные content/insert script versions.
