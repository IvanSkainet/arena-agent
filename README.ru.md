...
- **Insert strategy diagnostics**: настройки extension теперь поддерживают стратегии `auto`, `nativeInsertText`, `paragraphFallback` и `pasteOnly`.
- **Gemini speed A/B testing**: toolbar status показывает strategy/timing вроде `via nativeInsertText in 42ms`.
- **Manual и auto flows выровнены**: auto insert/submit использует ту же configured strategy.
