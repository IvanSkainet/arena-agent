...
- **Memory Profiles (`M3`)** — память теперь сегментируется по профилям во всех основных слоях: REST, MCP, runtime и dashboard. Можно держать отдельные пространства вроде `default`, `personal`, `projects/<name>`, `code`, `browser` или собственные profile id.
- **Profile-aware memory API** — `/v1/memory`, `/v1/recall` и `/v1/recall/digest` теперь принимают `profile`, а ответы списка памяти возвращают и список обнаруженных профилей.
- **Profile-aware MCP memory tools** — `mem.set`, `mem.get`, `memory.recall`, `memory.digest`, `memory.export` и `memory.import` теперь понимают профили.
- **Автоматическая миграция схемы** — старые single-profile memory DB автоматически переезжают в профиль `default` без потери фактов.
- **Dashboard memory profile controls** — вкладки Memory и Recall теперь работают с активным профилем памяти.
- **566 тестов проходят**, без регрессий.
