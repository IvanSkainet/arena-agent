...
- **Усилен KWin active-window detection** — `/v1/desktop/active_window` теперь принимает любой непустой ответ `queryWindowInfo` от KWin, поэтому Plasma helper/focus-proxy окна больше не заставляют bridge падать в fallback через `xdotool` только из-за отсутствия `caption` или `uuid`.
- **551 тест проходят**, без регрессий.
