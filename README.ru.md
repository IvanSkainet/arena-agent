...
### 🆕 Что нового в v3.64.0

- **Direct DOM block insertion**: добавлена стратегия `directDomBlocks` без `execCommand`, которая вставляет отдельный block на каждую строку вместо plain text nodes.
- **Диагностика структуры Gemini**: теперь можно сравнить `directDomText` и `directDomBlocks`, сохранив быстрый 1ms path и проверив многострочную структуру composer.
- **Auto не изменён**: default strategy остаётся консервативной, пока block-based путь не подтверждён в целевых чатах.
