...
- **Command lifecycle cards**: sidepanel теперь группирует связанные command events в одну card с цепочкой `detected → previewed → executed`, когда kind filter не выбран.
- **Audit-safe replay indexes**: actions в filtered/grouped history используют исходный stored history index, поэтому Replay Preview/Execute попадает в правильный payload.
- **Flow badges**: command cards показывают lifecycle и event-count badges, а raw per-event history остаётся доступна через kind filters.
