...
### 🆕 Что нового в v3.2.10

- **Нативный KWin fallback для active window** — если прямой `queryWindowInfo` от KWin отменяется, `/v1/desktop/active_window` теперь сначала использует уже рабочий нативный KWin window list и берёт из него активное окно, а не сразу падает в `xdotool`.
- **553 теста проходят**, без регрессий.
