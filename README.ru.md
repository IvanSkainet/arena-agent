...
### 🆕 Что нового в v3.26.0

- **Появился chat extension side panel** — у extension scaffold теперь есть более богатая panel для bridge status и execution history, а не только popup.
- **Путь ChatGPT adapter стал сильнее** — в adapter scaffold теперь есть composer-aware insertion helpers, с более нормальным ChatGPT/ChatGPT.com path и generic fallback insertion.
- **Detection/debug UX стал лучше** — detections, previews и executions теперь записываются как structured history entries, а detected blocks умеют открывать side panel напрямую.
- **Browser-chat слой продолжает становиться реальным** — у Arena теперь есть backend execution surfaces, popup configuration, side-panel UX и первый более сильный adapter-aware insertion path.
