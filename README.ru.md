...
- **Появился mission lifecycle v3** — поверх mission status/report/history/create/run/rerun у Arena теперь есть mission catalog и mission recovery surfaces.
- **Persisted missions теперь можно фильтровать и восстанавливать** — агент может запросить mission catalog, увидеть failed steps, получить план rerun и при необходимости собрать follow-up mission из recovery context через REST и MCP.
- **Mission/orchestration блок продолжает углубляться** — bridge двигается от «умеет создать и запустить mission» к «умеет проходить цикл mission state → recovery → next mission composition».
- **Desktop maturity никуда не делась** — этот orchestration-слой продолжает строиться поверх уже завершённого OCR/window/focus/semantic desktop stack.
- **624 теста проходят**, без регрессий.
