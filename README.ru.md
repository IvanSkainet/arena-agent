...
### 🆕 Что нового в v3.20.0

- **Появился mission lifecycle v4** — поверх mission catalog/recovery/history/report/create/run/rerun у Arena теперь есть mission follow-up и mission iteration loop surfaces.
- **Persisted missions теперь могут автоматически кормить следующую mission** — агент может взять артефакты прошлой mission, собрать follow-up mission и при необходимости сразу создать и запустить её через REST и MCP.
- **Mission/orchestration блок продолжает углубляться** — bridge двигается от «умеет восстановить mission» к «умеет итерировать mission state в следующий agentic mission loop».
