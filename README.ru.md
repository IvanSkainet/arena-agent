...
### 🆕 Что нового в v3.23.0

- **Появился automatic mission schedule worker** — поверх family/lineage/catalog/recovery/follow-up/iteration/history/report/create/run/rerun у Arena теперь есть фоновое recurring mission execution.
- **Recurring mission orchestration теперь bridge-managed** — агент может смотреть состояние schedule worker, задавать recurring schedules и полагаться на фоновый worker, а не только на ручной tick.
- **Workspace mission loop studio снова стал глубже** — во вкладке Workspace теперь можно смотреть schedule state рядом с family и schedule surfaces.
- **Mission/orchestration блок продолжает углубляться** — bridge двигается от «умеет задавать recurring mission families» к «умеет крутить recurring agentic mission loops».
- **629 тестов проходят**, без регрессий.
