...
### 🆕 Что нового в v3.61.0

- **Доступ extension к публичным туннелям исправлен обобщённо**: host permissions покрывают Tailscale Funnel (`https://*.ts.net/*`) и Cloudflare Quick Tunnels (`https://*.trycloudflare.com/*`).
- **Без приватного tailnet hardcoding**: случайно закоммиченный machine-specific Tailnet permission удалён.
- **Размер Cloudflared исправлен**: installer/docs теперь описывают optional `cloudflared` download как ~50 МБ, не ~40 МБ.
