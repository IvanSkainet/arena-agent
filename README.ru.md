...
### 🆕 Что нового в v3.46.0

- **Gemini Web detection стабилизирован** — JSONL из composer/user-input больше не должен получать controls.
- **Controls восстанавливаются после DOM churn** — Gemini re-render может вернуть inline controls без duplicate history detections.
- **Preview стал понятнее** — inline Preview показывает dry-run calls, tool names и approval state.
