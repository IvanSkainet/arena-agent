...
### 🆕 Что нового в v3.38.0

- **Fix layout regression в AI Studio** — controls для rendered JSONL теперь крепятся к конкретным `pre` / `code` blocks после удаления ancestor containers.
- **Повторяющиеся блоки обрабатываются отдельно** — одинаковые JSONL blocks в разных ответах больше не схлопываются в один fingerprint.
- **Понятные Bridge URL ошибки** — `127.0.0.1:8765` нормализуется в `http://127.0.0.1:8765`, а fetch failures показывают реальную ошибку вместо `unknown`.
