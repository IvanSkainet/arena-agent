...
### 🆕 Что нового в v3.67.0

- **Recommended Auto UX**: popup теперь помечает `Auto` как recommended insert strategy, а ручные стратегии — как debug options.
- **Понятнее insert status**: toolbar теперь явно пишет, какую конкретную стратегию выбрал Auto, например `directDomPreWrap`.
- **Более полезные ошибки**: insert failures включают compact attempt summary, чтобы отличать ignored, changed-but-unverified и settled paths.
