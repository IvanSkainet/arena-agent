...
### 🆕 Что нового в v3.71.0

- **Тише scan history**: повторные Scan Page результаты теперь агрегируются в одну строку с `×N`, а не засоряют sidepanel history.
- **Общая history aggregation**: `detected` и `scan` используют один short-window aggregation path, а `preview` и `execute` остаются явными событиями.
- **Чище baseline для sidepanel**: повторную диагностику легче читать перед Command Center card UI.
