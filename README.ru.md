...
| **Встроенные AI-инструменты** | MCP-сервер с 67 инструментами, интеграция BrowserAct, репозиторий навыков Superpowers (14 навыков), стелс-браузер Camoufox |
...
### 🆕 Что нового в v3.22.0

- **Появились mission family surfaces** — поверх lineage/catalog/recovery/follow-up/iteration/history/report/create/run/rerun у Arena теперь есть явные mission family summaries и mission schedule surfaces.
- **Persisted missions теперь можно группировать и гонять по расписанию** — агент может смотреть всю mission family, задавать recurring schedules и вручную тикать due schedules через REST и MCP.
- **Workspace mission loop studio стал глубже** — во вкладке Workspace теперь можно смотреть family state и управлять mission schedules, а не только lineage/follow-up/iterate flows.
- **Mission/orchestration блок продолжает углубляться** — bridge двигается от «умеет сопровождать mission chains» к «умеет сопровождать recurring mission families».
- **626 тестов проходят**, без регрессий.
