...
| **10 desktop + 4 control эндпоинта** | Автоматизация рабочего стола Wayland/X11: скриншот, OCR, text-target detection, клик, layout-safe ввод, нажатие клавиш, движение мыши, список окон, active window, focus, плюс pause/resume/revoke/status для control lease |
...
- **Desktop OCR + text-target detection (`D1`)** — в Arena появились `POST /v1/desktop/ocr` и `POST /v1/desktop/find_text`, которые возвращают распознанный текст, bounding boxes, confidence и click-ready coordinates.
- **MCP desktop OCR tools** — добавлены `desktop.ocr` и `desktop.find_text`, чтобы агентные фронтенды могли вызывать OCR и text targeting через MCP.
- **OpenAPI и prompt docs обновлены** — OCR/text-target detection теперь документированы в публичной API-спеке и AI prompt template.
- **592 теста проходят**, без регрессий.
...
```
        ...
        │   │ 36 эндпоинтов│  │ 10+4 эндпоинта│  │  + Log + Disk Mon.  │      │
        │   └──────────────┘  └──────────────┘  └──────────────────────┘      │
        │                                                                     │
        └─────────────────────────────────────────────────────────────────────┘
                                           │
...
### Автоматизация рабочего стола (10 эндпоинтов + 4 control lease эндпоинта)
...
| `POST` | `/v1/desktop/ocr` | Запустить OCR по свежему desktop screenshot и вернуть слова, полный текст, confidence и bounding boxes |
| `POST` | `/v1/desktop/find_text` | Найти текст на текущем рабочем столе и вернуть matching bounding boxes плюс click-ready center coordinates |
...
| **Workspace** | Companion-style поверхность для активного profile context, planner output, bounded ReAct runs, reflection и управления file watcher'ами |
