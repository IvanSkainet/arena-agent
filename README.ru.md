...
### 🆕 Что нового в v3.31.0

- **Основа совместимости с MCP SuperAssistant-style JSONL** — browser chat extension теперь распознаёт fenced `jsonl` function-call блоки и нормализует их в Arena tool payload.
- **Расширено покрытие chat adapter registry** — добавлено первое покрытие для Gemini / AI Studio, Perplexity, Grok, OpenRouter, DeepSeek, Kimi, Qwen и generic fallback.
- **Настройки режимов выполнения** — в popup появились toggles для auto-preview, safe auto-execute, auto-insert и auto-submit; ручное подтверждение остаётся режимом по умолчанию.
- **Bridge-native execution остаётся ядром** — совместимые входные форматы всё равно проходят через Arena `/v1/extension/*`, policy и audit, а не через отдельный локальный executor.
