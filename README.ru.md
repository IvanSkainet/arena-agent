...
| **190+ method/path маршрутов** | Публичные REST, MCP, gateway, dashboard, observability, desktop, browser, admin и compatibility-поверхности на одном порту |
...
| **8 desktop + 4 control эндпоинта** | Автоматизация рабочего стола Wayland/X11: скриншот, клик, layout-safe ввод, нажатие клавиш, движение мыши, список окон, active window, focus, плюс pause/resume/revoke/status для control lease |
...
- **Исправления KDE/Wayland desktop** — `/v1/desktop/active_window` теперь использует современный KWin DBus `queryWindowInfo`, а `/v1/desktop/windows` больше не сдаётся только потому, что у systemd-user сервиса не были выставлены `XDG_CURRENT_DESKTOP` или `XDG_SESSION_TYPE`.
- **Усилен session bootstrap** — bridge теперь умеет по возможности восстанавливать Linux desktop metadata (`XDG_SESSION_TYPE`, `XDG_CURRENT_DESKTOP`, `DESKTOP_SESSION`), что улучшает `/v1/capabilities` и надёжность desktop automation.
- **Очистка aiohttp AppKey** — состояние приложения переведено с raw string keys на нормальные `aiohttp.web.AppKey`, поэтому runtime и тесты больше не шумят предупреждениями.
- **Полировка installer'а** — `install.sh` теперь сохраняет desktop session metadata в сгенерированный systemd user unit, если эти значения доступны во время установки.
- **549 тестов проходят**, без регрессий.
...
```
        ...
        │   │ 36 эндпоинтов│  │ 8+4 эндпоинта│  │  + Log + Disk Mon.   │      │
        │   └──────────────┘  └──────────────┘  └──────────────────────┘      │
        │                                                                     │
        └─────────────────────────────────────────────────────────────────────┘
                                           │
...
| `PATCH` | `/v1/fs/edit` | Find-and-replace в текстовом файле (хирургическое редактирование, без перезагрузки файла). Body: `{"path": "...", "old_text": "foo()", "new_text": "bar()", "replace_all": false}` |
...
### Автоматизация рабочего стола (8 эндпоинтов + 4 control lease эндпоинта)
