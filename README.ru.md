...
| **13 desktop + 4 control эндпоинта** | Автоматизация рабочего стола Wayland/X11: скриншот, display/output discovery, OCR, text-target detection, semantic click-by-text, клик, layout-safe ввод, нажатие клавиш, движение мыши, список окон, active window, focus, window actions, плюс pause/resume/revoke/status для control lease |
...
| **Встроенные AI-инструменты** | MCP-сервер с 40+ инструментами, интеграция BrowserAct, репозиторий навыков Superpowers (14 навыков), стелс-браузер Camoufox |
...
### 🆕 Что нового в v3.10.0

- **Window actions стали полнее** — `POST /v1/desktop/window_action` и MCP `desktop.window_action` теперь поддерживают `maximize`, `unmaximize` и `close` помимо move/resize/minimize/restore/fullscreen.
- **KWin/Wayland window actions подтверждены live** — maximize, unmaximize и close теперь реально работают через тот же non-interactive journal-backed KWin path, что и остальная desktop maturity линия.
- **Проверка maximize стала умнее** — если KWin геометрически разворачивает окно, но не выставляет maximized-флаги в listing payload, Arena всё равно корректно верифицирует действие по before/after geometry.
- **Desktop maturity продолжает складываться в стек** — displays, filtered windows, focus dry-run, OCR ranking, click-by-text и более богатые window actions теперь образуют более сильный desktop-control слой.
- **609 тестов проходят**, без регрессий.
...
```
        ...
        │   │ 36 эндпоинтов│  │ 13+4 эндпоинта│  │  + Log + Disk Mon.  │      │
        │   └──────────────┘  └──────────────┘  └──────────────────────┘      │
        │                                                                     │
        └─────────────────────────────────────────────────────────────────────┘
                                           │
...
### Автоматизация рабочего стола (13 эндпоинтов + 4 control lease эндпоинта)
...
| `POST` | `/v1/desktop/focus` | Сфокусировать окно по id или semantic-фильтрам вроде title/class/display; поддерживает `dry_run` разрешение цели до реального focus |
| `POST` | `/v1/desktop/window_action` | Двигать, ресайзить, минимизировать, максимизировать, восстанавливать, закрывать или переключать fullscreen у окна, найденного по id или semantic-фильтрам; поддерживает `dry_run` |
...
| **Workspace** | Companion-style поверхность для активного profile context, planner output, bounded ReAct runs, reflection, управления file watcher'ами, profile notes, important lessons и recent activity |
