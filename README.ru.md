...
### 🆕 Что нового в v3.39.0

- **Placement controls в AI Studio** — content script теперь реально использует helper размещения после code block, а не append в случайный контейнер.
- **Latest-only controls mode** — в popup добавлен переключатель: показывать controls только для последнего видимого tool block или для всех видимых.
- **Надёжнее Insert & Submit** — submit ждёт/retry, пока composer активирует кнопку отправки после вставки.
