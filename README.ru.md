...
### 🆕 Что нового в v3.41.0

- **Cleanup для latest-only** — stale/orphan toolbars удаляются, а latest-only mode применяется на каждом scan.
- **Настройки применяются сразу** — Save в popup уведомляет активную вкладку AI Studio без refresh.
- **Toolbar polish** — короткий label `Copy` и компактная кнопка закрытия.
