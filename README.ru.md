...
### 🆕 Что нового в v3.28.0

- **Путь ChatGPT-oriented adapter стал сильнее** — extension теперь fingerprint'ит assistant messages, раньше фильтрует candidate nodes и использует более надёжный ChatGPT/ChatGPT.com detection path.
- **Insert & Submit стал реальнее** — adapter-aware submit-button discovery теперь реально встроен в extension flow, а не только generic insertion.
- **Extension replay/debug loop стал сильнее** — popup, side panel, structured history, replay actions и throttled detection теперь образуют более практичный browser-chat workflow.
- **Browser-chat путь продолжает укрепляться без раскола архитектуры** — выполнение по-прежнему идёт через локальный Arena bridge, но extension UX уже заметно ближе к daily-use состоянию.
- **635 тестов проходят**, без регрессий.
