...
- **History spam для detected уменьшен**: повторные detections deduplicate по fingerprint/site/adapter/detail.
- **Счётчик повторов сохранён**: duplicate detected rows обновляются до `×N`, не засоряя popup/sidepanel history.
- **История действий остаётся явной**: `preview`, `execute` и `scan` entries не схлопываются.
