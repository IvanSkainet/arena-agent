...
- **Safe editor foundation (`F4`)** — `PATCH /v1/fs/edit` теперь поддерживает `preview: true`, поэтому агент может сначала запросить неразрушающий diff перед применением правки.
- **Confirm + rollback workflow** — в Arena появились `POST /v1/fs/edit/apply`, `POST /v1/fs/edit/rollback` и MCP-компаньоны `fs.edit_apply` / `fs.edit_rollback`.
- **Conflict-aware safety** — apply/rollback теперь проверяют, не изменился ли файл после preview/apply, и отказываются от небезопасного перезаписывания, если не указан явный force.
- **582 теста проходят**, без регрессий.
