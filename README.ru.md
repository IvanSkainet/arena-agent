...
- **Нативный KDE window listing реально починен** — `/v1/desktop/windows` теперь держит временный KWin-скрипт максимально простым и выгружает его из Python, а не из тела самого скрипта. Это исправляет live-case на Plasma, где `loadScript` возвращал `0` и bridge всегда падал в fallback через `xdotool`.
- **Уточнено KDE backend reporting** — `/v1/capabilities` теперь корректно различает `kwin_journal` для списка окон и `kwin_dbus` для active-window discovery.
- **550 тестов проходят**, без регрессий.
