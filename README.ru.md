...
### 🆕 Что нового в v3.18.0

- **Появился mission lifecycle v2** — у Arena теперь есть mission status, mission report, mission history и mission rerun surfaces, а не только compose/create/run.
- **Mission artifacts стали inspectable и reusable** — агенты могут смотреть прошлые run'ы, step logs, забирать report и перезапускать missions или failed steps через REST и MCP.
- **Mission/orchestration блок продолжает углубляться** — bridge двигается от «умеет создать и запустить mission» к «умеет сопровождать и итерировать полный mission lifecycle».
- **Desktop maturity никуда не делась** — новый mission-слой продолжает строиться поверх уже завершённого OCR/window/focus/semantic desktop stack.
