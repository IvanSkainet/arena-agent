...
- **Built-in Planner (`A1`)** — в Arena появился first-party planner endpoint `POST /v1/plan` и MCP tool `plan.create`, которые возвращают структурированный план: steps, risks, required tools, next action и suggested Memory Profile.
- **Planner heuristics** — первая версия planner'а различает goals в доменах code/browser/desktop/system/task и помечает более рискованные шаги как требующие подтверждения.
- **OpenAPI обновлён** — `/v1/plan` теперь документирован в публичной API-спеке.
- **575 тестов проходят**, без регрессий.
