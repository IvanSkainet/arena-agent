...
- **Появились реальные window actions** — `POST /v1/desktop/window_action` и MCP `desktop.window_action` позволяют агентам двигать, ресайзить, минимизировать, восстанавливать и переключать fullscreen у desktop-окон.
...
- **KWin/Wayland остаётся non-interactive** — UUID-style Wayland окна можно манипулировать через journal-backed KWin script path, не возвращая старый класс focus-stealing регрессий.
- **Hotfix:** успешные KWin window actions больше не возвращают ложный `window_not_found` в result metadata.
