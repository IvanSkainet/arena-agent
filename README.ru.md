...
### 🆕 Что нового в v3.2.11

- **Убрано интерактивное KWin focus stealing поведение** — `/v1/desktop/active_window` больше не вызывает KWin DBus-метод, который мог включать крестик-выборщик окна и перехватывать фокус рабочего стола.
- **Стабилизирован нативный KWin desktop introspection** — `loadScript=0` теперь обрабатывается корректно, а и список окон, и active-window discovery на KDE/Wayland используют неинтерактивный `kwin_journal` путь.
- **552 теста проходят**, без регрессий.
