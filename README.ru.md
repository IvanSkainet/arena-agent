...
### 🆕 Что нового в v3.34.0

- **Исправлено обнаружение JSONL** — browser extension теперь считает MCP SuperAssistant-style fenced `jsonl` / `json` function-call блоки executable candidates, а не отфильтровывает их до parser.
- **AI Studio JSONL flow разблокирован** — после copied JSONL instructions inline controls должны появляться, когда модель выдаёт `function_call_start` / `function_call_end` блоки.
