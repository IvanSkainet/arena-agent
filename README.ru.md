...
- **Тише detected history**: detected events теперь dedupe по payload fingerprint, а не по DOM position, поэтому rescans меньше шумят в history.
- **Tool-aware detected rows**: detected history показывает имена tools, например `sys.status`.
- **Page-level detected suppression**: content script не логирует тот же payload повторно, если controls уже mounted на странице.
