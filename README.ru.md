...
| **12 desktop + 4 control эндпоинта** | Автоматизация рабочего стола Wayland/X11: скриншот, display/output discovery, OCR, text-target detection, semantic click-by-text, клик, layout-safe ввод, нажатие клавиш, движение мыши, список окон, active window, focus, плюс pause/resume/revoke/status для control lease |
...
- **Таргетинг по окнам стал умнее** — `/v1/desktop/windows` теперь умеет фильтровать по title/class/pid/display, а окна могут аннотироваться owning display'ем.
- **Более безопасные focus-workflow** — `POST /v1/desktop/focus` теперь поддерживает `dry_run: true`, чтобы агент мог сначала разрешить целевое окно и посмотреть candidates, прежде чем реально забирать фокус.
- **KWin/Wayland focus усилен** — UUID-style Wayland окна теперь можно фокусировать через non-interactive KWin script path, а не только через X11-style activation методы.
- **MCP desktop maturity расширена** — MCP теперь включает `desktop.windows`, `desktop.focus`, `desktop.displays`, `desktop.find_text` и `desktop.click_text` для более богатого semantic desktop control.
- **604 теста проходят**, без регрессий.
...
```
        ...
        │   │ 36 эндпоинтов│  │ 12+4 эндпоинта│  │  + Log + Disk Mon.  │      │
        │   └──────────────┘  └──────────────┘  └──────────────────────┘      │
        │                                                                     │
        └─────────────────────────────────────────────────────────────────────┘
                                           │
...
### Автоматизация рабочего стола (12 эндпоинтов + 4 control lease эндпоинта)
