...
- **MCP `fs.view` + `fs.create`** — просмотр файла с номерами строк (опционально диапазон `view_range=[start,end]`) и создание новых файлов (ошибка, если файл уже существует). Завершена **полная совместимость с str_replace_editor**: ИИ-агенты для кода (Claude Code, Cline, Cursor) могут использовать MCP-сервер Arena как полноценный файловый бэкенд (`fs.read`/`write`/`list`/`view`/`create`/`edit`).
- **MCP `fs.edit`** — точечный find-and-replace в текстовых файлах (`old_text`/`new_text`, с поддержкой `replace_all`), плюс его REST-эквивалент `PATCH /v1/fs/edit`.
- **`/v1/browser/search` починен** — DuckDuckGo сменил эндпоинт; Arena теперь обращается к `lite.duckduckgo.com`, который корректно парсится.
- **OpenAPI-спецификация** — `/api-docs` теперь документирует upload, download и `fs/edit`.
- **445 тестов проходят**, без регрессий.
