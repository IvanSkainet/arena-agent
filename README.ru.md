...
### 🆕 Что нового в v3.63.0

- **Проверяемая диагностика вставки**: contenteditable-стратегии теперь проверяют, что текст composer действительно изменился, прежде чем писать success.
- **Исправлен ложный paste success**: `pasteOnly` больше не показывает `Inserted`, когда Gemini игнорирует synthetic paste event.
- **Direct DOM text strategy**: добавлена стратегия `directDomText` без `execCommand` для диагностики медленной вставки в Gemini rich-textarea.
