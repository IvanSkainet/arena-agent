...
- **Появились реальные window actions** — `POST /v1/desktop/window_action` и MCP `desktop.window_action` теперь позволяют агентам двигать, ресайзить, минимизировать, восстанавливать и переключать fullscreen у desktop-окон.
- **Один и тот же semantic targeting теперь питает и focus, и window actions** — разрешение по title/class/pid/display переиспользуется во всём desktop window control, а не живёт в одном узком endpoint'е.
- **KWin/Wayland остаётся non-interactive** — UUID-style Wayland окна теперь можно не только фокусировать, но и манипулировать ими через journal-backed KWin script path, не возвращая старый класс focus-stealing регрессий.
- **Desktop maturity стала цельнее** — displays, filtered windows, focus dry-run, OCR ranking, click-by-text и window actions теперь образуют более связный стек desktop-control.
- **607 тестов проходят**, без регрессий.
...
| `POST` | `/v1/desktop/window_action` | Двигать, ресайзить, минимизировать, восстанавливать или переключать fullscreen у окна, найденного по id или semantic-фильтрам; поддерживает `dry_run` |
