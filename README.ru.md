...
### 🆕 Что нового в v3.73.0

- **Scan стал отдельным фильтром sidepanel**: Command Center filter теперь включает `scan`, как и агрегированные Scan Page history entries.
- **Scan diagnostics прямо в cards**: scan cards показывают counts кандидатов/блоков/controls, тип composer, Auto insertion plan и версии manifest/content/insert script без открытия raw JSON.
- **Быстрее видно stale-script проблемы**: metadata в sidepanel сразу показывает несовпадения версий после reload/refresh.
