...
- **DX2 recipes для интеграций** — в Arena появился полноценный набор integration recipes в `docs/INTEGRATIONS.md` для Arena Agent Mode, Claude-style chats, Cursor, Cline, Windsurf, Open Interpreter и локальных model backends.
- **AI prompt template обновлён под Memory Profiles** — базовый промпт теперь явно учит агента использовать профили памяти вроде `projects/<name>`, `personal`, `code` и `browser`.
- **Улучшена discoverability в README** — README и README.ru теперь напрямую ссылаются на index с integration recipes.
- **569 тестов проходят**, без регрессий.
