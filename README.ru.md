...
### 🆕 Что нового в v3.17.0

- **Появились mission lifecycle/inspection surfaces** — у Arena теперь есть first-class mission status и mission report API, а не только mission create/run endpoints.
- **Mission composition стала практичнее** — агенты теперь могут инспектировать persisted mission state и забирать generated report через REST и MCP.
- **Mission/orchestration блок становится глубже** — templates, compose, propose, create, run, status и report теперь все доступны как agent-facing surfaces.
- **Desktop maturity никуда не делась** — mission-слой продолжает строиться поверх уже завершённого OCR/window/focus/semantic desktop stack.
