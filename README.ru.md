...
- **File Watchers (`F5`)** — Arena теперь поддерживает realtime file watchers через `GET/POST/DELETE /v1/watch/files`, которые публикуют `file_watch_change` события в `/v1/events`, когда watched files добавляются, меняются или удаляются.
- **MCP `watch.files`** — управление watcher'ами теперь доступно и через MCP для агентных фронтендов.
- **OpenAPI обновлён** — `/v1/watch/files` теперь документирован в публичной API-спеке.
- **579 тестов проходят**, без регрессий.
