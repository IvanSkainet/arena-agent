...
### 🆕 Что нового в v3.47.0

- **Inline close стал sticky** — `×` dismisses конкретный tool block и observer не монтирует его обратно сразу.
- **Clear Page Controls стал sticky** — текущие controls остаются скрытыми до reload или новых tool blocks.
- **Diagnostics показывают dismiss state** — Scan Page теперь сообщает dismissed controls.
