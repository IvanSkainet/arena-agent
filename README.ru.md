...
### 🆕 Что нового в v3.2.13

- **Убраны stale backup-ссылки в CLI и workflow-слое** — `agentctl backup run` теперь явно сообщает, что backup feature удалена, а встроенные mission templates больше не генерируют мёртвые backup-команды.
- **Добавлены канонические planning docs** — в проект вошли `docs/ROADMAP_CANONICAL.md`, `docs/PRODUCT_DIRECTION.md` и `docs/EXPERIMENTS.md`.
- **Починен version string у `agentctl`** — CLI теперь показывает реальную версию bridge, а не старое захардкоженное значение.
- **558 тестов проходят**, без регрессий.
