...
- **Adaptive auto insert**: `auto` теперь сначала пробует быстрый verified `directDomPreWrap` path для contenteditable, затем откатывается на native `insertText` только если fast-path вообще не изменил composer.
- **Улучшенная settled verification**: проверка вставки использует normalized text и whitespace-free signatures, чтобы меньше ошибаться на DOM-path с изменёнными пробелами/переносами.
- **Без Gemini-only mode**: стратегия generic и verification-gated, поэтому неподдерживаемые чаты безопасно откатываются на fallback.
...
| **cloudflared** | Внутрь папки моста (`$INSTALL_DIR/cloudflared` или `%BRIDGE_DIR%\cloudflared.exe`) | Альтернатива Tailscale Funnel (Cloudflare Quick Tunnels, без аккаунта) | Да — скачивает ~50 МБ |
