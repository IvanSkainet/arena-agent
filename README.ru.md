...
### 🆕 Что нового в v3.57.0

- **Исправлена регрессия Gemini Insert/Send lag**: клики по toolbar больше не забирают фокус у composer чата.
- **Фокус composer теперь guarded**: вставка вызывает `focus()` только когда это действительно нужно, избегая дорогого Gemini blur/focus churn.
- **Общий insert path сохранён**: duplicate-safe `insertText` flow остался без отката.
