...
### 🆕 Что нового в v3.36.0

- **Понятные execution failures** — history и inline controls теперь показывают результат failed tool call вместо `error: unknown`.
- **Аккуратнее controls в AI Studio** — кнопки ставятся после rendered `Jsonl` code block, а не внутри UI блока кода.
- **Fallback для Panel** — если Chrome блокирует `sidePanel.open()`, side panel открывается как обычная extension tab.
