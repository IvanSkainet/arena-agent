...
### 🆕 Что нового в v3.35.0

- **Детект rendered JSONL в AI Studio** — browser extension теперь ловит MCP SuperAssistant-style блоки `function_call_start` / `function_call_end`, даже когда Google AI Studio показывает их как красивый `Jsonl` code block без literal triple backticks.
- **Более сильное сканирование Gemini / AI Studio** — расширение проверяет rendered `pre`, `code` и code-like nodes, чтобы inline Arena controls могли появляться рядом с реальным ответом модели.
