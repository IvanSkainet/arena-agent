...
### 🆕 Что нового в v3.25.0

- **Chat extension MVP стал полезнее** — у browser extension scaffold теперь есть popup UI для настройки bridge URL/token, connection test, просмотра policies и recent execution history.
- **Начат generic adapter layer** — у расширения теперь есть adapter registry и более нормальный generic detection flow вместо одного жёстко пришитого detector path.
- **Улучшена работа с результатами** — после выполнения расширение теперь умеет copy result и пытаться вставить результат обратно в активное поле ввода.
- **Browser-chat направление теперь не только backend-only** — у Arena теперь есть и bridge-side extension execution surfaces, и минимально полезный extension UX слой.
