...
### 🆕 Что нового в v3.59.0

- **Gemini insert path снова облегчён**: убран duplicate synthetic `InputEvent` после native contenteditable `insertText`.
- **Latency теперь видна**: toolbar status показывает `Inserted in Xms` / `Inserted/submitted in Xms`.
- **Textarea/input behavior не изменён**: direct value writes всё ещё dispatch explicit `input/change` events.
