...
### 🆕 Что нового в v3.16.0

- **Появился flow для mission proposal/orchestration** — `POST /v1/mission/propose` и MCP `mission.propose` теперь умеют запускать bounded agentic proposal loop, отражаться по результату и возвращать planner-backed mission bundle с опциональным create/run.
- **Mission composition больше не сводится к CRUD** — Arena теперь умеет проходить путь goal → observe/reflect → mission draft → persisted mission → optional mission run в одном agent-facing flow.
- **Следующий большой roadmap-блок пошёл по-настоящему** — это первый реальный мост между agentic runtime (`react` / `reflect`) и reusable mission artifacts.
- **Desktop maturity никуда не делась** — новая mission/orchestration работа строится поверх уже завершённого OCR/window/focus/semantic desktop stack, а не вместо него.
- **623 теста проходят**, без регрессий.
