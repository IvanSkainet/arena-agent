"""Compatibility exports for modular CDP browser process helpers."""
from __future__ import annotations

from cdp_browser_modules.process_discovery import find_browser_exe, _resolve_browser_binary, _build_session_env, _build_chromium_cmd
from cdp_browser_modules.process_helpers import _ts, _drain_stderr, _kill_port_processes, _write_diag_file
from cdp_browser_modules.process_launch import launch_browser

__all__ = [
    "find_browser_exe", "_resolve_browser_binary", "_build_session_env", "_build_chromium_cmd",
    "_ts", "_drain_stderr", "_kill_port_processes", "_write_diag_file", "launch_browser",
]
