...
def main():
    ...
    s.add_argument('--lang',default='eng+rus')
    s.set_defaults(func=ocr)
    sub.add_parser('windows').set_defaults(func=windows)
    s=sub.add_parser('move')
    s.add_argument('x')
    s.add_argument('y')
    s.add_argument('--steps',default=25)
    s.add_argument('--delay',default=.01)
    s.set_defaults(func=move)
    s=sub.add_parser('click')
    s.add_argument('x')
    s.add_argument('y')
    s.add_argument('--button',default='1')
    s.add_argument('--steps',default=25)
    s.add_argument('--delay',default=.01)
    s.set_defaults(func=click)
    s=sub.add_parser('key')
    s.add_argument('key')
    s.set_defaults(func=key)
    s=sub.add_parser('type')
    s.add_argument('text')
    s.set_defaults(func=type_text)
    a=ap.parse_args()
    a.func(a)
