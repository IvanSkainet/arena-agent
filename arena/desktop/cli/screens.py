...
def info(_):
    ...
    if have('qdbus6'):
        p=run('qdbus6 org.kde.KWin /KWin org.kde.KWin.activeOutputName')
        out['active_output']=p.stdout.strip()
    if have('xrandr'):
...
def shot(args):
    ...
    path=Path(args.path).expanduser() if args.path else REPORTS/f'desktop-shot-{stamp()}.png'
    ...
    for cmd in [f'grim {shq(str(path))}', f'spectacle -b -n -o {shq(str(path))}', f'gnome-screenshot -f {shq(str(path))}']:
        if shutil.which(cmd.split()[0]):
            ...
            methods.append({'cmd':cmd,'exit':p.returncode,'stderr':p.stderr[-500:]})
...
def ocr(args):
    ...
    if not img:
        tmp=REPORTS/f'ocr-shot-{stamp()}.png'
        shot(argparse.Namespace(path=str(tmp)))
        img=tmp
...
def windows(_):
    rows=[]
    if have('qdbus6'):
        p=run('qdbus6 org.kde.KWin /KWin org.kde.KWin.supportInformation',timeout=10)
        rows.append({'source':'kwin.supportInformation','summary_lines':[ln for ln in p.stdout.splitlines() if any(x in ln.lower() for x in ['activeview','geometry','compositing','xwayland'])][:80]})
