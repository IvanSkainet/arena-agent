...
v3.98.0: Migrated to @authed / err_json from arena.handler_helpers.
The 22 handlers in this module all had the same six-line prelude
(auth check, record_request, try/except with error accounting) —
the decorator collapses that to a single ``@authed(ctx)`` line,
cutting roughly 132 lines of boilerplate without changing any
wire behaviour.
"""
...
def make_mobile_handlers(ctx) -> MobileHandlers:
    ...
    async def handle_device_info(request: web.Request) -> web.Response:
        ...
        if not serial:
            return err_json(ctx, "serial required", status=400)
