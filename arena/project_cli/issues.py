...
def issue_new(args):
    ...
    iid=dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+uuid.uuid4().hex[:6]
    ...
    (d/f'{iid}.md').write_text(f"# {args.title}\n\nID: {iid}\nCreated: {obj['created_at']}\n\n{obj['body']}\n",encoding='utf-8')
    print(iid)
...
def issue_close(args):
    p=project_path(args.name)
    src=p/'issues/open'/f'{args.id}.json'
    srcmd=p/'issues/open'/f'{args.id}.md'
    if not src.exists():
        raise SystemExit('open issue not found')
    obj=json.loads(src.read_text())
    obj['state']='closed'
    obj['closed_at']=now()
    obj['close_reason']=args.reason or ''
    dst=p/'issues/closed'/src.name
