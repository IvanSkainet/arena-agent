...
def new_project(args):
    ...
    meta=p/'project.json'
    if not meta.exists():
        meta.write_text(json.dumps({'name':name,'created_at':now(),'description':args.description or ''}, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
    if args.git:
        git_init(argparse.Namespace(name=name, message='Initial project scaffold'))
    print(str(p))
...
def list_projects(args):
    ...
    for p in sorted(x for x in PROJECTS.iterdir() if x.is_dir() and not x.name.startswith('.')):
        print(('* ' if p.name==cur else '  ')+p.name)

def git_init(args):
    p=project_path(args.name)
    if not (p/'.git').exists():
        run('git init', p, check=True)
...
def status(args):
    ...
    for x in sorted(p.rglob('*')):
        if '.git' in x.parts:
            continue
        if x.is_file():
            files.append({'path':str(x.relative_to(p)),'size':x.stat().st_size})
    out['files']=files[:500]
...
def commit(args):
    p=project_path(args.name)
    if not (p/'.git').exists():
        git_init(argparse.Namespace(name=p.name,message='Initial commit'))
    ensure_git_identity(p)
    run('git add -A',p,check=True)
...
def branch(args):
    ...
    if args.create:
        ...
        print(run('git branch -vv',p).stdout)
