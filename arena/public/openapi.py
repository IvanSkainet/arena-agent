...
def build_openapi_spec(ctx) -> dict:
    ...
    spec: dict = {
        ...
        "paths": {
            ...
            "/v1/events": {"get": {"summary": "WebSocket real-time event stream", "description": "A WebSocket handshake, not a JSON operation: send `Upgrade: websocket` and `Connection: Upgrade`. A plain GET is answered 426 rather than upgraded, and an unauthenticated one is refused with 401 before the handshake.", "tags": ["Events"], "parameters": [{"name": "Upgrade", "in": "header", "required": True, "schema": {"type": "string", "enum": ["websocket"]}}, {"name": "Connection", "in": "header", "required": True, "schema": {"type": "string"}, "description": "Must contain the `Upgrade` token. Browsers and proxies send it in a list -- `keep-alive, Upgrade` -- which the handler accepts, so this is not an enum."}], "responses": {"101": {"description": "Switching protocols: the event stream is open"}, "426": _error_response("Not a WebSocket handshake -- send the Upgrade and Connection headers")}}},
