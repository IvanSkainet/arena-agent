...
def build_openapi_spec(ctx) -> dict:
    ...
    spec: dict = {
        ...
        "paths": {
            ...
            "/v1/token/regenerate": {"post": {"summary": "Rotate the bridge bearer token", "description": "Generates a new master token, writes it to the token file, and invalidates the current one from the next request onward. No restart is required. A write failure is a 500 (#211): only a 200 carries a usable token, so a caller may replace its stored credential when the status says so.", "tags": ["Bridge"], "responses": {"500": _error_response("The token file could not be written. Rotation did not happen and the CURRENT credential is still valid -- do not discard it."), "200": {"description": "Rotation succeeded. `token` carries the new credential; the previous one is rejected from the next request onward.", "content": {"application/json": {"schema": {"type": "object", "properties": {"ok": {"type": "boolean", "enum": [True]}, "token": {"type": "string", "minLength": 1, "description": "The new credential. A 200 always carries one (#211)."}, "written_to": {"type": "array", "items": {"type": "string"}}, "previous_token_revoked": {"type": "boolean"}, "restart_required": {"type": "boolean"}, "warning": {"type": "string", "description": "The rotation took effect, but the token file's mode could not be re-applied; check its permissions."}}, "required": ["ok", "token"]}}}}, }}},
