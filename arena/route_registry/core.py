...
def register_core_routes(app: web.Application, h: Mapping[str, Callable]) -> None:
    ...
    # v4.3.0: NDJSON streaming endpoint — one JSON event per line as
    # stdout/stderr bytes arrive from the child process.
    app.router.add_post("/v1/exec/stream", h["handle_v1_exec_stream"])
    ...
    app.router.add_get("/v1/autonomy/posture", h["handle_v1_autonomy_posture_get"])
    app.router.add_post("/v1/autonomy/posture", h["handle_v1_autonomy_posture_set"])
    ...
    app.router.add_post("/v1/bore/tunnel/{action}", h["handle_v1_bore_tunnel"])
    app.router.add_get("/v1/bore/tunnel/{action}", h["handle_v1_bore_tunnel"])
    ...
    # v4.14.0: manual reset of the tunnels_probe circuit breaker.
    app.router.add_post("/v1/tunnels/probe/reset", h["handle_v1_tunnels_probe_reset"])
