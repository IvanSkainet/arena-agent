...
def build_tasks_skills_resources_registries(g: MutableMapping[str, Any]) -> dict[str, Any]:
    ...
    task_queue_runtime_ctx = TaskQueueRuntimeContext(
        inbox=INBOX,
        running=RUNNING,
        done=DONE,
        failed=FAILED,
    ...
    task_queue_runtime = make_task_queue_runtime(task_queue_runtime_ctx)
    registry.update({
    ...
    task_handlers = make_task_handlers(task_handler_ctx)
    export_handler_attrs(registry, task_handlers, {"handle_v1_tasks_get": "tasks_get", "handle_v1_tasks_post": "tasks_post", "handle_v1_tasks_clean": "tasks_clean"})
    ...
    skill_handler_ctx = SkillHandlerContext(
        require_auth=require_auth,
        record_request=_record_request,
        cors_json_response=_cors_json_response,
        executor=_EXECUTOR,
    ...
    skill_handlers = make_skill_handlers(skill_handler_ctx)
    export_handler_attrs(registry, skill_handlers, {"handle_v1_skills": "skills", "handle_v1_skills_install": "install", "handle_v1_skills_uninstall": "uninstall", "handle_v1_skills_run": "run", "handle_v1_skills_reload": "reload"})
    ...
    resource_handler_ctx = ResourceHandlerContext(
        require_auth=require_auth,
        record_request=_record_request,
        cors_json_response=_cors_json_response,
        executor=_EXECUTOR,
    ...
    resource_handlers = make_resource_handlers(resource_handler_ctx)
    export_handler_attrs(registry, resource_handlers, {"handle_v1_missions": "missions", "handle_v1_reports": "reports", "handle_v1_hooks": "hooks", "handle_v1_agents": "agents", "handle_v1_subagents": "subagents", "handle_v1_subagents_spawn": "subagents_spawn", "handle_v1_mission_show": "mission_show"})
