...
class Bridge:
    ...
    def download(self, remote, local):
        ...
        with open(local, 'wb') as f:
            f.write(base64.b64decode(r.get('stdout', '').strip()))
    ...
    def _curl(self, path, data, timeout=30, auth=True):
        ...
        try:
            ...
            return {'ok': False, 'raw': p.stdout.decode('utf-8', 'replace')}
...
def main():
    ...
    if len(args) >= 3 and args[0].startswith('http'):
        url, tok = args[0], args[1]
        args = args[2:]
    b = Bridge(url, tok)
    if args[0] == 'health':
        print(json.dumps(b.health(), ensure_ascii=False, indent=2))
        return 0
    if args[0] == '--upload' and len(args) >= 3:
        print(json.dumps(b.upload(args[1], args[2]), ensure_ascii=False, indent=2))
        return 0
    if args[0] == '--download' and len(args) >= 2:
        local = args[2] if len(args) >= 3 else os.path.basename(args[1])
        print(json.dumps(b.download(args[1], local), ensure_ascii=False, indent=2))
        return 0
