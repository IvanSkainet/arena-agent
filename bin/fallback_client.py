...
def main():
    a=sys.argv[1:]
    url=os.getenv('AURL') or os.getenv('ARENA_BRIDGE_URL')
    tok=os.getenv('ATOK') or os.getenv('ARENA_BRIDGE_TOKEN')
    if len(a)>=3 and a[0].startswith('http'):
        url,tok=a[0],a[1]
        a=a[2:]
    if not url or not tok or not a:
        print(__doc__.strip())
        return 2
    cmd=' '.join(a)
    errs=[]
    for name,fn in [('urllib',ureq),('requests',reqs),('curl',curl),('wget',wget)]:
        try:
            print(f'[{name}] '+fn(url,tok,cmd), end='')
            return 0
