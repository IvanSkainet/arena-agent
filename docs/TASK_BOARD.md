...
- [ ] **T37 [GOVERNANCE / GITHUB]** ([#8](https://github.com/IvanSkainet/arena-agent/issues/8)) Перевести значимые изменения на Issue → branch → PR: добавить структурированные Issue Forms и PR template, создать стабильные aggregate required checks, затем применить ruleset к `master` без обязательного внешнего approval и без merge queue. Прямые push/force-push/delete должны перестать быть штатным путём; emergency bypass фиксируется отдельным incident issue.
  ...
  - Direct-push sabotage `2211bf90` отклонён GitHub с `GH013`; remote `master` остался `c18fdf3f`. Финальный критерий выполняется отдельным Markdown-only PR: expensive matrix должна быть skipped, а `CI required` — success.
