...
- [ ] **T55 [DEPLOYMENT / ARCHIVE PROVENANCE]** ([#59](https://github.com/IvanSkainet/arena-agent/issues/59)) Сделать ZIP-развёртывание самоидентифицируемым без `.git`: source commit/release tag/candidate run внутри детерминированного архива, SHA-256 и install timestamp в `DEPLOYED_PROVENANCE.json`, authenticated/fail-closed status, сохранённая identity предыдущего deployment и rollback evidence. Обязательны substituted/stale/wrong-source sabotage, 0 mutants и exact-candidate Windows acceptance.
- [ ] **T56 [RELEASE / LIVE ACCEPTANCE]** ([#78](https://github.com/IvanSkainet/arena-agent/issues/78)) Выпустить `v4.169.48` как первый provenance-aware archive release: exact candidate A/B, ZIP/APK attestations и SBOM, штатная Windows установка, честный authenticated status, post-publication reinstall тех же bytes и rollback evidence.
