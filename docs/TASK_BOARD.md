...
- [ ] **T37 [GOVERNANCE / GITHUB]** ([#8](https://github.com/IvanSkainet/arena-agent/issues/8)) Перевести значимые изменения на Issue → branch → PR: добавить структурированные Issue Forms и PR template, создать стабильные aggregate required checks, затем применить ruleset к `master` без обязательного внешнего approval и без merge queue. Прямые push/force-push/delete должны перестать быть штатным путём; emergency bypass фиксируется отдельным incident issue.
- [ ] **T38 [CI / BADGE WRITER]** ([#9](https://github.com/IvanSkainet/arena-agent/issues/9)) Удалить лишний write-to-master контур `version-badge.yml` только после миграции его реальных инвариантов: `docs/version.json`, pre-release freshness, monotonic release version и связанные sabotage tests. README уже использует GitHub release badge; замена не должна ослабить version/tag/release consistency.
- [ ] **T39 [SUPPLY CHAIN / PROVENANCE]** ([#10](https://github.com/IvanSkainet/arena-agent/issues/10)) Связать release ZIP с исходным commit: детерминированная CI-сборка, byte-for-byte source rebuild check, SBOM и GitHub/Sigstore provenance attestation. Один и тот же artifact должен пройти реальный Windows acceptance, публикацию без пересборки и анонимную post-publication переустановку.
- [ ] **T40 [SECURITY / FAIL CLOSED]** ([#11](https://github.com/IvanSkainet/arena-agent/issues/11)) Разделить advisory findings и scanner execution failures в Security workflow. TruffleHog, OSV, Syft/Grype, Socket Firewall и DevSkim не могут давать зелёный результат при падении самого сканера; пороги findings задаются явно и sabotage-проверяются.
- [ ] **T41 [CROSS-REPO / BOOK OF ETERNITY]** ([#12](https://github.com/IvanSkainet/arena-agent/issues/12)) Добавить pinned-commit compatibility manifest и Windows contract workflow для `StanislavSmetaninSSM/The-Book-of-Eternity-Reborn`: build `BookOfEternityGMBridge`, два последовательных multiline ConPTY dispatch, correlated replies и validation-repair cycle без переноса игровых правил в Bridge.
- [ ] **T42 [ARENA.AI / GM PRODUCTIZATION]** ([#13](https://github.com/IvanSkainet/arena-agent/issues/13)) Довести Arena.ai Agent Mode до пользовательского внешнего Game Master: универсальные full-capability `relay.*` / `fs.*` / `exec.*`, актуальный bootstrap skill, resume новой Arena-сессией, честные active/queued/busy состояния и многоходовой live E2E. Узкий game-specific token/toolset не является обязательным; operator-owned глобальная safety posture остаётся отдельным уровнем.
