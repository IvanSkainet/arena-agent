...
- [ ] **T52 [GOVERNANCE / REVIEWER BENCHMARK]** ([#67](https://github.com/IvanSkainet/arena-agent/issues/67)) Создать controlled never-merged benchmark PR для Marketplace reviewers: общий defect/control corpus, exact-head evidence collector, signal/quota/permissions/cost matrix и fail-closed shadow policy. Hosted/BYOK integrations подключаются только после отдельного permission/secret approval.
  - Exact-head collector готов: submitted reviews/inline comments связываются с commit SHA, ordinary comments остаются честно unbound, checks берутся только с exact commit endpoint, head race/wrong expected SHA/pagination truncation/malformed records fail closed. Live PR #70: намеренно устаревший expected SHA отклонён; current head показал `exact=0`, `stale=2`, `unbound=3`, DeepSource/Sourcery skipped. Новый governance module закреплён на 0/217 surviving mutants. Следующий этап — merge инфраструктуры и never-merged defect/control PR.
