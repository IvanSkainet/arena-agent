...
- [x] **T45 [GOVERNANCE / AI REVIEW]** ([#22](https://github.com/IvanSkainet/arena-agent/issues/22)) Перекалибровать GitHub Apps по живым exact-head evidence и независимым quota lanes: CodeRabbit/Qodo semantic, SonarQube Cloud/Codacy/CodeFactor static, Sourcery quota-dependent, DeepSource pending vendor recovery; bot autofix остаётся непроверенным входом.
  - Завершено T52: permission audit подтвердил широкий write surface у hosted Apps; overlap разрешён как quota redundancy, но каждый finding проходит exact-head triage. CodeFactor оставлен informational на probationary 10-real-PR sample по явному availability решению; DeepSource skipped остаётся no-signal до ответа вендора. Ни один внешний reviewer не является required check.
