...
- [ ] **T41 [CROSS-REPO / BOOK OF ETERNITY]** ([#12](https://github.com/IvanSkainet/arena-agent/issues/12)) Добавить pinned-commit compatibility manifest и Windows contract workflow для `StanislavSmetaninSSM/The-Book-of-Eternity-Reborn`: build `BookOfEternityGMBridge`, два последовательных multiline ConPTY dispatch, correlated replies и validation-repair cycle без переноса игровых правил в Bridge.
  - Реализация pin-ит upstream `11ddf9f5`, собирает и запускает Arena release artifact + game GM bridge на Windows, проводит два turn dispatch и repair через generic `arena-relay terminal`, требует empty mailbox/no process survivors и блокирует candidate attestation до cross-repo success. T41 останется открытой до первого exact-head live workflow evidence.
