...
- [ ] **T42 [ARENA.AI / GM PRODUCTIZATION]** ([#13](https://github.com/IvanSkainet/arena-agent/issues/13)) Довести Arena.ai Agent Mode до пользовательского внешнего Game Master: универсальные full-capability `relay.*` / `fs.*` / `exec.*`, актуальный bootstrap skill, resume новой Arena-сессией, честные active/queued/busy состояния и многоходовой live E2E. Узкий game-specific token/toolset не является обязательным; operator-owned глобальная safety posture остаётся отдельным уровнем.
  - Реализован generic lifecycle: `relay.status/check/resume/busy/reply/send`, durable claimed-work resume, явные queued/claimed/busy/replied/repair depths, Arena.ai extension scopes `relay`/`gm`, обновлённый bootstrap skill и public guide. Full repository (`8354 passed, 36 skipped`), focused/architecture/quality/security gates и destructive resume/Dashboard controls пройдены. T42 остаётся открытой до Windows host deployment и принятого multi-turn Arena Agent Mode live run с repair, session restart и unattended queue persistence; журнал — `docs/audits/arena-agent-gm-live-findings-13.md`.
