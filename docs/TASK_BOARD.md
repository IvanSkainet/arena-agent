...
- [ ] **T47 [TEST / SHIP SMOKE]** ([#38](https://github.com/IvanSkainet/arena-agent/issues/38)) Устранить зависимость `test_ship_smoke_shape` от пустого каталога flight-records: проверять дельту до/после, точный возвращённый `report_path` и двусторонним саботажем доказывать обнаружение двойной записи.
