...
- [ ] **T39 [SUPPLY CHAIN / PROVENANCE]** ([#10](https://github.com/IvanSkainet/arena-agent/issues/10)) Связать release ZIP с исходным commit: детерминированная CI-сборка, byte-for-byte source rebuild check, SBOM и GitHub/Sigstore provenance attestation. Один и тот же artifact должен пройти реальный Windows acceptance, публикацию без пересборки и анонимную post-publication переустановку.
  ...
  - T39 останется открытой до live candidate run на merged SHA, следующего versioned Windows acceptance и анонимной переустановки опубликованных без пересборки bytes.
...
- [ ] **T41 [CROSS-REPO / BOOK OF ETERNITY]** ([#12](https://github.com/IvanSkainet/arena-agent/issues/12)) Добавить pinned-commit compatibility manifest и Windows contract workflow для `StanislavSmetaninSSM/The-Book-of-Eternity-Reborn`: build `BookOfEternityGMBridge`, два последовательных multiline ConPTY dispatch, correlated replies и validation-repair cycle без переноса игровых правил в Bridge.
  - Реализация pin-ит upstream `11ddf9f5`, собирает и запускает Arena release artifact + game GM bridge на Windows, проводит два turn dispatch и repair через generic `arena-relay terminal`, требует empty mailbox/no process survivors и блокирует candidate attestation до cross-repo success. Live-находки и исправления записываются в `docs/audits/boe-contract-live-findings-12.md`; T41 останется открытой до первого exact-head live workflow evidence.
