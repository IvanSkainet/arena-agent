...
def doctor(a):
    ...
    try:
        import requests  # noqa: F401  # presence check for the doctor report
...
def test(a):
    ...
    if shutil.which('python3'):
        cmds.append('python3 ~/arena-bridge/bin/ai_client.py "$AURL" "$ATOK" whoami')
    if shutil.which('curl'):
        cmds.append('./a whoami 30 4000' if os.path.exists('./a') else f'curl --curves X25519 -sS {AURL}/health')
    out=[]
    for c in cmds:
        p=run(c)
        out.append({'cmd':c,'exit':p.returncode,'stdout':p.stdout[:1000],'stderr':p.stderr[:1000]})
...
def main():
    ...
    s.add_argument('kind', choices=['bash','python','powershell'])
    s.set_defaults(func=gen)
