...
async def run_mcp_client(server_name, action, tool_name=None, tool_args=None):
    ...
    cfg=load_config()
    if server_name not in cfg['mcpServers']:
        raise SystemExit(f"Error: MCP Server '{server_name}' not found.")
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if action == 'tools':
                ...
                print(json.dumps([{'name':t.name,'description':t.description,'schema':t.inputSchema} for t in tools.tools], indent=2))
            elif action == 'call':
                args=json.loads(tool_args) if tool_args else {}
                res=await session.call_tool(tool_name, arguments=args)
                for c in res.content:
                    print(c.text if c.type=='text' else f'[{c.type} content]')
def stream_health():
    st,h,b=http_json('/health')
    print(b)
def stream_init():
    payload={'jsonrpc':'2.0','id':1,'method':'initialize','params':{}}
    st,h,b=http_json('/mcp', payload)
    print(b)
def stream_tools():
...
def run():
    if len(sys.argv)<2:
        print('Usage: agentctl mcp [install|list|tools|call|stream-health|stream-init|stream-tools|stream-call|stream-sse]')
        return
    action=sys.argv[1]
    if action=='install':
        if len(sys.argv)<4:
            print('Usage: agentctl mcp install <name> <command> [args...]')
            return
        cfg=load_config()
        cfg['mcpServers'][sys.argv[2]]={'command':sys.argv[3],'args':sys.argv[4:],'env':{}}
        save_config(cfg)
        print(f"Installed MCP server '{sys.argv[2]}'")
    elif action=='list':
        print(json.dumps(load_config(), indent=2))
    elif action=='tools':
        asyncio.run(run_mcp_client(sys.argv[2], 'tools'))
    elif action=='call':
        asyncio.run(run_mcp_client(sys.argv[2], 'call', sys.argv[3], sys.argv[4] if len(sys.argv)>4 else '{}'))
    elif action=='stream-health':
        stream_health()
    elif action=='stream-init':
        stream_init()
    elif action=='stream-tools':
        stream_tools()
    elif action=='stream-call':
        stream_call(sys.argv[2], sys.argv[3] if len(sys.argv)>3 else '{}')
    elif action=='stream-sse':
        stream_sse_probe()
    else:
