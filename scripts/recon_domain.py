...
def dns_records(domain, rtype):
    try:
        return [r.to_text() for r in dns.resolver.resolve(domain, rtype, lifetime=5)]
...
def http_probe(url):
    try:
        t=time.time()
        with httpx.Client(http2=True, follow_redirects=True, timeout=12, headers={'user-agent':'arena-agent-recon/0.1'}) as c:
            r=c.get(url)
...
def main():
    ...
    out={'ok': True, 'domain': d, 'ts': datetime.now(timezone.utc).isoformat(timespec='seconds'), 'dns': {}, 'http': {}, 'tls': {}}
    for rt in ['A','AAAA','MX','NS','TXT','CAA','SOA']:
        out['dns'][rt]=dns_records(d, rt)
    out['tls']['443']=tls_info(d,443)
    out['http']['http']=http_probe('http://'+d)
    out['http']['https']=http_probe('https://'+d)
    if args.whois:
        try:
