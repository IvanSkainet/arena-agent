...
def dns_lookup(domain, types):
    ...
    for rt in types:
        try:
            out[rt]=[r.to_text() for r in dns.resolver.resolve(domain, rt, lifetime=6)]
...
def tls_check(host, port=443):
    ...
    try:
        ...
        return {'ok': False, 'host': host, 'port': port, 'error': repr(e)}
...
def sitemap(url):
    ...
    for cnd in candidates:
        try:
            r=fetch_text(cnd)
            found.append({'url': str(r.url), 'status_code': r.status_code, 'content_type': r.headers.get('content-type'), 'text_preview': r.text[:10000]})
        except Exception as e:
            found.append({'url': cnd, 'error': repr(e)})
...
def main():
    ...
    s.add_argument('--types', nargs='*', default=['A','AAAA','MX','NS','TXT','CAA','SOA'])
    s.set_defaults(func=lambda a: j({'ok':True,'domain':a.domain,'dns':dns_lookup(a.domain,a.types)}))
    s=sub.add_parser('tls')
    s.add_argument('host')
    s.add_argument('--port', type=int, default=443)
    s.set_defaults(func=lambda a: j(tls_check(a.host,a.port)))
    s=sub.add_parser('headers')
    s.add_argument('url')
    s.set_defaults(func=lambda a: j(headers(a.url)))
    s=sub.add_parser('robots')
    s.add_argument('url')
    s.set_defaults(func=lambda a: j(robots(a.url)))
    s=sub.add_parser('sitemap')
    s.add_argument('url')
    s.set_defaults(func=lambda a: j(sitemap(a.url)))
    s=sub.add_parser('tech')
    s.add_argument('url')
    s.set_defaults(func=lambda a: j(tech(a.url)))
    args=p.parse_args()
    args.func(args)
