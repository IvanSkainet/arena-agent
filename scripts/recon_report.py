...
def run(cmd, timeout=60):
    ...
    try:
        ...
        data={'exit_code':p.returncode,'stdout':p.stdout,'stderr':p.stderr}
    return data
def main():
    if len(sys.argv)<2:
        raise SystemExit('usage: recon_report.py DOMAIN')
    domain=sys.argv[1].strip().removeprefix('https://').removeprefix('http://').split('/')[0]
    REPORTS.mkdir(parents=True, exist_ok=True)
    st=dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    json_path=REPORTS/f'recon-report-{domain}-{st}.json'
    md_path=REPORTS/f'recon-report-{domain}-{st}.md'
