...
def run_one_file(p: Path):
    ...
    try:
        ...
        if len(out.encode('utf-8','replace')) > max_output:
            out = out.encode('utf-8','replace')[:max_output].decode('utf-8','replace')
            truncated = True
        if len(err.encode('utf-8','replace')) > max_output:
            err = err.encode('utf-8','replace')[:max_output].decode('utf-8','replace')
            truncated = True
...
def run_once(args):
    ...
    for p in sorted(INBOX.glob('*.json'))[:args.max]:
        if run_one_file(p):
            count += 1
    if count == 0 and not getattr(args, 'quiet', False):
        print('no tasks')
...
def main():
    ...
    p.add_argument('--max-output', type=int, default=2_000_000)
