...
# Expected candidate paths are joined with the runtime's os.path.join on
# purpose: the module joins the same way, so on a Windows runner both sides
# use backslashes (CI v4.169.34 red cell proved literal "/" expectations
# fail there). The SEGMENTS stay test-side literal copies -- a mutant that
# edits any segment still diverges and dies on any OS.
