...
def test_content_version_bumped_to_0_14_7():
    ...
    assert (
        "ARENA_CONTENT_SCRIPT_VERSION = '0.14.17'" in src
        or 'ARENA_CONTENT_SCRIPT_VERSION = "0.14.17"' in src
...
def test_manifest_version_bumped():
    ...
    assert manifest["version"] == "0.14.17"


def test_insert_script_version_bumped():
    src = _read("insert_strategies.js")
    assert "return '0.14.17';" in src or 'return "0.14.17";' in src


def test_readme_version_banner_bumped():
    readme = _read("README.md")
    assert "Current extension version: `0.14.17`" in readme
