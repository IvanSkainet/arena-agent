"""Regression guards for extension 0.14.11 (v4.49.1).
...
def test_grok_per_adapter_user_filter_lives_in_arenaWhyUserAuthored():
    """v0.14.11: Grok-specific check must be inside arenaWhyUserAuthored
    ...
    # v0.14.11 widened this branch to cover grok AND duckai.
...
def test_controls_host_takes_adapter_and_all_call_sites_pass_it():
    """v0.14.11: controlsHost(node, adapter) signature change. Every
...
def test_duckai_hoist_out_of_overflow_hidden():
    """v0.14.11: DuckAI branch of controlsHost must escape .overflow-hidden."""
    src = _read("content.js")
...
def test_qwen_hoist_anchors_on_outer_pre():
    """v0.14.11: Qwen branch anchors on the outer <pre.qwen-markdown-code>.
...
def test_why_user_authored_call_site_passes_adapter():
    """v0.14.11: the mount-time skip check must pass the adapter so
