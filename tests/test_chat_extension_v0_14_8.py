"""Regression guards for extension 0.14.8 (v4.49.1).
...
def test_versions_pinned_to_0_14_8():
    import json
    assert "ARENA_CONTENT_SCRIPT_VERSION = '0.14.8'" in _read("content.js")
    assert json.loads(_read("manifest.json"))["version"] == "0.14.8"
    assert "return '0.14.8';" in _read("insert_strategies.js")
    assert "Current extension version: `0.14.8`" in _read("README.md")


def test_grok_per_adapter_user_filter_lives_in_arenaWhyUserAuthored():
    """v0.14.8: Grok-specific check must be inside arenaWhyUserAuthored
    ...
    assert "adapterName === 'grok'" in src, (
        "per-adapter Grok branch must be gated on adapter.name"
    )
    assert 'closest(\'[data-testid="user-message"].message-bubble' in src or \
           'closest("[data-testid=\\"user-message\\"].message-bubble' in src, (
        "Grok branch must query the user-message bubble via closest()"
    )
    assert "reason: 'grok:user-message-bubble@DIV'" in src, (
        "Grok skip must be tagged with a distinct reason for scan-report"
    )
...
def test_controls_host_takes_adapter_and_all_call_sites_pass_it():
    """v0.14.8: controlsHost(node, adapter) signature change. Every
...
def test_duckai_hoist_out_of_overflow_hidden():
    """v0.14.8: DuckAI branch of controlsHost must escape .overflow-hidden."""
    src = _read("content.js")
...
def test_qwen_hoist_out_of_monaco_viewport():
    """v0.14.8: Qwen branch of controlsHost must escape the Monaco viewport."""
    src = _read("content.js")
    assert "adapterName === 'qwen'" in src
    assert ".qwen-markdown-code-editor-viewport" in src
    assert ".qwen-markdown-code-body" in src, (
        "Qwen branch must prefer .qwen-markdown-code-body as anchor"
    )


def test_why_user_authored_call_site_passes_adapter():
    """v0.14.8: the mount-time skip check must pass the adapter so
...
def test_content_js_stays_at_or_below_700_lines():
    ...
    assert lines <= 700, f"content.js is {lines} lines (limit 700)"
