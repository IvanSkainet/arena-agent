"""Regression guards for extension 0.14.10 (v4.49.1).
...
def test_versions_pinned_to_0_14_8():
    import json
    assert "ARENA_CONTENT_SCRIPT_VERSION = '0.14.10'" in _read("content.js")
    assert json.loads(_read("manifest.json"))["version"] == "0.14.10"
    assert "return '0.14.10';" in _read("insert_strategies.js")
    assert "Current extension version: `0.14.10`" in _read("README.md")


def test_grok_per_adapter_user_filter_lives_in_arenaWhyUserAuthored():
    """v0.14.10: Grok-specific check must be inside arenaWhyUserAuthored
    ...
    # v0.14.10 widened this branch to cover grok AND duckai.
...
def test_controls_host_takes_adapter_and_all_call_sites_pass_it():
    """v0.14.10: controlsHost(node, adapter) signature change. Every
...
def test_duckai_hoist_out_of_overflow_hidden():
    """v0.14.10: DuckAI branch of controlsHost must escape .overflow-hidden."""
    src = _read("content.js")
...
def test_qwen_hoist_anchors_on_outer_pre():
    """v0.14.10: Qwen branch anchors on the outer <pre.qwen-markdown-code>.
...
def test_why_user_authored_call_site_passes_adapter():
    """v0.14.10: the mount-time skip check must pass the adapter so
