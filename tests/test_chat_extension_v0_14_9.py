"""Regression guards for extension 0.14.9 (v4.49.2).
...
def test_skip_user_authored_does_not_dismiss_semantic_fingerprint():
    """v0.14.9: skip_user_authored MUST dismiss only the message
...
def test_duckai_gets_per_adapter_user_message_filter():
    """v0.14.9: DuckAI joins Grok in the per-adapter user-message check."""
...
def test_qwen_hoist_anchors_on_outer_pre_not_body():
    """v0.14.9: Qwen anchor is now the outer <pre.qwen-markdown-code>.
