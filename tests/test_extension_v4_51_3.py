...
def test_content_script_version_bumped():
    ...
    assert "const ARENA_CONTENT_SCRIPT_VERSION = '0.14.42';" in src


def test_insert_strategies_version_bumped():
    src = _read(INSERT_STRATEGIES_JS)
    assert "return '0.14.42';" in src
...
def test_parser_has_unlabeled_fence_pattern():
    ...
    assert ("v4.51.4" in src or "v4.52.0" in src or "v4.52.1" in src or "v4.52.2" in src or "v4.52.3" in src or "v4.52.4" in src or "v4.52.6" in src or "v4.53.0" in src or "v4.53.1" in src) or "v4.51.3" in src or "v4.52.0" in src or "v0.14.33" in src or "v0.14.32" in src or "v0.14.42" in src
