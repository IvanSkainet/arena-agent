...
def test_manifest_version_bumped():
    assert json.loads(_read(CHAT_EXT / "manifest.json"))["version"] == "0.14.34"


def test_content_script_version_bumped():
    assert "const ARENA_CONTENT_SCRIPT_VERSION = '0.14.34';" in _read(CHAT_EXT / "content.js")


def test_insert_strategies_version_bumped():
    assert "return '0.14.34';" in _read(CHAT_EXT / "insert_strategies.js")


def test_readme_mentions_v4_52_0():
    src = _read(CHAT_EXT / "README.md")
    assert "0.14.34" in src
    assert "v4.52.0" in src


def test_constants_version_bumped():
    assert 'VERSION = "4.52.0"' in _read(REPO_ROOT / "arena" / "constants.py")


def test_pyproject_version_bumped():
    assert 'version = "4.52.0"' in _read(REPO_ROOT / "pyproject.toml")
