"""v0.14.40 / v4.52.6 tests: Scan Now tab picker + auto-inject.
...
def test_readme_mentions_v4_52_6():
    ...
    assert "v4.52.6" in src


def test_constants_version_bumped():
    assert 'VERSION = "4.52.6"' in _read(REPO_ROOT / "arena" / "constants.py")


def test_pyproject_version_bumped():
    assert 'version = "4.52.6"' in _read(REPO_ROOT / "pyproject.toml")
