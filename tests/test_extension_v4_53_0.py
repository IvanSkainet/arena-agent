"""v0.14.43 / v4.53.0 tests: MCP-SA-style pretty preview + inline result.
...
def test_manifest_version_bumped():
    assert json.loads(_read(CHAT_EXT / "manifest.json"))["version"] in ("0.14.43",)


def test_content_script_version_bumped():
    assert "const ARENA_CONTENT_SCRIPT_VERSION = '0.14.43';" in _read(CHAT_EXT / "content.js")


def test_insert_strategies_version_bumped():
    assert "return '0.14.43';" in _read(CHAT_EXT / "insert_strategies.js")
