...
def test_an_android_phone_is_not_reported_as_plain_linux():
    ...
    assert hp.detect_host_class(ANDROID_ADB_ENV) == hp.ANDROID
    assert hp.is_android(ANDROID_ADB_ENV)
...
def test_termux_is_recognised_as_running_on_the_device():
    ...
    """
    assert hp.detect_host_class(TERMUX_ENV) == hp.ANDROID
    assert hp.is_termux(TERMUX_ENV) is True
    assert hp.termux_prefix(TERMUX_ENV) == "/data/data/com.termux/files/usr"

    described = hp.describe(TERMUX_ENV)
...
def test_adb_shell_is_android_but_not_termux():
    ...
    assert hp.is_android(ANDROID_ADB_ENV)
    assert hp.is_termux(ANDROID_ADB_ENV) is False
    assert hp.termux_prefix(ANDROID_ADB_ENV) is None
    assert hp.describe(ANDROID_ADB_ENV)["role"] == "android-host"
...
def test_a_linux_desktop_is_never_mistaken_for_a_phone(monkeypatch):
    ...
    assert hp.detect_host_class(LINUX_DESKTOP_ENV) == hp.LINUX
    assert hp.is_android(LINUX_DESKTOP_ENV) is False
    assert hp.is_termux(LINUX_DESKTOP_ENV) is False
...
def test_a_stray_android_data_variable_does_not_make_a_desktop_a_phone(monkeypatch):
    ...
    assert hp.detect_host_class(env) == hp.LINUX
...
def test_android_never_claims_systemd(env):
    ...
    """
    assert hp.has_systemd(env) is False
    assert hp.describe(env)["systemd"] is False


def test_windows_is_never_android(monkeypatch):
    """Windows must short-circuit before any Android probing.

    A Windows host with ANDROID_ROOT set (an SDK install) would
    otherwise be misclassified, and every service call would break.
    """
    monkeypatch.setattr(hp.sys, "platform", "win32")
    env = dict(ANDROID_ADB_ENV)
    assert hp.detect_host_class(env) == hp.WINDOWS
...
def test_detection_never_raises_on_a_hostile_environment():
    ...
    for env in ({}, {"ANDROID_ROOT": ""}, {"PREFIX": ""}, {"ANDROID_ROOT": "/"}):
        assert isinstance(hp.detect_host_class(env), str)
        assert isinstance(hp.describe(env), dict)
