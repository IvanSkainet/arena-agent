...
def test_memory_runtime_crud_recall_digest(tmp_path):
    ...
    runtime.write_fact({"key": "k1", "value": "hello world", "tags": ["unit"]})
    facts = runtime.load_facts()
    assert len(facts) == 1
    total, rows = runtime.search_facts_paged("hello")
    assert total == 1
    assert rows[0]["key"] == "k1"
    recall = runtime.recall_sync("hello", 5)
    assert recall["ok"] is True
    digest = runtime.recall_digest_sync()
    assert digest["ok"] is True
    assert runtime.delete_fact("k1") is True
